        ...
        echo '
			<tr>
				<td class="windowbg2" align="right" colspan="8">', $context['can_delete_members'] ? '
					...
					<input type="hidden" name="start" value="', $context['start'], '">', 'up' == $context['sort_direction'] ? '
...
function template_search_members()
{
				...
				<td colspan="5">', $txt['search_for'], ':</td>
						...
						<tr>
							...
							<td align="left" colspan="2">
								<label for="gender-0"><input type="checkbox" name="gender[]" value="0" id="gender-0" checked class="check"> ', $txt['undefined_gender'], '</label>&nbsp;&nbsp;
								<label for="gender-1"><input type="checkbox" name="gender[]" value="1" id="gender-1" checked class="check"> ', $txt[238], '</label>&nbsp;&nbsp;
								<label for="gender-2"><input type="checkbox" name="gender[]" value="2" id="gender-2" checked class="check"> ', $txt[239], '</label>
							</td>
							...
							<td align="left" colspan="2">
								<label for="activated-0"><input type="checkbox" name="activated[]" value="1" id="activated-0" checked class="check"> ', $txt['activated'], '</label>&nbsp;&nbsp;
								<label for="activated-1"><input type="checkbox" name="activated[]" value="0" id="activated-1" checked class="check"> ', $txt['not_activated'], '</label>
							</td>
			</tr>
		</table></div>
    ...
    foreach ($context['membergroups'] as $membergroup) {
        echo '
						<tr class="windowbg2">
							...
							<td align="center">
								<input type="checkbox" name="membergroups[1][]" value="', $membergroup['id'], '" checked class="check">
							</td>
							<td align="center">
								', $membergroup['can_be_additional'] ? '<input type="checkbox" name="membergroups[2][]" value="' . $membergroup['id'] . '" checked class="check">' : '', '
							</td>
						</tr>';
    }
    ...
    echo '
						<tr class="windowbg2">
							<td><em>', $txt[737], '</em></td>
							<td align="center"><input type="checkbox" onclick="invertAll(this, this.form, \'membergroups[1]\');" checked class="check"></td>
							<td align="center"><input type="checkbox" onclick="invertAll(this, this.form, \'membergroups[2]\');" checked class="check"></td>
						</tr>
					</table>
				</td>
        ...
        echo '
						<tr class="windowbg2">
							...
							<td width="40" align="center">
								<input type="checkbox" name="postgroups[]" value="', $postgroup['id'], '" checked class="check">
							</td>
						</tr>';
    }
    ...
    echo '
						<tr class="windowbg2">
							<td><em>', $txt[737], '</em></td>
							<td align="center"><input type="checkbox" onclick="invertAll(this, this.form, \'postgroups[]\');" checked class="check"></td>
						</tr>
					</table>
				</td>
			</tr>
...
function template_admin_browse()
{
    ...
    if (4 == $context['current_filter']) {
        ...
        echo '
							if (document.forms.postForm.todo.value.indexOf("delete") != -1)
								...
								message = "', 'approve' == $context['browse_type'] ? $txt['admin_browse_w_approve'] : $txt['admin_browse_w_activate'], '";';
    }
    ...
    if (empty($context['members'])) {
        echo '
			<tr class="windowbg2">
				<td colspan="6" align="center">', 'approve' == $context['browse_type'] ? $txt['admin_browse_no_members_approval'] : $txt['admin_browse_no_members_activate'], '</td>
			</tr>';
    } else {
        foreach ($context['members'] as $member) {
            echo '
        ...
        if (!empty($context['available_filters']) && count($context['available_filters']) > 1) {
            ...
            foreach ($context['available_filters'] as $filter) {
                echo '
									<option value="', $filter['type'], '"', $filter['selected'] ? ' selected="selected"' : '', '>', $filter['desc'], ' - ', $filter['amount'], ' ', 1 == $filter['amount'] ? $txt['user'] : $txt['users'], '</option>';
            }
        ...
        echo '
								...
								<input type="hidden" name="sa" value="approve">', 'up' == $context['sort_direction'] ? '
								<input type="hidden" name="desc" value="1">' : '', '
							</td>
						</tr>
					</table>
	...
	<form action="', $scripturl, '?action=viewmembers" method="post" accept-charset="', $context['character_set'], '" name="postFormOutstanding" id="postFormOutstanding" onsubmit="return onOutstandingSubmit();">
		<table border="0" cellspacing="0" cellpadding="4" align="center" width="100%" class="tborder">
			...
			<tr class="windowbg2">
				...
				<td align="left">
					<select name="todo">
						', 'activate' == $context['browse_type'] ? '
						<option value="ok">' . $txt['admin_browse_w_activate'] . '</option>' : '', '
						<option value="okemail">', 'approve' == $context['browse_type'] ? $txt['admin_browse_w_approve'] : $txt['admin_browse_w_activate'], ' ', $txt['admin_browse_w_email'], '</option>', 'activate' == $context['browse_type'] ? '' : '
						...
						<option value="deleteemail">', $txt['admin_browse_w_delete'], ' ', $txt['admin_browse_w_email'], '</option>', 'activate' == $context['browse_type'] ? '
