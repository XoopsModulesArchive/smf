...
<table border="0" cellpadding="4" cellspacing="1" align="center" class="bordercolor">
    ...
    if (!empty($context['activate_message']) || !empty($context['member']['bans'])) {
        ...
        if (!empty($context['activate_message'])) {
            echo '
				<tr>
					<td colspan="2">
						<span style="color: red;">', $context['activate_message'], '</span>&nbsp;(<a href="' . $scripturl . '?action=profile2;sa=activateAccount;userID=' . $context['member']['id'] . ';sesc=' . $context['session_id'] . '" ', (4
                                                                                                                                                                                                                                                  == $context['activate_type'] ? 'onclick="return confirm(\''
                                                                                                                                                                                                                                                         . $txt['profileConfirm']
                                                                                                                                                                                                                                                         . '\');"' : ''), '>', $context['activate_link_text'], '</a>)
					</td>
				</tr>';
        }
    ...
    echo '
					...
					<td><b>', $txt[113], ' </b></td>
					<td>
						<i>', $context['can_send_pm'] ? '<a href="' . $context['member']['online']['href'] . '" title="' . $context['member']['online']['label'] . '">' : '', $settings['use_image_buttons'] ? '<img src="'
                                                                                                                                                                                                               . $context['member']['online']['image_href']
                                                                                                                                                                                                               . '" alt="'
                                                                                                                                                                                                               . $context['member']['online']['text']
                                                                                                                                                                                                               . '" align="middle">' : $context['member']['online']['text'], $context['can_send_pm'] ? '</a>' : '', $settings['use_image_buttons'] ? '<span class="smalltext"> '
                                                                                                                                                                                                                                                                                                                                                       . $context['member']['online']['text']
                                                                                                                                                                                                                                                                                                                                                       . '</span>' : '', '</i>';
    ...
    foreach ($context['buddies'] as $buddy) {
        echo '
			<tr class="', $alternate ? 'windowbg' : 'windowbg2', '">
				...
				<td align="center">', ($buddy['hide_email'] ? '' : '<a href="' . $scripturl . '?action=useremail;u=' . $buddy['id'] . '"><img src="' . $settings['images_url'] . '/user_email.gif" alt="' . $txt[69] . '" title="' . $txt[69] . ' ' . $buddy['name'] . '"></a>'), '</td>
...
function template_trackUser()
{
    ...
    else { // For every error message print the IP address that caused it, the message displayed and the date it occurred.
...
function template_trackIP()
{
    ...
    } else { // Loop through each of the members and display them.
    ...
    } else { // For every message print the IP, member who posts it, subject (with link) and date posted.
    ...
    } else { // For each error print IP address, member, message received and date caused.
        foreach ($context['error_messages'] as $error) {
            echo '
				<tr>
					<td class="windowbg2">
				...
				<td align="left" colspan="2">
					...
					<form action="' . $scripturl . '?action=profile;u=', $context['member']['id'], ';sa=showPermissions#board_permissions" method="post" accept-charset="', $context['character_set'], '">
						...
						<select name="board" onchange="if (this.options[this.selectedIndex].value) this.form.submit();">
							<option value="0"', 0 == $context['board'] ? ' selected="selected"' : '', '>', $txt['showPermissions_global'], '</option>';
...
function template_statPanel()
{
    ...
    if (empty($context['posts_by_time'])) {
        ...
        foreach ($context['posts_by_time'] as $time_of_day) {
            echo '
							<td width="4%" valign="bottom" align="center" style="border-color: black; border-style: solid; border-width: 1px ', 23 != $time_of_day['hour'] ? '1px' : '0px', ' 0px 0px">', $time_of_day['hour'], '</td>';
        }
...
function template_account()
{
    ...
    if ($context['allow_edit_membergroups'] && $context['user']['is_owner'] && 1 == $context['member']['group']) {
        echo '
    ...
    if ($context['allow_edit_membergroups']) {
        ...
        foreach ($context['member_groups'] as $member_group) {
            if ($member_group['can_be_additional']) {
                echo '
										<label for="additionalGroups-', $member_group['id'], '"><input type="checkbox" name="additionalGroups[]" value="', $member_group['id'], '" id="additionalGroups-', $member_group['id'], '"', $member_group['is_additional'] ? ' checked' : '', ' class="check"> ', $member_group['name'], '</label><br>';
            }
        }
        ...
        if ($context['allow_hide_email']) {
            echo '
							<tr>
								...
								<td><input type="hidden" name="hideEmail" value="0"><input type="checkbox" name="hideEmail"', $context['member']['hide_email'] ? ' checked' : '', ' value="1" class="check"></td>
							</tr>';
        }
        ...
        if ($context['allow_hide_online']) {
            echo '
							<tr>
								...
								<td><input type="hidden" name="showOnline" value="0"><input type="checkbox" name="showOnline"', $context['member']['show_online'] ? ' checked' : '', ' value="1" class="check"></td>
							</tr>';
        }
							...
							<tr>
								<td width="40%" valign="top" style="padding: 0 2px;">
									<table width="100%" cellpadding="5" cellspacing="0" border="0" style="height: 25ex;"><tr>
										<td valign="top" width="20" class="windowbg"><input type="radio" name="avatar_choice" id="avatar_choice_server_stored" value="server_stored"', ('server_stored' == $context['member']['avatar']['choice'] ? ' checked' : ''), ' class="check"></td>
										<td valign="top" style="padding-left: 1ex;">
											...
											<div style="margin: 2ex;"><img name="avatar" id="avatar" src="', !empty($context['member']['avatar']['allow_external']) && 'external' == $context['member']['avatar']['choice'] ? $context['member']['avatar']['external'] : $modSettings['avatar_url']
                                                                                                                                                                                                                                                                         . '/blank.gif', '" alt="Do Nothing"></div>
										</td>
									</tr></table>
								</td>
								<td>
							...
							<tr>
								<td valign="top" style="padding: 0 2px;">
									<table width="100%" cellpadding="5" cellspacing="0" border="0"><tr>
										<td valign="top" width="20" class="windowbg"><input type="radio" name="avatar_choice" id="avatar_choice_external" value="external"', ('external' == $context['member']['avatar']['choice'] ? ' checked' : ''), ' class="check"></td>
        ...
        if (!empty($context['member']['avatar']['allow_upload'])) {
            echo '
							<tr>
								<td valign="top" style="padding: 0 2px;">
									<table width="100%" cellpadding="5" cellspacing="0" border="0"><tr>
										<td valign="top" width="20" class="windowbg"><input type="radio" name="avatar_choice" id="avatar_choice_upload" value="upload"', ('upload' == $context['member']['avatar']['choice'] ? ' checked' : ''), ' class="check"></td>
							...
							<tr>
								...
								<td>
									<select name="gender" size="1">
										...
										<option value="1"', ('m' == $context['member']['gender']['name'] ? ' selected="selected"' : ''), '>', $txt[238], '</option>
										<option value="2"', ('f' == $context['member']['gender']['name'] ? ' selected="selected"' : ''), '>', $txt[239], '</option>
									</select>
								</td>
							</tr><tr>
								<td colspan="2"><hr width="100%" size="1" class="hrcolor"></td>
...
function template_theme()
{
    ...
    if (!empty($modSettings['smiley_sets_enable'])) {
        ...
        echo '
									</select> <img id="smileypr" src="', 'none' != $context['member']['smiley_set']['id'] ? $modSettings['smileys_url']
                                                                                                                            . '/'
                                                                                                                            . (''
                                                                                                                               != $context['member']['smiley_set']['id'] ? $context['member']['smiley_set']['id'] : (!empty($settings['smiley_sets_default']) ? $settings['smiley_sets_default'] : $modSettings['smiley_sets_default']))
                                                                                                                            . '/smiley.gif' : $settings['images_url'] . '/blank.gif', '" alt=":)" align="top" style="padding-left: 20px;">
								</td>
							</tr>';
    }
									...
									<table width="100%" cellspacing="0" cellpadding="3">
										<tr>
											<td colspan="2">
												...
												<label for="show_board_desc"><input type="checkbox" name="default_options[show_board_desc]" id="show_board_desc" value="1"', !empty($context['member']['options']['show_board_desc']) ? ' checked' : '', ' class="check"> ', $txt[732], '</label>
											</td>
										</tr><tr>
											<td colspan="2">
												<input type="hidden" name="default_options[show_children]" value="0">
												<label for="show_children"><input type="checkbox" name="default_options[show_children]" id="show_children" value="1"', !empty($context['member']['options']['show_children']) ? ' checked' : '', ' class="check"> ', $txt['show_children'], '</label>
											</td>
										</tr><tr>
											<td colspan="2">
												<input type="hidden" name="default_options[show_no_avatars]" value="0">
												<label for="show_no_avatars"><input type="checkbox" name="default_options[show_no_avatars]" id="show_no_avatars" value="1"', !empty($context['member']['options']['show_no_avatars']) ? ' checked' : '', ' class="check"> ', $txt['show_no_avatars'], '</label>
											</td>
										</tr><tr>
											<td colspan="2">
												<input type="hidden" name="default_options[show_no_signatures]" value="0">
												<label for="show_no_signatures"><input type="checkbox" name="default_options[show_no_signatures]" id="show_no_signatures" value="1"', !empty($context['member']['options']['show_no_signatures']) ? ' checked' : '', ' class="check"> ', $txt['show_no_signatures'], '</label>
											</td>
										</tr>';
        ...
        echo '
										<tr>
											<td colspan="2">
												...
												<label for="show_no_censored"><input type="checkbox" name="default_options[show_no_censored]" id="show_no_censored" value="1"'
             . (!empty($context['member']['options']['show_no_censored']) ? ' checked' : '')
             . ' class="check"> '
             . $txt['show_no_censored']
             . '</label>
											</td>
										</tr>';
    }
    ...
    echo '
										<tr>
											<td colspan="2">
												...
												<label for="return_to_post"><input type="checkbox" name="default_options[return_to_post]" id="return_to_post" value="1"', !empty($context['member']['options']['return_to_post']) ? ' checked' : '', ' class="check"> ', $txt['return_to_post'], '</label>
											</td>
										</tr>
										<tr>
											<td colspan="2">
												<input type="hidden" name="default_options[no_new_reply_warning]" value="0">
												<label for="no_new_reply_warning"><input type="checkbox" name="default_options[no_new_reply_warning]" id="no_new_reply_warning" value="1"', !empty($context['member']['options']['no_new_reply_warning']) ? ' checked' : '', ' class="check"> ', $txt['no_new_reply_warning'], '</label>
											</td>
										</tr>
										<tr>
											<td colspan="2">
												<input type="hidden" name="default_options[view_newest_first]" value="0">
												<label for="view_newest_first"><input type="checkbox" name="default_options[view_newest_first]" id="view_newest_first" value="1"', !empty($context['member']['options']['view_newest_first']) ? ' checked' : '', ' class="check"> ', $txt['recent_posts_at_top'], '</label>
											</td>
										</tr><tr>
											<td colspan="2">
												<input type="hidden" name="default_options[view_newest_pm_first]" value="0">
												<label for="view_newest_pm_first"><input type="checkbox" name="default_options[view_newest_pm_first]" id="view_newest_pm_first" value="1"', !empty($context['member']['options']['view_newest_pm_first']) ? ' checked' : '', ' class="check"> ', $txt['recent_pms_at_top'], '</label>
											</td>
										</tr><tr>
											<td colspan="2"><label for="calendar_start_day">', $txt['calendar_start_day'], ':</label>
												<select name="default_options[calendar_start_day]" id="calendar_start_day">
													...
													<option value="1"', !empty($context['member']['options']['calendar_start_day']) && 1 == $context['member']['options']['calendar_start_day'] ? ' selected="selected"' : '', '>', $txt['days'][1], '</option>
													<option value="6"', !empty($context['member']['options']['calendar_start_day']) && 6 == $context['member']['options']['calendar_start_day'] ? ' selected="selected"' : '', '>', $txt['days'][6], '</option>
												</select>
											</td>
										</tr><tr>
											<td colspan="2"><label for="display_quick_reply">', $txt['display_quick_reply'], '</label>
												<select name="default_options[display_quick_reply]" id="display_quick_reply">
													...
													<option value="1"', !empty($context['member']['options']['display_quick_reply']) && 1 == $context['member']['options']['display_quick_reply'] ? ' selected="selected"' : '', '>', $txt['display_quick_reply2'], '</option>
													<option value="2"', !empty($context['member']['options']['display_quick_reply']) && 2 == $context['member']['options']['display_quick_reply'] ? ' selected="selected"' : '', '>', $txt['display_quick_reply3'], '</option>
												</select>
											</td>
										</tr><tr>
											<td colspan="2"><label for="display_quick_mod">', $txt['display_quick_mod'], '</label>
												<select name="default_options[display_quick_mod]" id="display_quick_mod">
													...
													<option value="1"', !empty($context['member']['options']['display_quick_mod']) && 1 == $context['member']['options']['display_quick_mod'] ? ' selected="selected"' : '', '>', $txt['display_quick_mod_check'], '</option>
													<option value="2"', !empty($context['member']['options']['display_quick_mod']) && 1 != $context['member']['options']['display_quick_mod'] ? ' selected="selected"' : '', '>', $txt['display_quick_mod_image'], '</option>
												</select>
											</td>
										</tr>
									</table>
...
function template_notification()
{
    ...
    if (!empty($modSettings['allow_disableAnnounce'])) {
        echo '
							...
							<label for="notifyAnnouncements"><input type="checkbox" id="notifyAnnouncements" name="notifyAnnouncements"', !empty($context['member']['notify_announcements']) ? ' checked' : '', ' class="check"> ', $txt['notifyXAnn4'], '</label><br>';
    }
    ...
    echo '
							...
							<label for="notifyOnce"><input type="checkbox" id="notifyOnce" name="notifyOnce"', !empty($context['member']['notify_once']) ? ' checked' : '', ' class="check"> ', $txt['notifyXOnce1'], '</label><br>

							<input type="hidden" name="default_options[auto_notify]" value="0">
							<label for="auto_notify"><input type="checkbox" id="auto_notify" name="default_options[auto_notify]" value="1"', !empty($context['member']['options']['auto_notify']) ? ' checked' : '', ' class="check"> ', $txt['auto_notify'], '</label><br>';

    if (empty($modSettings['disallow_sendBody'])) {
        echo '
							<input type="hidden" name="notifySendBody" value="0">
							<label for="notifySendBody"><input type="checkbox" id="notifySendBody" name="notifySendBody"', !empty($context['member']['notify_send_body']) ? ' checked' : '', ' class="check"> ', $txt['notify_send_body'], '</label><br>';
    }
    ...
    echo '
							...
							<select name="notifyTypes" id="notifyTypes">
								<option value="1"', 1 == $context['member']['notify_types'] ? ' selected="selected"' : '', '>', $txt['notify_send_type_everything'], '</option>
								<option value="2"', 2 == $context['member']['notify_types'] ? ' selected="selected"' : '', '>', $txt['notify_send_type_everything_own'], '</option>
								<option value="3"', 3 == $context['member']['notify_types'] ? ' selected="selected"' : '', '>', $txt['notify_send_type_only_replies'], '</option>
								<option value="4"', 4 == $context['member']['notify_types'] ? ' selected="selected"' : '', '>', $txt['notify_send_type_nothing'], '</option>
							</select><br>
    ...
    echo '
							<tr>
								<td colspan="2">
									...
									<label for="copy_to_outbox"><input type="checkbox" name="default_options[copy_to_outbox]" id="copy_to_outbox" value="1"', !empty($context['member']['options']['copy_to_outbox']) ? ' checked' : '', ' class="check"> ', $txt['copy_to_outbox'], '</label><br>
									<input type="hidden" name="default_options[popup_messages]" value="0">
									<label for="popup_messages"><input type="checkbox" name="default_options[popup_messages]" id="popup_messages" value="1"', !empty($context['member']['options']['popup_messages']) ? ' checked' : '', ' class="check"> ', $txt['popup_messages'], '</label><br>
									...
									<select name="pm_email_notify" id="pm_email_notify">
										...
										<option value="1"', !empty($context['send_email']) && (1 == $context['send_email'] || (empty($modSettings['enable_buddylist']) && $context['send_email'] > 1)) ? ' selected="selected"' : '', '>', $txt['email_notify_always'], '</option>';
...
function template_error_message()
{
    ...
    foreach ($context['post_errors'] as $error) { //if (isset($txt['profile_error_' . $error]))
        echo '
