...
function template_permission_index()
{
    ...
    foreach ($context['groups'] as $group) {
        echo '
				<tr>
					<td class="windowbg2">', $group['name'], -1 == $group['id'] ? ' (<a href="' . $scripturl . '?action=helpadmin;help=membergroup_guests" onclick="return reqWin(this.href);">?</a>)' : (0 == $group['id'] ? ' (<a href="'
                                                                                                                                                                                                                              . $scripturl
                                                                                                                                                                                                                              . '?action=helpadmin;help=membergroup_regular_members" onclick="return reqWin(this.href);">?</a>)' : (1
                                                                                                                                                                                                                                                                                                                                    == $group['id'] ? ' (<a href="'
                                                                                                                                                                                                                                                                                                                                           . $scripturl
                                                                                                                                                                                                                                                                                                                                           . '?action=helpadmin;help=membergroup_administrator" onclick="return reqWin(this.href);">?</a>)' : (3
                                                                                                                                                                                                                                                                                                                                                                                                                                               == $group['id'] ? ' (<a href="'
                                                                                                                                                                                                                                                                                                                                                                                                                                                      . $scripturl
                                                                                                                                                                                                                                                                                                                                                                                                                                                      . '?action=helpadmin;help=membergroup_moderator" onclick="return reqWin(this.href);">?</a>)' : ''))), '</td>
					<td class="windowbg" align="center">', $group['can_search'] ? $group['link'] : $group['num_members'], '</td>
					<td class="windowbg2" align="center"', 1 == $group['id'] ? ' style="font-style: italic;"' : '', '>';
        ...
        if (empty($modSettings['permission_enable_deny'])) {
            ...
            echo '
						<div style="float: left; width: 50%;">', $group['num_permissions']['allowed'], '</div> ', empty($group['num_permissions']['denied']) || 1 == $group['id'] ? $group['num_permissions']['denied'] : (-1 == $group['id'] ? '<span style="font-style: italic;">'
        ...
        if (!empty($context['board'])) {
            ...
            if (1 != $group['id'] && 3 != $group['id']) {
                echo '
						<input type="checkbox" name="access[', $group['id'], ']" value="', $group['id'], '" ', $group['access'] ? ' checked' : '', ' class="check">';
            }
    ...
    foreach ($context['permissions'] as $permissionType) {
        if ('membergroup' == $permissionType['id'] && !empty($context['board'])) {
            continue;
        }
        ...
        echo '
				<tr class="windowbg2">
					<td align="left" class="windowbg">
						<b><a', $board['use_local_permissions'] ? ' href="' . $scripturl . '?action=permissions;boardid=' . $board['id'] . '"' : '', ' name="', $board['id'], '"> ', str_repeat('-', $board['child_level']), ' '
                                                                                                                                                                                                                             . $board['name']
                                                                                                                                                                                                                             . '</a></b>', empty($modSettings['permission_enable_by_board']) ? '' : ' ('
                                                                                                                                                                                                                                                                                                    . ($board['use_local_permissions'] ? $txt['permissions_local'] : $txt['permissions_global'])
                                                                                                                                                                                                                                                                                                    . ')', '
					</td>';
        ...
        if (empty($modSettings['permission_enable_by_board'])) {
            echo '
					<td align="center" style="font-weight: ', 'normal'
                                                              == $board['permission_mode'] ? 'bold' : 'normal', ';"><a href="', $scripturl, '?action=permissions;sa=board;mode=0;boardid=', $board['id'], ';sesc=', $context['session_id'], '#', $board['id'], '">', $txt['permission_mode_normal'], '</a></td>
					<td align="center" style="font-weight: ', 'no_polls'
                                                              == $board['permission_mode'] ? 'bold' : 'normal', ';"><a href="', $scripturl, '?action=permissions;sa=board;mode=2;boardid=', $board['id'], ';sesc=', $context['session_id'], '#', $board['id'], '">', $txt['permission_mode_no_polls'], '</a></td>
					<td align="center" style="font-weight: ', 'reply_only'
                                                              == $board['permission_mode'] ? 'bold' : 'normal', ';"><a href="', $scripturl, '?action=permissions;sa=board;mode=3;boardid=', $board['id'], ';sesc=', $context['session_id'], '#', $board['id'], '">', $txt['permission_mode_reply_only'], '</a></td>
					<td align="center" style="font-weight: ', 'read_only'
                                                              == $board['permission_mode'] ? 'bold' : 'normal', ';"><a href="', $scripturl, '?action=permissions;sa=board;mode=4;boardid=', $board['id'], ';sesc=', $context['session_id'], '#', $board['id'], '">', $txt['permission_mode_read_only'], '</a></td>';
        } else {
            echo '
...
function template_modify_group()
{
    ...
    if (!empty($modSettings['permission_enable_deny']) && -1 != $context['group']['id']) {
        echo '
				<tr class="windowbg">
    ...
    foreach ($context['permissions'] as $permission_type) {
        if ($permission_type['show']) {
            ...
            if ($context['local']) {
                ...
                echo '
						', 'membergroup' == $permission_type['id'] ? $txt['permissions_general'] : $txt['permissions_board'], ' - <span style="color: red;">', $context['group']['name'], '</span>';
            }
                        ...
                        foreach ($permissionGroup['permissions'] as $permission) {
							...
							<tr class="', $alternate ? 'windowbg' : 'windowbg2', '">
								<td valign="top" width="10" style="padding-right: 1ex;">
									', $permission['show_help'] ? '<a href="'
                                                                  . $scripturl
                                                                  . '?action=helpadmin;help=permissionhelp_'
                                                                  . $permission['id']
                                                                  . '" onclick="return reqWin(this.href);" class="help"><img src="'
                                                                  . $settings['images_url']
                                                                  . '/helptopics.gif" alt="'
                                                                  . $txt[119]
                                                                  . '"></a>' : '', '
                            ...
                            if ($permission['has_own_any']) {
                                ...
                                if (empty($modSettings['permission_enable_deny']) || -1 == $context['group']['id']) {
                                    echo '
								<td colspan="3"><input type="checkbox" name="perm[', $permission_type['id'], '][', $permission['own']['id'], ']"', 'on' == $permission['own']['select'] ? ' checked' : '', ' value="on" id="', $permission['own']['id'], '_on" class="check"></td>';
                                } else {
                                    echo '
								<td valign="top" width="10"><input type="radio" name="perm[', $permission_type['id'], '][', $permission['own']['id'], ']"', 'on' == $permission['own']['select'] ? ' checked' : '', ' value="on" id="', $permission['own']['id'], '_on" class="check"></td>
								<td valign="top" width="10"><input type="radio" name="perm[', $permission_type['id'], '][', $permission['own']['id'], ']"', 'off' == $permission['own']['select'] ? ' checked' : '', ' value="off" class="check"></td>
								<td valign="top" width="10"><input type="radio" name="perm[', $permission_type['id'], '][', $permission['own']['id'], ']"', 'denied' == $permission['own']['select'] ? ' checked' : '', ' value="deny" class="check"></td>';
                                }
                                ...
                                if (empty($modSettings['permission_enable_deny']) || -1 == $context['group']['id']) {
                                    echo '
								<td colspan="3" style="padding-bottom: 1.5ex;"><input type="checkbox" name="perm[', $permission_type['id'], '][', $permission['any']['id'], ']"', 'on' == $permission['any']['select'] ? ' checked' : '', ' value="on" class="check"></td>';
                                } else {
                                    echo '
								<td valign="top" width="10" style="padding-bottom: 1.5ex;"><input type="radio" name="perm[', $permission_type['id'], '][', $permission['any']['id'], ']"', 'on'
                                                                                                                                                                                           == $permission['any']['select'] ? ' checked' : '', ' value="on" onclick="document.forms.permissionForm.', $permission['own']['id'], '_on.checked = true;" class="check"></td>
								<td valign="top" width="10" style="padding-bottom: 1.5ex;"><input type="radio" name="perm[', $permission_type['id'], '][', $permission['any']['id'], ']"', 'off' == $permission['any']['select'] ? ' checked' : '', ' value="off" class="check"></td>
								<td valign="top" width="10" style="padding-bottom: 1.5ex;"><input type="radio" name="perm[', $permission_type['id'], '][', $permission['any']['id'], ']"', 'denied'
                                                                                                                                                                                           == $permission['any']['select'] ? ' checked' : '', ' value="deny" id="', $permission['any']['id'], '_deny" onclick="window.smf_usedDeny = true;" class="check"></td>';
                                }
                                ...
                                if (empty($modSettings['permission_enable_deny']) || -1 == $context['group']['id']) {
                                    echo '
								<td valign="top" style="padding-bottom: 2px;"><input type="checkbox" name="perm[', $permission_type['id'], '][', $permission['id'], ']"', 'on' == $permission['select'] ? ' checked' : '', ' value="on" class="check"></td>';
                                } else {
                                    echo '
								<td valign="top" width="10" style="padding-bottom: 2px;"><input type="radio" name="perm[', $permission_type['id'], '][', $permission['id'], ']"', 'on' == $permission['select'] ? ' checked' : '', ' value="on" class="check"></td>
								<td valign="top" width="10" style="padding-bottom: 2px;"><input type="radio" name="perm[', $permission_type['id'], '][', $permission['id'], ']"', 'off' == $permission['select'] ? ' checked' : '', ' value="off" class="check"></td>
								<td valign="top" width="10" style="padding-bottom: 2px;"><input type="radio" name="perm[', $permission_type['id'], '][', $permission['id'], ']"', 'denied'
                                                                                                                                                                                  == $permission['select'] ? ' checked' : '', ' value="deny" onclick="window.smf_usedDeny = true;" class="check"></td>';
                                }
...
';
    ...
    echo '
			<tr class="windowbg2">
				...
				<td>
					<input type="checkbox" name="permission_enable_deny" id="permission_enable_deny_check"', empty($modSettings['permission_enable_deny']) ? '' : ' checked', ' class="check"', empty($modSettings['permission_enable_deny']) ? '' : ' onclick="if (!this.checked) alert(\''
				...
				<td>
					<input type="checkbox" name="permission_enable_postgroups" id="permission_enable_postgroups_check"', empty($modSettings['permission_enable_postgroups']) ? '' : ' checked', ' class="check"', empty($modSettings['permission_enable_postgroups']) ? '' : ' onclick="if (!this.checked) alert(\''
				...
				<td>
					<input type="checkbox" name="permission_enable_by_board" id="permission_enable_by_board_check"', empty($modSettings['permission_enable_by_board']) ? '' : ' checked', ' class="check"', empty($modSettings['permission_enable_by_board']) ? '' : ' onclick="if (!this.checked) alert(\''
...
function template_inline_permissions()
{
    ...
    foreach ($context['member_groups'] as $group) {
        ...
        if (empty($modSettings['permission_enable_deny'])) {
            echo '
					<td align="center"><input type="checkbox" name="', $context['current_permission'], '[', $group['id'], ']" value="on"', 'on' == $group['status'] ? ' checked' : '', ' class="check"></td>';
        } else {
            echo '
					<td align="center"><input type="radio" name="', $context['current_permission'], '[', $group['id'], ']" value="on"', 'on' == $group['status'] ? ' checked' : '', ' class="check"></td>
					<td align="center"><input type="radio" name="', $context['current_permission'], '[', $group['id'], ']" value="off"', 'off' == $group['status'] ? ' checked' : '', ' class="check"></td>
					<td align="center"><input type="radio" name="', $context['current_permission'], '[', $group['id'], ']" value="deny"', 'deny' == $group['status'] ? ' checked' : '', ' class="check"></td>';
        }
