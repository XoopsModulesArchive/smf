...
function template_main()
{
    ...
    echo '
							<tr', empty($context['post_error']['messages']) ? ' style="display: none"' : '', ' id="errors">
								<td></td>
								<td align="left">
									<div style="padding: 0px; font-weight: bold;', empty($context['error_type']) || 'serious' != $context['error_type'] ? ' display: none;' : '', '" id="error_serious">
    ...
    if ($context['make_poll']) {
        echo '
							<tr>
								...
								<td align="left">
									<input type="text" name="question" value="', $context['question'] ?? '', '" tabindex="', $context['tabindex']++, '" size="80">
								</td>
							</tr>
							<tr>
								<td align="right"></td>
        ...
        echo '
							...
							<tr>
								<td align="right"></td>
								<td class="smalltext"><label for="poll_change_vote"><input type="checkbox" id="poll_change_vote" name="poll_change_vote"', !empty($context['poll_options']['change_vote']) ? ' checked' : '', ' class="check"> ', $txt['poll_options7'], '</label></td>
							</tr>
							<tr>
								<td align="right"></td>
								<td class="smalltext">
									<input type="radio" id="poll_hide" name="poll_hide" value="0"', 0 == $context['poll_options']['hide'] ? ' checked' : '', ' class="check"> ', $txt['poll_options2'], '<br>
									<input type="radio" id="poll_hide" name="poll_hide" value="1"', 1 == $context['poll_options']['hide'] ? ' checked' : '', ' class="check"> ', $txt['poll_options3'], '<br>
									<input type="radio" id="poll_hide" name="poll_hide" value="2"', 2 == $context['poll_options']['hide'] ? ' checked' : '', empty($context['poll_options']['expire']) ? ' disabled="disabled"' : '', ' class="check"> ', $txt['poll_options4'], '<br>
									<br>
								</td>
							</tr>';
    }
										...
										<td>
											<div id="postMoreOptions">
												<table width="80%" cellpadding="0" cellspacing="0" border="0">
													<tr>
														<td class="smalltext">', $context['can_notify'] ? '<input type="hidden" name="notify" value="0"><label for="check_notify"><input type="checkbox" name="notify" id="check_notify"'
                                                                                                          . ($context['notify']
                                                                                                             || !empty($options['auto_notify']) ? ' checked' : '')
                                                                                                          . ' value="1" class="check"> '
                                                                                                          . $txt['smf14']
                                                                                                          . '</label>' : '', '</td>
														<td class="smalltext">', $context['can_lock'] ? '<input type="hidden" name="lock" value="0"><label for="check_lock"><input type="checkbox" name="lock" id="check_lock"'
                                                                                                        . ($context['locked'] ? ' checked' : '')
                                                                                                        . ' value="1" class="check"> '
                                                                                                        . $txt['smf15']
                                                                                                        . '</label>' : '', '</td>
													</tr>
													<tr>
														<td class="smalltext"><label for="check_back"><input type="checkbox" name="goback" id="check_back"'
                                                                                                                           . ($context['back_to_topic'] || !empty($options['return_to_post']) ? ' checked' : '')
                                                                                                                           . ' value="1" class="check"> '
                                                                                                                           . $txt['back_to_topic']
                                                                                                                           . '</label></td>
														<td class="smalltext">', $context['can_sticky'] ? '<input type="hidden" name="sticky" value="0"><label for="check_sticky"><input type="checkbox" name="sticky" id="check_sticky"'
													...
													<tr>
														<td class="smalltext"><label for="check_smileys"><input type="checkbox" name="ns" id="check_smileys"', $context['use_smileys'] ? '' : ' checked', ' value="NS" class="check"> ', $txt[277], '</label></td>', '
    ...
    if (!empty($context['current_attachments'])) {
        ...
        foreach ($context['current_attachments'] as $attachment) {
            echo '
									<input type="checkbox" name="attach_del[]" value="', $attachment['id'], '"', empty($attachment['unchecked']) ? ' checked' : '', ' class="check"> ', $attachment['name'], '<br>';
        }
...
function template_postbox($message)
{
    ...
    echo '
			<tr>
				...
				<td>
					<textarea class="editor" name="', $context['post_box_name'], '" rows="', $context['post_box_rows'], '" cols="', $context['post_box_columns'], '" onselect="storeCaret(this);" onclick="storeCaret(this);" onkeyup="storeCaret(this);" onchange="storeCaret(this);" tabindex="', $context['tabindex']++, '"', isset($context['post_error']['no_message'])
                                                                                                                                                                                                                                                                                                                                 || isset($context['post_error']['long_message']) ? ' style="border: 1px solid red;"' : '', '>', $message, '</textarea>
				</td>
			</tr>';
}
...
function template_announce()
{
    ...
    foreach ($context['groups'] as $group) {
        echo '
						<label for="who_', $group['id'], '"><input type="checkbox" name="who[', $group['id'], ']" id="who_', $group['id'], '" value="', $group['id'], '" checked class="check"> ', $group['name'], '</label> <i>(', $group['member_count'], ')</i><br>';
    }

    echo '
						<br>
						<label for="checkall"><input type="checkbox" id="checkall" class="check" onclick="invertAll(this, this.form);" checked> <i>', $txt[737], '</i></label>
					</td>
				</tr><tr>
