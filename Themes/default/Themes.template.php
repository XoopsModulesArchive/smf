    ...
    echo '
		<form action="', $scripturl, '?action=theme;sa=admin" method="post" accept-charset="', $context['character_set'], '">
			...
			<table width="80%" cellpadding="4" cellspacing="0" border="0" align="center" class="tborder">
				...
				<tr class="windowbg2">
					<td colspan="3"><label for="options-theme_allow"><input type="checkbox" name="options[theme_allow]" id="options-theme_allow" value="1"', !empty($modSettings['theme_allow']) ? ' checked' : '', ' class="check"> ', $txt['theme_allow'], '</label></td>
				</tr>
				<tr class="windowbg2">
					<td colspan="3"><label for="options-theme_default"><input type="checkbox" name="options[theme_default]" id="options-theme_default" value="1"', !empty($modSettings['theme_default']) ? ' checked' : '', ' class="check"> ', $txt['theme_default'], '</label></td>
				</tr>
				<tr class="windowbg2">
        ...
        echo '
			...
			<tr class="windowbg2">
				<td style="padding-left: 5ex;" class="smalltext">
					<div style="padding-bottom: 2px;"><div style="float: left; width: 38ex; padding-bottom: 2px;">', $txt['themeadmin_list_theme_dir'], ':</div> <b style="', $theme['valid_path'] ? '' : 'color: red; ', 'white-space: nowrap;">', $theme['theme_dir'], '</b>', $theme['valid_path'] ? '' : ' '
                                                                                                                                                                                                                                                                                                             . $txt['themeadmin_list_invalid'], '</div>
    ...
    foreach ($context['options'] as $setting) {
        ...
        if ('checkbox' == $setting['type']) {
            echo '
						...
						<label for="options_', $setting['id'], '"><input type="checkbox" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="options_', $setting['id'], '"', !empty($setting['value']) ? ' checked' : '', $context['theme_options_reset'] ? ' disabled="disabled"' : '', ' value="1" class="check"> ', $setting['label'], '</label>';
        } elseif ('list' == $setting['type']) {
            echo '
            ...
            echo '
						&nbsp;<label for="options_', $setting['id'], '">', $setting['label'], '</label>
						<input type="text" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="options_', $setting['id'], '" value="', $setting['value'], '"', 'number'
                                                                                                                                                                                                       == $setting['type'] ? ' size="5"' : '', $context['theme_options_reset'] ? ' disabled="disabled"' : '', '>';
        }
...
function template_set_settings()
{
    ...
    if (1 != $context['theme_settings']['theme_id']) {
        echo '
				<tr class="catbg">
    ...
    foreach ($context['settings'] as $setting) {
        ...
        if ('checkbox' == $setting['type']) {
            echo '
					<input type="hidden" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" value="0">
					<label for="', $setting['id'], '"><input type="checkbox" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '"', !empty($setting['value']) ? ' checked' : '', ' value="1" class="check"> ', $setting['label'], '</label>';
        } elseif ('list' == $setting['type']) {
            echo '
            ...
            echo '
					<label for="', $setting['id'], '">', $setting['label'], '</label>
					<input type="text" name="', !empty($setting['default']) ? 'default_' : '', 'options[', $setting['id'], ']" id="', $setting['id'], '" value="', $setting['value'], '"', 'number' == $setting['type'] ? ' size="5"' : ' size="40"', '>';
        }
	...
	<table align="center" width="85%" cellpadding="3" cellspacing="0" border="0" class="tborder">
		...
		<tr class="', $theme['selected'] ? 'windowbg' : 'windowbg2', '">
			<td valign="bottom" align="right" style="padding: 6px; padding-top: 0;">
				<div style="float: left;" class="smalltext"><i>', $theme['num_users'], ' ', (1 == $theme['num_users'] ? $txt['theme_user'] : $txt['theme_users']), '</i></div>
