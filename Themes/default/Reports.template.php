    ...
    foreach ($context['report_types'] as $type) {
        echo '
				<tr class="', $alternate ? 'windowbg' : 'windowbg2', '" valign="top">
					<td width="20">
						<input type="radio" id="rt_', $type['id'], '" name="rt" value="', $type['id'], '"', $type['is_first'] ? ' checked' : '', ' class="check">
					</td>
...
function template_main()
{
    ...
    if (empty($settings['use_tabs'])) {
        echo '
					...
					<a href="', $scripturl, '?action=reports;rt=', $context['report_type'], ';st=print" target="_blank">', ($settings['use_image_buttons'] ? '<img src="'
                                                                                                                                                             . $settings['images_url']
                                                                                                                                                             . '/'
    ...
    foreach ($context['tables'] as $table) {
        ...
        foreach ($table['data'] as $row) {
            if (0 == $row_number && !empty($table['shading']['top'])) {
                echo '
            ...
            foreach ($row as $key => $data) {
                ...
                if (!empty($data['seperator']) && 0 == $column_number) {
                    echo '
                ...
                if (0 == $column_number && !empty($table['shading']['left'])) {
                    echo '
				<td align="', $table['align']['shaded'], '" class="titlebg" ', 'auto' != $table['width']['shaded'] ? 'width="' . $table['width']['shaded'] . '"' : '', '>
					', $data['value'] == $table['default_value'] ? '' : ($data['value'] . (empty($data['value']) ? '' : ':')), '
				</td>';
                } else {
                    echo '
				<td align="', $table['align']['normal'], '" ', 'auto' != $table['width']['normal'] ? 'width="' . $table['width']['normal'] . '"' : '', ' ', !empty($data['style']) ? 'style="' . $data['style'] . '"' : '', '>
					', $data['value'], '
				</td>';
                }
...
function template_print()
{
    ...
    foreach ($context['tables'] as $table) {
        echo '
		<div style="overflow: visible; ', 'auto' != $table['max_width'] ? 'width: ' . $table['max_width'] . 'px;' : '', '">
