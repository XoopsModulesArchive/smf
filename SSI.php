...
if (2 == $maintenance && (!isset($ssi_maintenance_off) || true !== $ssi_maintenance_off)) {
    die($mmessage);
}

// Fix for using the current directory as a path.
if ('.' == mb_substr($sourcedir, 0, 1) && '.' != mb_substr($sourcedir, 1, 1)) {
    $sourcedir = __DIR__ . mb_substr($sourcedir, 1);
}
...
if (false === mb_strpos($db_prefix, '.')) {
    $db_prefix = is_numeric(mb_substr($db_prefix, 0, 1)) ? $db_name . '.' . $db_prefix : '`' . $db_name . '`.' . $db_prefix;
} else {
    @mysqli_select_db($GLOBALS['xoopsDB']->conn, $db_name, $db_connection);
}
...
if (isset($_REQUEST['GLOBALS']) || isset($_COOKIE['GLOBALS'])) {
    ...
    if ((get_magic_quotes_gpc() ? addslashes($_REQUEST['ssi_layers']) : $_REQUEST['ssi_layers']) == htmlspecialchars($ssi_layers, ENT_QUOTES | ENT_HTML5)) {
        die('Hacking attempt...');
    }
}
...
if (isset($ssi_gzip) && true === $ssi_gzip && '1' != @ini_get('zlib.output_compression') && 'ob_gzhandler' != @ini_get('outputHandler') && -1 != @version_compare(PHP_VERSION, '4.2.0')) {
...
if (isset($_REQUEST['ssi_ban']) || (isset($ssi_ban) && true === $ssi_ban)) {
    is_not_banned();
}
...
if (!isset($_SESSION['USER_AGENT']) && (!isset($_GET['ssi_function']) || 'pollVote' !== $_GET['ssi_function'])) {
...
elseif ('SSI.php' == basename($_SERVER['PHP_SELF'])) {
...
{
    if (!isset($_GET['ssi_function']) || 'shutdown' != $_GET['ssi_function']) {
        template_footer();
    }
}
...
function ssi_welcome($output_method = 'echo')
{
    ...
    if ('echo' == $output_method) {
        if ($context['user']['is_guest']) {
            echo $txt['welcome_guest'];
        } else {
            echo $txt['hello_member'], ' <b>', $context['user']['name'], '</b>', allowedTo('pm_read') ? ', '
                                                                                                        . $txt[152]
                                                                                                        . ' <a href="'
                                                                                                        . $scripturl
                                                                                                        . '?action=pm">'
                                                                                                        ...
                                                                                                        . ' '
                                                                                                        . ('1' == $context['user']['messages'] ? $txt[471] : $txt[153])
                                                                                                        . '</a>'
                                                                                                        . $txt['newmessages4']
                                                                                                        . ' '
                                                                                                        . $context['user']['unread_messages']
                                                                                                        . ' '
                                                                                                        . ('1' == $context['user']['unread_messages'] ? $txt['newmessages0'] : $txt['newmessages1']) : '', '.';
        }
...
function ssi_recentPosts($num_recent = 8, $exclude_boards = null, $output_method = 'echo')
{
    ...
    if (null === $exclude_boards && !empty($modSettings['recycle_enable']) && $modSettings['recycle_board'] > 0) {
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        $row['body'] = strip_tags(strtr(parse_bbc($row['body'], $row['smileysEnabled'], $row['ID_MSG']), ['<br>' => '&#10;']));
    ...
    if ('echo' != $output_method || empty($posts)) {
        return $posts;
    }
...
function ssi_topBoards($num_top = 10, $output_method = 'echo')
{
    ...
    if ('echo' != $output_method || empty($boards)) {
        return $boards;
    }
...
function ssi_topTopics($type = 'replies', $num_topics = 10, $output_method = 'echo')
{
    ...
    if ($modSettings['totalMessages'] > 100000) {
        $request = db_query(
			...
			WHERE num" . ('replies' != $type ? 'Views' : 'Replies') . ' != 0
			ORDER BY num' . ('replies' != $type ? 'Views' : 'Replies') . ' DESC
			LIMIT 100',
            __FILE__,
            __LINE__
        );
    ...
    $request = db_query(
		...
		ORDER BY t.num' . ('replies' != $type ? 'Views' : 'Replies') . " DESC
		LIMIT $num_topics",
        __FILE__,
        __LINE__
    );
    ...
    if ('echo' != $output_method || empty($topics)) {
        return $topics;
    }
...
function ssi_boardStats($output_method = 'echo')
{
    ...
    [$totals['boards']] = $GLOBALS['xoopsDB']->fetchRow($result);
    ...
    [$totals['categories']] = $GLOBALS['xoopsDB']->fetchRow($result);
...
function ssi_whosOnline($output_method = 'echo')
{
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
        if (!isset($row['realName'])) {
            ...
            $is_spider = ob_googlebot_getAgent($actions['USER_AGENT'], $spider_name, $agent, 0 == $row['ID_MEMBER']);
            ...
            if ($show_buddies && in_array($row['ID_MEMBER'], $user_info['buddies'], true)) {
                $return['buddies']++;

                $link = '<b>' . $link . '</b>';
            }

            $return['users'][$row['logTime'] . $row['memberName']] = [
                'id' => $row['ID_MEMBER'],
    ...
    echo '
		', (empty($return['spiders']) ? '' : $return['num_spiders'] . ' ' . (1 == $return['num_spiders'] ? $txt['ob_googlebot_spider'] : $txt['ob_googlebot_spiders']) . ', '), $return['guests'], ' ', 1
                                                                                                                                                                                                        == $return['guests'] ? $txt['guest'] : $txt['guests'], ', ', $return['num_users'], ' ', 1
                                                                                                                                                                                                                                                                                == $return['num_users'] ? $txt['user'] : $txt['users'];

    // Hidden users, or buddies?

    if ($return['hidden'] > 0 || $show_buddies) {
        echo '
			(' . ($show_buddies ? ($return['buddies'] . ' ' . (1 == $return['buddies'] ? $txt['buddy'] : $txt['buddies'])) : '') . ($show_buddies && $return['hidden'] ? ', ' : '') . (!$return['hidden'] ? '' : $return['hidden'] . ' ' . $txt['hidden']) . ')';
    }
...
function ssi_login($redirect_to = '', $output_method = 'echo')
{
    ...
    if ('echo' != $output_method || !$user_info['is_guest']) {
...
function ssi_recentPoll($output_method = 'echo', $topPollInstead = false)
{
    ...
    $request = db_query(
		...
		WHERE p.votingLocked = 0" . ($topPollInstead ? '
			...
			AND $user_info[query_see_board]" . (!in_array(0, $boardsAllowed, true) ? '
    ...
    [$total] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    while (false !== ($rowChoice = $GLOBALS['xoopsDB']->fetchArray($request))) {
    ...
    $divisor = 0 == $return['total_votes'] ? 1 : $return['total_votes'];

    foreach ($options as $i => $option) {
        $bar = floor(($option[1] * 100) / $divisor);

        $barWide = 0 == $bar ? 1 : floor(($bar * 5) / 3);

        $return['options'][$i] = [
            'id' => 'options-' . $i,
            'percent' => $bar,
            'votes' => $option[1],
            'bar' => '<span style="white-space: nowrap;"><img src="'
                             . $settings['images_url']
                             . '/poll_left.gif" alt=""><img src="'
                             . $settings['images_url']
                             . '/poll_middle.gif" width="'
                             . $barWide
                             . '" height="12" alt="-"><img src="'
                             . $settings['images_url']
                             . '/poll_right.gif" alt=""></span>',
...
function ssi_showPoll($topic = null, $output_method = 'echo')
{
    ...
    if (null === $topic && isset($_REQUEST['ssi_topic'])) {
    ...
    if (!empty($row['expireTime']) && $row['expireTime'] < time()) {
        ...
        $allow_vote = 0 == $GLOBALS['xoopsDB']->getRowsNum($request);
    ...
    $divisor = 0 == $total_votes ? 1 : $total_votes;
...
<body>&laquo;</body>
    ...
    if (!empty($row['votingLocked']) || -1 != $row['selected'] || (!empty($row['expireTime']) && time() > $row['expireTime'])) {
    ...
    $setString = mb_substr($setString, 0, -1);
...
function ssi_boardNews($board = null, $limit = null, $start = null, $length = null, $output_method = 'echo')
{
    ...
    $request = db_query(
		...
		SELECT ID_BOARD
		FROM {$db_prefix}boards
		WHERE " . (null === $board ? '' : "ID_BOARD = $board
    ...
    [$board] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        if (!empty($length) && $length < $func['strlen']($row['body'])) {
            $row['body'] = $func['substr']($row['body'], 0, $length);

            // The first space or line break. (<br>, etc.)

            $cutoff = max(mb_strrpos($row['body'], ' '), mb_strrpos($row['body'], '<'));
        ...
        $return[] = [
            ...
            'body' => $row['body'],
            'href' => $scripturl . '?topic=' . $row['ID_TOPIC'] . '.0',
            'link' => '<a href="' . $scripturl . '?topic=' . $row['ID_TOPIC'] . '.0">' . $row['numReplies'] . ' ' . (1 == $row['numReplies'] ? $txt['smf_news_1'] : $txt['smf_news_2']) . '</a>',
...
function ssi_recentEvents($max_events = 7, $output_method = 'echo')
{
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        $return[$date][] = [
            ...
            'modify_href' => $scripturl . '?action=' . (0 == $row['ID_BOARD'] ? 'calendar;sa=post;' : 'post;msg=' . $row['ID_FIRST_MSG'] . ';topic=' . $row['ID_TOPIC'] . '.0;calendar;') . 'eventid=' . $row['ID_EVENT'] . ';sesc=' . $sc,
            'href' => 0 == $row['ID_BOARD'] ? '' : $scripturl . '?topic=' . $row['ID_TOPIC'] . '.0',
            'link' => 0 == $row['ID_BOARD'] ? $row['title'] : '<a href="' . $scripturl . '?topic=' . $row['ID_TOPIC'] . '.0">' . $row['title'] . '</a>',
    ...
    if ('echo' != $output_method || empty($return)) {
        return $return;
    }
...
function smf_loadCalendarInfo()
{
    ...
    $context['calendar_only_today'] = 1 == $modSettings['cal_days_for_index'];
    ...
    for ($i = $now; $i < $now + $days_for_index; $i += 86400) {
        if (isset($events[strftime('%Y-%m-%d', $i)])) {
            foreach ($events[strftime('%Y-%m-%d', $i)] as $ev => $event) {
                if (empty($event['topic']) || (0 != count(array_intersect($user_info['groups'], $event['allowed_groups'])) || allowedTo('admin_forum'))) {
                    ...
                    $this_event['href'] = 0 == $this_event['topic'] ? '' : $scripturl . '?topic=' . $this_event['topic'] . '.0';

                    $this_event['link'] = 0 == $this_event['topic'] ? $this_event['title'] : '<a href="' . $this_event['href'] . '">' . $this_event['title'] . '</a>';

                    $this_event['modify_href'] = $scripturl . '?action=' . (0 == $this_event['topic'] ? 'calendar;sa=post;' : 'post;msg=' . $this_event['msg'] . ';topic=' . $this_event['topic'] . '.0;calendar;') . 'eventid=' . $this_event['id'] . ';sesc=' . $sc;
...
function ssi_checkPassword($id = null, $password = null, $is_username = false)
{
    ...
    [$pass, $user, $active] = $GLOBALS['xoopsDB']->fetchRow($request);

    $GLOBALS['xoopsDB']->freeRecordSet($request);

    return sha1(mb_strtolower($user) . $password) == $pass && 1 == $active;
}
