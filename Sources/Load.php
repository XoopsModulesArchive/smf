...
function reloadSettings()
{
    ...
    $ent_check = empty($modSettings['disableEntityCheck']) ? ['preg_replace(\'~(&#(\d{1,7}|x[0-9a-fA-F]{1,6});)~e\', \'$func[\\\'entity_fix\\\'](\\\'\\2\\\')\', ', ')'] : ['', ''];

    // Preg_replace can handle complex characters only for higher PHP versions.
    $space_chars = $utf8 ? (@version_compare(PHP_VERSION, '4.3.3') != -1 ? '\x{A0}\x{2000}-\x{200F}\x{201F}\x{202F}\x{3000}\x{FEFF}' : pack('C*', 0xC2, 0xA0, 0xE2, 0x80, 0x80) . '-' . pack(
            'C*',
            0xE2,
            0x80,
            0x8F,
    ...
    $func = [
        ...
        'substr'           => create_function(
            ...
            '
			global $func;
			$ent_arr = preg_preg_split(\'~(&#' . (empty($modSettings['disableEntityCheck']) ? '\d{1,7}' : '021') . ';|&quot;|&amp;|&lt;|&gt;|&nbsp;|.)~' . ($utf8 ? 'u' : '') . '\', ' . implode('$string', $ent_check) . ', -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
			return $length === null ? implode(\'\', array_slice($ent_arr, $start)) : implode(\'\', array_slice($ent_arr, $start, $length));'
        ),
        'strlen'           => create_function(
            '$string',
            '
			global $func;
			return strlen(preg_replace(\'~' . $ent_list . ($utf8 ? '|.~u' : '~') . '\', \'_\', ' . implode('$string', $ent_check) . '));'
        ),
        'strpos'           => create_function(
            '$haystack, $needle, $offset = 0',
            '
			global $func;
			$haystack_arr = preg_preg_split(\'~(&#' . (empty($modSettings['disableEntityCheck']) ? '\d{1,7}' : '021') . ';|&quot;|&amp;|&lt;|&gt;|&nbsp;|.)~' . ($utf8 ? 'u' : '') . '\', ' . implode('$haystack', $ent_check) . ', -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
			...
			{
				$needle_arr = preg_preg_split(\'~(&#' . (empty($modSettings['disableEntityCheck']) ? '\d{1,7}' : '021') . ';|&quot;|&amp;|&lt;|&gt;|&nbsp;|.)~' . ($utf8 ? 'u' : '') . '\',  ' . implode('$needle', $ent_check) . ', -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
        ...
        'htmlspecialchars' => create_function(
            '$string, $quote_style = ENT_COMPAT, $charset = \'ISO-8859-1\'',
            '
			global $func;
			return ' . strtr($ent_check[0], ['&' => '&amp;']) . 'htmlspecialchars($string, $quote_style, ' . ($utf8 ? '\'UTF-8\'' : '$charset') . ')' . $ent_check[1] . ';'
        ),
        'htmltrim'         => create_function(
            '$string',
            '
			global $func;
			return preg_replace(\'~^([ \t\n\r\x0B\x00' . $space_chars . ']|&nbsp;)+|([ \t\n\r\x0B\x00' . $space_chars . ']|&nbsp;)+$~' . ($utf8 ? 'u' : '') . '\', \'\', ' . implode('$string', $ent_check) . ');'
        ),
        'truncate'         => create_function(
            '$string, $length',
            (empty($modSettings['disableEntityCheck']) ? '
			global $func;
        ...
        'strtolower'       => $utf8 ? (function_exists('mb_strtolower') ? create_function(
            '$string',
            '
        ...
        'strtoupper'       => $utf8 ? (function_exists('mb_strtoupper') ? create_function(
            '$string',
            '
        ...
        'ucfirst'          => $utf8 ? create_function(
            '$string',
            '
			global $func;
			return $func[\'strtoupper\']($func[\'substr\']($string, 0, 1)) . $func[\'substr\']($string, 1);'
        ) : 'ucfirst',
        'ucwords'          => $utf8 ? (function_exists('mb_convert_case') ? create_function(
            '$string',
            '
    ...
    if (isset($modSettings['integrate_pre_include']) && file_exists(strtr($modSettings['integrate_pre_include'], ['$boarddir' => $boarddir]))) {
        require_once strtr($modSettings['integrate_pre_include'], ['$boarddir' => $boarddir]);
    }
    ...
    if (!empty($modSettings['autoOptMaxOnline'])) {
        ...
        [$dont_do_it] = $GLOBALS['xoopsDB']->fetchRow($request);
        $GLOBALS['xoopsDB']->freeRecordSet($request);
    ...
    updateSettings(['autoOptLastOpt' => time()]);
...
function loadUserSettings()
{
    ...
    if (empty($ID_MEMBER) && isset($_COOKIE[$cookiename])) {
        ...
        if (preg_match('~^a:[34]:\{i:0;(i:\d{1,6}|s:[1-8]:"\d{1,8}");i:1;s:(0|40):"([a-fA-F0-9]{40})?";i:2;[id]:\d{1,14};(i:3;i:\d;)?\}$~', $_COOKIE[$cookiename]) == 1) {
            [$ID_MEMBER, $password] = @unserialize($_COOKIE[$cookiename]);
        ...
        [$ID_MEMBER, $password, $login_span] = @unserialize(stripslashes($_SESSION['login_' . $cookiename]));
    ...
    if ($ID_MEMBER != 0) {
        ...
        if (SMF != 'SSI' && !isset($_REQUEST['xml']) && (!isset($_REQUEST['action']) || $_REQUEST['action'] != '.xml') && empty($_SESSION['ID_MSG_LAST_VISIT'])
            && (empty($modSettings['cache_enable'])
                || ($_SESSION['ID_MSG_LAST_VISIT'] = cache_get_data('user_last_visit-' . $ID_MEMBER, 5 * 3600)) === null)) {
            ...
            [$visitTime] = $GLOBALS['xoopsDB']->fetchRow($result);
            ...
            if ($visitTime < time() - 5 * 3600) {
                updateMemberData($ID_MEMBER, ['ID_MSG_LAST_VISIT' => (int)$modSettings['maxMsgID'], 'lastLogin' => time(), 'memberIP' => '\'' . $_SERVER['REMOTE_ADDR'] . '\'', 'memberIP2' => '\'' . $_SERVER['BAN_CHECK_IP'] . '\'']);
    ...
    $user_info           += [
        ...
        'avatar'               => [
            'url'        => $user_settings['avatar'] ?? '',
            'filename'   => empty($user_settings['filename']) ? '' : $user_settings['filename'],
            'custom_dir' => !empty($user_settings['attachmentType']) && $user_settings['attachmentType'] == 1,
            'ID_ATTACH'  => $user_settings['ID_ATTACH'] ?? 0,
        ],
        'smiley_set'           => $user_settings['smileySet'] ?? '',
    ...
    // SIMPLE-XOOPS START
    if (isset($modSettings['integrate_login']) && function_exists($modSettings['integrate_login']) && !empty($user_info['username'])) {
        global $xoopsUser;
        if (!$xoopsUser) {
            $modSettings['integrate_login']($user_info['username'], isset($_REQUEST['hash_passwrd']) && strlen($_REQUEST['hash_passwrd']) == 40 ? $_REQUEST['hash_passwrd'] : null, $modSettings['cookieTime']);
        }
    }
    //SIMPLE-XOOPS END
...
function loadBoard()
{
    ...
    if (empty($temp)) {
        ...
        if ($GLOBALS['xoopsDB']->getRowsNum($request) > 0) {
            ...
            $board_info['groups'] = $row['memberGroups'] == '' ? [] : explode(',', $row['memberGroups']);
...
function loadPermissions()
{
    ...
    if (!empty($modSettings['cache_enable'])) {
        ...
        if ($modSettings['cache_enable'] >= 2 && !empty($board) && ($temp = cache_get_data('permissions:' . $cache_groups . ':' . $board, 240)) != null) {
            [$user_info['permissions']] = $temp;
            banPermissions();

            return;
        } elseif (($temp = cache_get_data('permissions:' . $cache_groups, 240)) != null) {
            [$user_info['permissions'], $removals] = $temp;
        }
    }
    ...
    if (!empty($board)) {
        ...
        $request = db_query(
			...
			WHERE ID_GROUP IN (" . implode(', ', $user_info['groups']) . ')
				AND ID_BOARD = ' . ($board_info['use_local_permissions'] ? $board : '0'),
            __FILE__,
            __LINE__
        );
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
...
function loadMemberData($users, $is_name = false, $set = 'normal')
{
    ...
    if (!empty($users)) {
        ...
        $request        = db_query(
			...
			WHERE mem." . ($is_name ? 'memberName' : 'ID_MEMBER') . (count($users) == 1 ? " = '" . current($users) . "'" : " IN ('" . implode("', '", $users) . "')"),
            __FILE__,
            __LINE__
        );
        $new_loaded_ids = [];
...
function loadMemberContext($user)
{
    ...
    $profile['stars']     = empty($profile['stars']) ? ['', ''] : explode('#', $profile['stars']);
    ...
    $memberContext[$user] = [
        ...
        'icq'                  => $profile['ICQ'] != '' && (empty($modSettings['guest_hideContacts']) || !$user_info['is_guest']) ? [
            'name'      => &$profile['ICQ'],
        ...
        ] : ['name' => '', 'add' => '', 'href' => '', 'link' => '', 'link_text' => ''],
        'aim'                  => $profile['AIM'] != '' && (empty($modSettings['guest_hideContacts']) || !$user_info['is_guest']) ? [
            'name'      => &$profile['AIM'],
            'href'      => 'aim:goim?screenname=' . urlencode(strtr($profile['AIM'], [' ' => '%20'])) . '&amp;message=' . $txt['aim_default_message'],
            'link'      => '<a href="aim:goim?screenname=' . urlencode(strtr($profile['AIM'], [' ' => '%20'])) . '&amp;message=' . $txt['aim_default_message'] . '"><img src="' . $settings['images_url'] . '/aim.gif" alt="' . $profile['AIM'] . '" border="0"></a>',
            'link_text' => '<a href="aim:goim?screenname=' . urlencode(strtr($profile['AIM'], [' ' => '%20'])) . '&amp;message=' . $txt['aim_default_message'] . '">' . $profile['AIM'] . '</a>',
        ] : ['name' => '', 'href' => '', 'link' => '', 'link_text' => ''],
        'yim'                  => $profile['YIM'] != '' && (empty($modSettings['guest_hideContacts']) || !$user_info['is_guest']) ? [
            'name'      => &$profile['YIM'],
        ...
        'msn'                  => $profile['MSN'] != '' && (empty($modSettings['guest_hideContacts']) || !$user_info['is_guest']) ? [
            'name'      => &$profile['MSN'],
        ...
        'avatar'               => [
            'name'  => &$profile['avatar'],
            'image' => $profile['avatar'] == '' ? ($profile['ID_ATTACH'] > 0 ? '<img src="'
                                                                               . (empty($profile['attachmentType']) ? $scripturl . '?action=dlattach;attach=' . $profile['ID_ATTACH'] . ';type=avatar' : $modSettings['custom_avatar_url'] . '/' . $profile['filename'])
                                                                               . '" alt="" class="avatar" border="0">' : '') : (false !== stripos($profile['avatar'], 'http://') ? '<img src="' . $profile['avatar'] . '"' . $avatar_width . $avatar_height . ' alt="" class="avatar" border="0">' : '<img src="'
            ...
            'href'  => $profile['avatar'] == '' ? ($profile['ID_ATTACH'] > 0 ? (empty($profile['attachmentType']) ? $scripturl . '?action=dlattach;attach=' . $profile['ID_ATTACH'] . ';type=avatar' : $modSettings['custom_avatar_url'] . '/' . $profile['filename']) : '') : (false !== stripos(
                $profile['avatar'],
                'http://'
            ) ? $profile['avatar'] : $modSettings['avatar_url'] . '/' . $profile['avatar']),
            'url'   => $profile['avatar'] == '' ? '' : (false !== stripos($profile['avatar'], 'http://') ? $profile['avatar'] : $modSettings['avatar_url'] . '/' . $profile['avatar']),
        ],
        ...
        'ip'                   => htmlspecialchars($profile['memberIP'], ENT_QUOTES | ENT_HTML5),
        'ip2'                  => htmlspecialchars($profile['memberIP2'], ENT_QUOTES | ENT_HTML5),
        'online'               => [
        ...
        'language'             => $func['ucwords'](strtr($profile['lngfile'], ['_' => ' ', '-utf8' => ''])),
        'is_activated'         => $profile['is_activated'] ?? 1,
        ...
        'group_stars'          => str_repeat(
            '<img src="' . str_replace('$language', $context['user']['language'], isset($profile['stars'][1]) ? $settings['images_url'] . '/' . $profile['stars'][1] : '') . '" alt="*" border="0">',
...
function loadTheme($ID_THEME = 0, $initialize = true)
{
    ...
    $user_info['smiley_set']       = (!in_array($user_info['smiley_set'], explode(',', $modSettings['smiley_sets_known'])) && $user_info['smiley_set'] != 'none')
                                     || empty($modSettings['smiley_sets_enable']) ? (!empty($settings['smiley_sets_default']) ? $settings['smiley_sets_default'] : $modSettings['smiley_sets_default']) : $user_info['smiley_set'];
    ...
    $context['current_action']    = $_REQUEST['action'] ?? null;
    $context['current_subaction'] = $_REQUEST['sa'] ?? null;
    ...
    $context['server'] = [
        ...
        'is_windows'         => false !== stripos(PHP_OS, 'WIN'),
    ...
    if ((isset($_REQUEST['action']) && in_array($_REQUEST['action'], ['login', 'login2', 'register'])) || !$context['user']['is_guest']) {
    ...
    if (WIRELESS) {
        $context['template_layers'] = [WIRELESS_PROTOCOL];
...
<div style="color: red; padding: 2ex; background-color: white; border: 2px dashed;">
    ...
    elseif ($template_name != 'Errors' && $template_name != 'index' && $fatal) {
        fatal_lang_error('theme_template_error', true, [(string)$template_name]);
    } elseif ($fatal) {
        die(log_error(sprintf($txt['theme_template_error'] ?? 'Unable to load Themes/default/%s.template.php!', (string)$template_name)));
    } else {
        return false;
    }
...
function loadSubTemplate($sub_template_name, $fatal = false)
{
    ...
    if (function_exists($theme_function)) {
        ...
        fatal_lang_error('theme_template_error', true, [(string)$sub_template_name]);
    } elseif ($fatal !== 'ignore') {
        die(log_error(sprintf($txt['theme_template_error'] ?? 'Unable to load the %s sub template!', (string)$sub_template_name)));
    }

    // Are we showing debugging for templates?  Just make sure not to do it before the doctype...
    if (allowedTo('admin_forum') && isset($_REQUEST['debug']) && !in_array($sub_template_name, ['init', 'main_below']) && ob_get_length() > 0 && !isset($_REQUEST['xml'])) {
        echo '
...
function loadLanguage($template_name, $lang = '', $fatal = true)
{
    ...
    $attempts = [
        [$settings['theme_dir'], $template_name, $lang, $settings['theme_url']],
        [$settings['theme_dir'], $template_name, $language, $settings['theme_url']],
    ];

    // Do we have a base theme to worry about?
    if (isset($settings['base_theme_dir'])) {
        $attempts[] = [$settings['base_theme_dir'], $template_name, $lang, $settings['base_theme_url']];
        $attempts[] = [$settings['base_theme_dir'], $template_name, $language, $settings['base_theme_url']];
    }

    // Fallback on the default theme if necessary.
    $attempts[] = [$settings['default_theme_dir'], $template_name, $lang, $settings['default_theme_url']];
    $attempts[] = [$settings['default_theme_dir'], $template_name, $language, $settings['default_theme_url']];
...
function &censorText(&$text)
{
    ...
    if ($censor_vulgar === null) {
        ...
        for ($i = 0, $n = count($censor_vulgar); $i < $n; $i++) {
            $censor_vulgar[$i] = strtr(preg_quote($censor_vulgar[$i], '/'), ['\\\\\\*' => '[*]', '\\*' => '[^\s]*?', '&' => '&amp;']);
            $censor_vulgar[$i] = (empty($modSettings['censorWholeWord']) ? '/' . $censor_vulgar[$i] . '/' : '/(?<=^|\W)' . $censor_vulgar[$i] . '(?=$|\W)/')
                                 . (empty($modSettings['censorIgnoreCase']) ? '' : 'i')
                                 . ((empty($modSettings['global_character_set']) ? $txt['lang_character_set'] : $modSettings['global_character_set']) === 'UTF-8' ? 'u' : '');

            if (strpos($censor_vulgar[$i], '\'') !== false) {
                $censor_proper[count($censor_vulgar)] = $censor_proper[$i];
                $censor_vulgar[count($censor_vulgar)] = strtr($censor_vulgar[$i], ['\'' => '&#039;']);
            }
        }
    }
...
function template_include($filename, $once = false)
{
    ...
    if ($file_found !== true) {
        ...
        if (!isset($txt['template_parse_error'])) {
            ...
            $txt['template_parse_error_details'] = 'There was a problem loading the <tt><b>%1$s</b></tt> template or language file.  Please check the syntax and try again - remember, single quotes (<tt>\'</tt>) often have to be escaped with a slash (<tt>\\</tt>).  To see more specific error information from PHP, try <a href="'
                                                   . $boardurl
                                                   . '%1$s" target="_blank">accessing the file directly</a>.<br><br>You may want to try to <a href="javascript:location.reload();">refresh this page</a> or <a href="'
                                                   . $scripturl
...
<html xmlns="http://www.w3.org/1999/xhtml"', !empty($context['right_to_left']) ? ' dir="rtl"' : '', '>
	...
	<body>
		...
		', $txt['template_parse_error_message'], '
            ...
            $error = fetch_web_data($boardurl . strtr($filename, [$boarddir => '', strtr($boarddir, '\\', '/') => '']));
            if (empty($error)) {
                $error = $php_errormsg;
            }
	...
	<body>
		<h3>', $txt['template_parse_error'], '</h3>
		', sprintf($txt['template_parse_error_details'], strtr($filename, [$boarddir => '', strtr($boarddir, '\\', '/') => '']));

            if (!empty($error)) {
                echo '
		<hr>

		<div style="margin: 0 20px;"><tt>', strtr(strtr($error, ['<b>' . $boarddir => '<b>...', '<b>' . strtr($boarddir, '\\', '/') => '<b>...']), '\\', '/'), '</tt></div>';
            }
            ...
            if (preg_match('~ <b>(\d+)</b><br( /)?' . '>$~i', $error, $match) != 0) {
                ...
                $data2 = preg_preg_split('~\<br( /)?\>~', $data2);
...
function loadSession()
{
    ...
    if ((@ini_get('session.auto_start') == 1 && !empty($modSettings['databaseSession_enable'])) || session_id() == '') {
        ...
        if (!empty($modSettings['databaseSession_enable']) && @version_compare(PHP_VERSION, '4.2.0') != -1) {
            session_set_saveHandler('sessionOpen', 'sessionClose', 'sessionRead', 'sessionWrite', 'sessionDestroy', 'sessionGC');
        ...
        if (empty($modSettings['databaseSession_enable']) && !empty($modSettings['cache_enable']) && php_sapi_name() != 'cli') {
            if (function_exists('mmcache_set_sessionHandlers')) {
                mmcache_set_sessionHandlers();
            } elseif (function_exists('eaccelerator_set_sessionHandlers')) {
                eaccelerator_set_sessionHandlers();
            }
        }
...
function sessionRead($session_id)
{
    ...
    [$sess_data] = $GLOBALS['xoopsDB']->fetchRow($result);
    $GLOBALS['xoopsDB']->freeRecordSet($result);
...
function sessionWrite($session_id, $data)
{
    ...
    if (db_affected_rows() == 0) {
        $result = db_query(
			...
			VALUES ('" . addslashes($session_id) . "', '" . addslashes($data) . "', " . time() . ')',
            __FILE__,
            __LINE__
        );
    }
...
function sessionGC($max_lifetime)
{
    ...
    return db_query(
        "
		DELETE FROM {$db_prefix}sessions
		WHERE last_update < " . (time() - $max_lifetime),
        __FILE__,
        __LINE__
    );
}
...
function cache_put_data($key, $value, $ttl = 120)
{
    ...
    if (isset($db_show_debug) && $db_show_debug === true) {
        $cache_hits[$cache_count] = ['k' => $key, 'd' => 'put', 's' => $value === null ? 0 : strlen(serialize($value))];
        $st                       = microtime();
    }
...
function get_memcached_server($level = 3)
{
    ...
    if (!$memcached && $level > 0) {
        ...
        @stream_set_timeout($memcached, 1);
        @stream_set_write_buffer($memcached, 0);
    }
}
