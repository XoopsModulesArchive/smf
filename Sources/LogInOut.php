...
function Login()
{
    ...
    if (isset($_SESSION['old_url']) && 0 != preg_match('~(board|topic)[=,]~', $_SESSION['old_url'])) {
...
function Login2()
{
    ...
    if (isset($_GET['sa']) && 'salt' == $_GET['sa'] && !$user_info['is_guest']) {
        if (isset($_COOKIE[$cookiename])) {
            [, , $timeout] = @unserialize($_COOKIE[$cookiename]);
        } elseif (isset($_SESSION['login_' . $cookiename])) {
            [, , $timeout] = @unserialize(stripslashes($_SESSION['login_' . $cookiename]));
        } else {
        ...
        updateMemberData($ID_MEMBER, ['passwordSalt' => '\'' . $user_settings['passwordSalt'] . '\'']);
    ...
    elseif (isset($_GET['sa']) && 'check' == $_GET['sa']) {
    ...
    if (empty($_SESSION['login_url']) && isset($_SESSION['old_url']) && 0 != preg_match('~(board|topic)[=,]~', $_SESSION['old_url'])) {
        $_SESSION['login_url'] = $_SESSION['old_url'];
    }
    ...
    if (isset($_POST['cookieneverexp']) || (!empty($_POST['cookielength']) && -1 == $_POST['cookielength'])) {
    ...
    $context['default_username'] = isset($_REQUEST['user']) ? htmlspecialchars(stripslashes($_REQUEST['user']), ENT_QUOTES | ENT_HTML5) : '';

    $context['default_password'] = '';

    $context['never_expire'] = 525600 == $modSettings['cookieTime'] || 3153600 == $modSettings['cookieTime'];
    ...
    if ((!isset($_REQUEST['passwrd']) || '' == $_REQUEST['passwrd']) && (!isset($_REQUEST['hash_passwrd']) || 40 != mb_strlen($_REQUEST['hash_passwrd']))) {
    ...
    if (isset($modSettings['integrate_validate_login']) && function_exists($modSettings['integrate_validate_login'])) {
        if ('retry' == call_user_func($modSettings['integrate_validate_login'], $_REQUEST['user'], isset($_REQUEST['hash_passwrd']) && 40 == mb_strlen($_REQUEST['hash_passwrd']) ? $_REQUEST['hash_passwrd'] : null, $modSettings['cookieTime'])) {
    ...
    elseif (3 == $activation_status) {
        fatal_lang_error('still_awaiting_approval');
    } // Awaiting deletion, changed their mind?

    elseif (4 == $activation_status) {
    ...
    if (isset($_REQUEST['hash_passwrd']) && 40 == mb_strlen($_REQUEST['hash_passwrd'])) {
        // Needs upgrading?

        if (40 != mb_strlen($user_settings['passwd'])) {
            $context['login_error'] = $txt['login_hash_error'];
        ...
        $sha_passwd = sha1(mb_strtolower($user_settings['memberName']) . un_htmlspecialchars(stripslashes($_REQUEST['passwrd'])));
    }
    ...
    if ($user_settings['passwd'] != $sha_passwd) {
        ...
        if ('' == $user_settings['passwordSalt']) {
            ...
            $other_passwords[] = crypt($_REQUEST['passwrd'], mb_substr($_REQUEST['passwrd'], 0, 2));

            $other_passwords[] = crypt($_REQUEST['passwrd'], mb_substr($user_settings['passwd'], 0, 2));

            $other_passwords[] = md5($_REQUEST['passwrd']);

            $other_passwords[] = sha1($_REQUEST['passwrd']);

            $other_passwords[] = md5_hmac($_REQUEST['passwrd'], mb_strtolower($user_settings['memberName']));

            $other_passwords[] = md5($_REQUEST['passwrd'] . mb_strtolower($user_settings['memberName']));
            ...
            if (64 == mb_strlen($user_settings['passwd']) && function_exists('mhash') && defined('MHASH_SHA256')) {
                $other_passwords[] = bin2hex(mhash(MHASH_SHA256, $_REQUEST['passwrd']));
            }
        } // The hash should be 40 if it's SHA-1, so we're safe with more here too.

        elseif (32 == mb_strlen($user_settings['passwd'])) {
        ...
        $other_passwords[] = sha1(mb_strtolower($user_settings['memberName']) . addslashes(un_htmlspecialchars(stripslashes($_REQUEST['passwrd']))));
        ...
        $other_passwords[] = sha1_smf(mb_strtolower($user_settings['memberName']) . un_htmlspecialchars(stripslashes($_REQUEST['passwrd'])));

        // Whichever encryption it was using, let's make it use SMF's now ;).

        if (in_array($user_settings['passwd'], $other_passwords, true)) {
            ...
            updateMemberData($user_settings['ID_MEMBER'], ['passwd' => '\'' . $user_settings['passwd'] . '\'', 'passwordSalt' => '\'' . $user_settings['passwordSalt'] . '\'']);
    ...
    elseif ('' == $user_settings['passwordSalt']) {
        ...
        updateMemberData($user_settings['ID_MEMBER'], ['passwordSalt' => '\'' . $user_settings['passwordSalt'] . '\'']);
    }

    if (isset($modSettings['integrate_login']) && function_exists($modSettings['integrate_login'])) {
        $modSettings['integrate_login']($user_settings['memberName'], isset($_REQUEST['hash_passwrd']) && 40 == mb_strlen($_REQUEST['hash_passwrd']) ? $_REQUEST['hash_passwrd'] : null, $modSettings['cookieTime']);
    }
    ...
    $user_info['is_admin'] = 1 == $user_settings['ID_GROUP'] || in_array(1, $user_settings['additionalGroups'], true);
    ...
    updateMemberData($ID_MEMBER, ['lastLogin' => time(), 'memberIP' => '\'' . $user_info['ip'] . '\'', 'memberIP2' => '\'' . $_SERVER['BAN_CHECK_IP'] . '\'']);
...
{
    $key = str_pad(mb_strlen($key) <= 64 ? $key : pack('H*', md5($key)), 64, chr(0x00));
