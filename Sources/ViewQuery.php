...
function ViewQuery()
{
    ...
    if (isset($_REQUEST['sa']) && 'hide' == $_REQUEST['sa']) {
        $_SESSION['view_queries'] = 1 == $_SESSION['view_queries'] ? 0 : 1;

        if (false !== mb_strpos($_SESSION['old_url'], 'action=viewquery')) {
            redirectexit();
        } else {
...
<html>
    ...
    foreach ($_SESSION['debug'] as $q => $query_data) {
        ...
        foreach ($query as $line) {
            ...
            if (mb_strlen($temp[0]) < $min_indent || 0 == $min_indent) {
                $min_indent = mb_strlen($temp[0]);
            }
        }

        foreach ($query as $l => $dummy) {
            $query[$l] = mb_substr($dummy, $min_indent);
        }

        $query_data['q'] = implode("\n", $query);

        $is_select_query = 'SELECT' == mb_substr(trim($query_data['q']), 0, 6);

        if ($is_select_query) {
            $select = $query_data['q'];
        } elseif (0 != preg_match('~^INSERT(?: IGNORE)? INTO \w+(?:\s+\([^)]+\))?\s+(SELECT .+)$~s', trim($query_data['q']), $matches)) {
            $is_select_query = true;

            $select = $matches[1];
        } elseif (0 != preg_match('~^CREATE TEMPORARY TABLE .+?(SELECT .+)$~s', trim($query_data['q']), $matches)) {
            $is_select_query = true;
		...
		<div id="qq', $q, '" style="margin-bottom: 2ex;">
			<a', $is_select_query ? ' href="' . $scripturl . '?action=viewquery;qq=' . ($q + 1) . '#qq' . $q . '"' : '', ' style="font-weight: bold; color: black; text-decoration: none;">
				', nl2br(str_replace("\t", '&nbsp;&nbsp;&nbsp;', htmlspecialchars($query_data['q'], ENT_QUOTES | ENT_HTML5))), '
			</a><br>';
		...
		<table border="1" cellpadding="4" cellspacing="0" style="empty-cells: show; font-family: serif; margin-bottom: 2ex;">
			<tr><td>', $GLOBALS['xoopsDB']->error($db_connection), '</td></tr>
		</table>';
