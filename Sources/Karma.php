...
function ModifyKarma()
{
    ...
    if (!empty($modSettings['karmaTimeRestrictAdmins']) || !allowedTo('moderate_forum')) {
        ...
        if ($GLOBALS['xoopsDB']->getRowsNum($request) > 0) {
            [$action] = $GLOBALS['xoopsDB']->fetchRow($request);
        }
    ...
    if (empty($action) || empty($modSettings['karmaWaitTime'])) {
        ...
        db_query(
			...
			VALUES ($dir, $_REQUEST[uid], $ID_MEMBER, " . time() . ')',
            __FILE__,
            __LINE__
        );
        ...
        updateMemberData($_REQUEST['uid'], [1 == $dir ? 'karmaGood' : 'karmaBad' => '+']);
    } else {
        ...
        if (1 == $dir) {
            updateMemberData($_REQUEST['uid'], ['karmaGood' => '+', 'karmaBad' => '-']);
        } else {
            updateMemberData($_REQUEST['uid'], ['karmaBad' => '+', 'karmaGood' => '-']);
        }
    }
...
function BookOfUnknown()
{
    ...
    if (false !== mb_strpos($_GET['action'], 'mozilla') && !$context['browser']['is_gecko']) {
        redirectexit('http://www.getfirefox.com/');
    } elseif (false !== mb_strpos($_GET['action'], 'mozilla')) {
...
<html>
	<head>
		<title>The Book of Unknown, ', '2:18' == @$_GET['verse'] ? '2:18' : '4:16', '</title>
		<style type="text/css">
			em
			{
				font-size: 1.3em;
