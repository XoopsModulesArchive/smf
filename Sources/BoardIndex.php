...
function BoardIndex()
{
    ...
    while (false !== ($row_board = $GLOBALS['xoopsDB']->fetchArray($result_boards))) {
        // Haven't set this category yet.

        if (empty($context['categories'][$row_board['ID_CAT']])) {
            $context['categories'][$row_board['ID_CAT']] = [
                'id' => $row_board['ID_CAT'],
                'name' => $row_board['catName'],
                'is_collapsed' => isset($row_board['canCollapse']) && 1 == $row_board['canCollapse'] && $row_board['isCollapsed'] > 0,
                'can_collapse' => isset($row_board['canCollapse']) && 1 == $row_board['canCollapse'],
            ...
            $context['categories'][$row_board['ID_CAT']]['link'] = '<a name="'
                                                                   . $row_board['ID_CAT']
                                                                   . '" href="'
                                                                   . (isset($row_board['canCollapse']) ? $context['categories'][$row_board['ID_CAT']]['collapse_href'] : $context['categories'][$row_board['ID_CAT']]['href'])
                                                                   . '">'
                                                                   . $row_board['catName']
                                                                   . '</a>';
        }
        ...
        if (empty($row_board['ID_PARENT'])) {
            ...
            if (!empty($row_board['ID_MODERATOR'])) {
                $this_category[$row_board['ID_BOARD']]['moderators'][$row_board['ID_MODERATOR']] = [
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
        ...
        $context['users_online'][$row['logTime'] . $row['memberName']] = [
            'id' => $row['ID_MEMBER'],
    ...
    if (!empty($modSettings['trackStats'])) {
        ...
        if (!isset($modSettings['mostOnline']) || $total_users >= $modSettings['mostOnline']) {
            updateSettings(['mostOnline' => $total_users, 'mostDate' => time()]);
        }
        ...
        if (!isset($modSettings['mostOnlineUpdated']) || $modSettings['mostOnlineUpdated'] != $date) {
            ...
            if (0 == $GLOBALS['xoopsDB']->getRowsNum($request)) {
                ...
                [$modSettings['mostOnlineToday']] = $GLOBALS['xoopsDB']->fetchRow($request);

                if ($total_users > $modSettings['mostOnlineToday']) {
                    trackStats(['mostOn' => $total_users]);
                }
            ...
            updateSettings(['mostOnlineUpdated' => $date, 'mostOnlineToday' => $total_users]);
    ...
    if ((!empty($settings['number_recent_posts']) && 1 == $settings['number_recent_posts']) || $settings['show_sp1_info']) {
