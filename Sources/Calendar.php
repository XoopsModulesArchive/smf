...
function CalendarMain()
{
    ...
    $nLastDay = (int)strftime('%d', mktime(0, 0, 0, 12 == $curPage['month'] ? 1 : $curPage['month'] + 1, 0, 12 == $curPage['month'] ? $curPage['year'] + 1 : $curPage['year']));
    ...
    $bday = !empty($modSettings['cal_showbdaysoncalendar']) ? calendarBirthdayArray($low, $high) : [];

    $events = !empty($modSettings['cal_showeventsoncalendar']) ? calendarEventArray($low, $high) : [];

    $holidays = !empty($modSettings['cal_showholidaysoncalendar']) ? calendarHolidayArray($low, $high) : [];
    ...
    if (!empty($modSettings['cal_showweeknum'])) {
        ...
        // Note that we offset by two, as some of this will get undone quite quickly by the statement below.
            ...
            $nWeekAdjust = $nStartDay > $foy && 0 != $foy ? 2 : 1;
        }
    ...
    for ($nRow = 0; $nRow < $nRows; $nRow++) {
        ...
        if (53 == $context['weeks'][$nRow]['number'] && 4 != $nShift) {
    ...
    if ($curPage['month'] > 1 || (1 == $curPage['month'] && $curPage['year'] > $modSettings['cal_minyear'])) {
        // Need to roll the year back one?

        $context['previous_calendar'] = [
            'year' => 1 == $curPage['month'] ? $curPage['year'] - 1 : $curPage['year'],
            'month' => 1 == $curPage['month'] ? 12 : $curPage['month'] - 1,
        ];
    ...
    if ($curPage['month'] < 12 || (12 == $curPage['month'] && $curPage['year'] < $modSettings['cal_maxyear'])) {
        $context['next_calendar'] = [
            'year' => 12 == $curPage['month'] ? $curPage['year'] + 1 : $curPage['year'],
            'month' => 12 == $curPage['month'] ? 1 : $curPage['month'] + 1,
        ];
...
function calendarBirthdayArray($low_date, $high_date)
{
    ...
    if (mb_substr($low_date, 0, 4) != mb_substr($high_date, 0, 4)) {
        $allyear_part = "birthdate BETWEEN '0004" . mb_substr($low_date, 4) . "' AND '0004-12-31'
			OR birthdate BETWEEN '0004-01-01' AND '0004" . mb_substr($high_date, 4) . "'";
    } else {
        $allyear_part = "birthdate BETWEEN '0004" . mb_substr($low_date, 4) . "' AND '0004" . mb_substr($high_date, 4) . "'";
    }

    // We need to search for any birthday in this range, and whatever year that birthday is on.

    $year_low = (int)mb_substr($low_date, 0, 4);

    $year_high = (int)mb_substr($high_date, 0, 4);
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
        if ($year_low != $year_high) {
            $age_year = mb_substr($row['birthdate'], 5) < mb_substr($high_date, 5) ? $year_high : $year_low;
        } else {
            $age_year = $year_low;
        }

        $bday[$age_year . mb_substr($row['birthdate'], 4)][] = [
            'id' => $row['ID_MEMBER'],
            'name' => $row['realName'],
...
function calendarHolidayArray($low_date, $high_date)
{
    ...
    if (mb_substr($low_date, 0, 4) != mb_substr($high_date, 0, 4)) {
        $allyear_part = "eventDate BETWEEN '0004" . mb_substr($low_date, 4) . "' AND '0004-12-31'
			OR eventDate BETWEEN '0004-01-01' AND '0004" . mb_substr($high_date, 4) . "'";
    } else {
        $allyear_part = "eventDate BETWEEN '0004" . mb_substr($low_date, 4) . "' AND '0004" . mb_substr($high_date, 4) . "'";
    }
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
        if (mb_substr($low_date, 0, 4) != mb_substr($high_date, 0, 4)) {
            $event_year = mb_substr($row['eventDate'], 5) < mb_substr($high_date, 5) ? mb_substr($high_date, 0, 4) : mb_substr($low_date, 0, 4);
        } else {
            $event_year = mb_substr($low_date, 0, 4);
        }

        $holidays[$event_year . mb_substr($row['eventDate'], 4)][] = $row['title'];
    }
...
function calendarInsertEvent($id_board, $id_topic, $title, $id_member, $month, $day, $year, $span)
{
    ...
    db_query(
		...
		VALUES ($id_board, $id_topic, SUBSTRING('$title', 1, 48), $id_member, '" . strftime('%Y-%m-%d', mktime(0, 0, 0, $month, $day, $year)) . "', '" . strftime('%Y-%m-%d', mktime(0, 0, 0, $month, $day, $year) + $span * 86400) . "')",
        __FILE__,
        __LINE__
    );
...
function CalendarPost()
{
    ...
    if (isset($_POST['sc'], $_REQUEST['eventid'])) {
        ...
        if ($_REQUEST['eventid'] > 0 && !allowedTo('calendar_edit_any')) {
            ...
            [$poster] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        if (-1 == $_REQUEST['eventid'] && isset($_POST['link_to_board'])) {
            $_REQUEST['calendar'] = 1;
        ...
        elseif (-1 == $_REQUEST['eventid']) {
            calendarInsertEvent(0, 0, $_POST['evtitle'], $ID_MEMBER, $_POST['month'], $_POST['day'], $_POST['year'], $_POST['span'] ?? null);
        } // Deleting...
        ...
        elseif (isset($_REQUEST['deleteevent'])) {
            ...
            $span = empty($modSettings['cal_allowspan']) || empty($_POST['span']) || 1 == $_POST['span'] || empty($modSettings['cal_maxspan']) || $_POST['span'] > $modSettings['cal_maxspan'] ? 0 : min((int)$modSettings['cal_maxspan'], (int)$_POST['span'] - 1);
    ...
    if (!isset($_REQUEST['eventid'])) {
        ...
        $context['event'] = [
            ...
            'new' => 1,
            'eventid' => -1,
            'year' => $_REQUEST['year'] ?? $today['year'],
            'month' => $_REQUEST['month'] ?? $today['mon'],
            'day' => $_REQUEST['day'] ?? $today['mday'],
            'title' => '',
            'span' => 1,
        ];
        ...
        $request = db_query(
			...
			WHERE c.ID_EVENT = $_REQUEST[eventid]",
            __FILE__,
            __LINE__
        );
