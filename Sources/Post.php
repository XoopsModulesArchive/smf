...
function Post()
{
    ...
    if (!empty($topic)) {
        ...
        [$locked, $context['notify'], $sticky, $pollID, $context['num_replies'], $ID_MEMBER_POSTER, $ID_FIRST_MSG, $first_subject, $lastPostTime] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        if ((!$context['make_event'] || !empty($board)) && (!isset($_REQUEST['poll']) || '1' != $modSettings['pollMode'])) {
    ...
    if (isset($_REQUEST['poll']) && '1' == $modSettings['pollMode']) {
        // New topic, new poll.
    ...
    if ($context['make_event']) {
        ...
        $context['event']['title'] = $_REQUEST['evtitle'] ?? '';

        $context['event']['id'] = isset($_REQUEST['eventid']) ? (int)$_REQUEST['eventid'] : -1;

        $context['event']['new'] = -1 == $context['event']['id'];
        ...
        if (!$context['event']['new'] && !isset($_REQUEST['subject'])) {
            ...
            $context['event']['day'] = $_REQUEST['day'] ?? ($_REQUEST['month'] == $today['mon'] ? $today['mday'] : 0);
            ...
            $request = db_query(
				...
				WHERE $user_info[query_see_board]" . (in_array(0, $boards, true) ? '' : '
					AND b.ID_BOARD IN (' . implode(', ', $boards) . ')'),
                __FILE__,
                __LINE__
            );

            $context['event']['boards'] = [];
        ...
        $context['event']['last_day'] = (int)strftime('%d', mktime(0, 0, 0, 12 == $context['event']['month'] ? 1 : $context['event']['month'] + 1, 0, 12 == $context['event']['month'] ? $context['event']['year'] + 1 : $context['event']['year']));
        ...
        if (empty($options['no_new_reply_warning']) && isset($_REQUEST['num_replies'])) {
            ...
            if (!empty($newReplies)) {
                ...
                if (isset($_GET['num_replies'])) {
                    ...
                    $context['post_error'][1 == $newReplies ? 'new_reply' : 'new_replies'] = true;
                }
    ...
    if (isset($_REQUEST['message']) || !empty($context['post_error'])) {
        ...
        if (empty($context['post_error'])) {
            ...
            if (!empty($modSettings['max_messageLength']) && $modSettings['max_messageLength'] < $func['strlen']($_REQUEST['message'])) {
            ...
            if ($user_info['is_guest']) {
                ...
                if (!isset($_REQUEST['guestname']) || '' == trim(strtr($_REQUEST['guestname'], '_', ' '))) {
                    ...
                    if (isReservedName(htmlspecialchars($_REQUEST['guestname'], ENT_QUOTES | ENT_HTML5), 0, true, false)) {
                ...
                if (empty($modSettings['guest_post_no_email'])) {
                    ...
                    } elseif (0 == preg_match('~^[0-9A-Za-z=_+\-/][0-9A-Za-z=_\'+\-/\.]*@[\w\-]+(\.[\w\-]+)*(\.[\w]{2,6})$~', stripslashes($_REQUEST['email']))) {
        ...
        $form_subject = strtr($func['htmlspecialchars'](stripslashes($_REQUEST['subject'])), ["\r" => '', "\n" => '', "\t" => '']);
        ...
        if (!empty($context['post_error'])) {
            ...
            foreach ($context['post_error'] as $post_error => $dummy) {
                ...
                if (!in_array($post_error, ['new_reply', 'new_replies', 'old_topic'], true)) {
        ...
        if (isset($_REQUEST['poll'])) {
            ...
            $_POST['options'] = empty($_POST['options']) ? [] : htmlspecialchars__recursive(stripslashes__recursive($_POST['options']));
        ...
        if ($user_info['is_guest']) {
            ...
            $_REQUEST['guestname'] = htmlspecialchars($_REQUEST['guestname'], ENT_QUOTES | ENT_HTML5);

            $context['name'] = $_REQUEST['guestname'];

            $_REQUEST['email'] = htmlspecialchars($_REQUEST['email'], ENT_QUOTES | ENT_HTML5);
        ...
        if (true === $really_previewing || isset($_REQUEST['xml'])) {
        ...
        if (isset($_REQUEST['msg'])) {
            ...
            if (allowedTo('moderate_forum')) {
                ...
                if (empty($row['ID_MEMBER'])) {
                    $context['name'] = htmlspecialchars($row['posterName'], ENT_QUOTES | ENT_HTML5);

                    $context['email'] = htmlspecialchars($row['posterEmail'], ENT_QUOTES | ENT_HTML5);
                }
            }
        }
    ...
    elseif (isset($_REQUEST['msg'])) {
        ...
        while (false !== ($row2 = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        if ($row['ID_MEMBER'] == $ID_MEMBER && !allowedTo('modify_any')) {
            ...
            if (!empty($modSettings['edit_disable_time']) && time() > $row['posterTime'] + ($modSettings['edit_disable_time'] + 5) * 60) {
        ...
        if (!empty($topic) && !empty($_REQUEST['quote'])) {
            ...
            [$form_subject, $mname, $mdate, $form_message] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (allowedTo('post_attachment')) {
        ...
        if (isset($_REQUEST['msg'])) {
            ...
            [$quantity, $total_size] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        if (!empty($_SESSION['temp_attachments'])) {
            foreach ($_SESSION['temp_attachments'] as $attachID => $name) {
                ...
                if (0 == preg_match('~^post_tmp_' . $ID_MEMBER . '_\d+$~', $attachID)) {
                    unset($_SESSION['temp_attachments'][$attachID]);

                    continue;
                }

                if (!empty($_POST['attach_del']) && !in_array($attachID, $_POST['attach_del'], true)) {
        ...
        if (!empty($_POST['attach_del'])) {
            ...
            foreach ($context['current_attachments'] as $k => $dummy) {
                if (!in_array($dummy['id'], $del_temp, true)) {
        ...
        if (!empty($_FILES['attachment'])) {
            foreach ($_FILES['attachment']['tmp_name'] as $n => $dummy) {
                ...
                if (!empty($modSettings['attachmentSizeLimit']) && $_FILES['attachment']['size'][$n] > $modSettings['attachmentSizeLimit'] * 1024) {
                    fatal_lang_error('smf122', false, [$modSettings['attachmentSizeLimit']]);
                }

                $quantity++;

                if (!empty($modSettings['attachmentNumPerPostLimit']) && $quantity > $modSettings['attachmentNumPerPostLimit']) {
                    fatal_lang_error('attachments_limit_per_post', false, [$modSettings['attachmentNumPerPostLimit']]);
                }

                $total_size += $_FILES['attachment']['size'][$n];

                if (!empty($modSettings['attachmentPostLimit']) && $total_size > $modSettings['attachmentPostLimit'] * 1024) {
                    fatal_lang_error('smf122', false, [$modSettings['attachmentPostLimit']]);
                }

                if (!empty($modSettings['attachmentCheckExtensions'])) {
                    if (!in_array(mb_strtolower(mb_substr(mb_strrchr($_FILES['attachment']['name'][$n], '.'), 1)), explode(',', mb_strtolower($modSettings['attachmentExtensions'])), true)) {
    ...
    if (isset($newRepliesError)) {
        $context['post_error']['messages'][] = 1 == $newRepliesError ? $txt['error_new_reply'] : $txt['error_new_replies'];
    ...
    if (isset($_REQUEST['poll'])) {
        ...
        $context['page_title'] = -1 == $context['event']['id'] ? $txt['calendar23'] : $txt['calendar20'];
    } elseif (isset($_REQUEST['msg'])) {
    ...
    $context['num_allowed_attachments'] = min($modSettings['attachmentNumPerPostLimit'] - count($context['current_attachments']) + ($deleted_attachments ?? 0), $modSettings['attachmentNumPerPostLimit']);

    $context['can_post_attachment'] = !empty($modSettings['attachmentEnable']) && 1 == $modSettings['attachmentEnable'] && allowedTo('post_attachment') && $context['num_allowed_attachments'] > 0;
    ...
    $context['allowed_extensions'] = strtr($modSettings['attachmentExtensions'], [',' => ', ']);
    ...
    if (empty($modSettings['messageIcons_enable'])) {
        $context['icons'] = [
            ['value' => 'xx', 'name' => $txt[281]],
            ['value' => 'thumbup', 'name' => $txt[282]],
            ['value' => 'thumbdown', 'name' => $txt[283]],
            ['value' => 'exclamation', 'name' => $txt[284]],
            ['value' => 'question', 'name' => $txt[285]],
            ['value' => 'lamp', 'name' => $txt[286]],
            ['value' => 'smiley', 'name' => $txt[287]],
            ['value' => 'angry', 'name' => $txt[288]],
            ['value' => 'cheesy', 'name' => $txt[289]],
            ['value' => 'grin', 'name' => $txt[293]],
            ['value' => 'sad', 'name' => $txt[291]],
            ['value' => 'wink', 'name' => $txt[292]],
        ];
        ...
        $context['javascript_icons'] = [];

        if (null === ($temp = cache_get_data('posting_icons-' . $board, 480))) {
            $request = db_query(
                "
        ...
        foreach ($icon_data as $icon) {
            ...
            if ('default_images_url' == $context['icon_sources'][$icon['filename']]) {
        ...
        $context['icon_url'] = $settings[$context['icon_sources'][$context['icon']] ?? 'images_url'] . '/post/' . $context['icon'] . '.gif';
    }
...
function Post2()
{
    ...
    if (!empty($topic) && !isset($_REQUEST['msg'])) {
        ...
        [$tmplocked, $tmpstickied, $pollID, $numReplies, $ID_MEMBER_POSTER] = $GLOBALS['xoopsDB']->fetchRow($request);

        $GLOBALS['xoopsDB']->freeRecordSet($request);

        // Don't allow a post if it's locked.

        if (0 != $tmplocked && !allowedTo('moderate_board')) {
            fatal_lang_error(90, false);
        }
        ...
        if (isset($_POST['lock'])) {
            ...
            elseif (!allowedTo(['lock_any', 'lock_own']) || (!allowedTo('lock_any') && $ID_MEMBER != $ID_MEMBER_POSTER)) {
                unset($_POST['lock']);
    ...
    elseif (isset($_REQUEST['msg']) && !empty($topic)) {
        ...
        if (isset($_POST['lock'])) {
            ...
            elseif (!allowedTo(['lock_any', 'lock_own']) || (!allowedTo('lock_any') && $ID_MEMBER != $row['ID_MEMBER_POSTER'])) {
                unset($_POST['lock']);
        ...
        if ($row['ID_MEMBER'] == $ID_MEMBER && !allowedTo('modify_any')) {
            if (!empty($modSettings['edit_disable_time']) && time() > $row['posterTime'] + $modSettings['edit_disable_time'] * 60) {
                fatal_lang_error('modify_post_time_passed', false);
            } elseif ($row['ID_MEMBER_POSTER'] == $ID_MEMBER && !allowedTo('modify_own')) {
                isAllowedTo('modify_replies');
            } else {
    ...
    if ($posterIsGuest) {
        ...
        if ('' == $_POST['guestname'] || '_' == $_POST['guestname']) {
            $post_errors[] = 'no_name';
        }
        ...
        if (empty($modSettings['guest_post_no_email'])) {
            ...
            if (!isset($row) || $row['posterEmail'] != $_POST['email']) {
                ...
                if (!allowedTo('moderate_forum') && 0 == preg_match('~^[0-9A-Za-z=_+\-/][0-9A-Za-z=_\'+\-/\.]*@[\w\-]+(\.[\w\-]+)*(\.[\w]{2,6})$~', stripslashes($_POST['email']))) {
    ...
    } elseif (!empty($modSettings['max_messageLength']) && $modSettings['max_messageLength'] < $func['strlen']($_POST['message'])) {
    ...
    if (isset($_POST['message']) && 'i am the administrator.' == mb_strtolower($_POST['message']) && !$user_info['is_admin']) {
    ...
    $_POST['subject'] = strtr($func['htmlspecialchars']($_POST['subject']), ["\r" => '', "\n" => '', "\t" => '']);

    $_POST['guestname'] = htmlspecialchars($_POST['guestname'], ENT_QUOTES | ENT_HTML5);

    $_POST['email'] = htmlspecialchars($_POST['email'], ENT_QUOTES | ENT_HTML5);
    ...
    if (isset($_REQUEST['poll'])) {
        ...
        elseif (empty($_POST['poll_expire']) && 2 == $_POST['poll_hide']) {
            $_POST['poll_hide'] = 1;
        }
    ...
    if (isset($_FILES['attachment']['name']) || !empty($_SESSION['temp_attachments'])) {
        ...
        if (!empty($_SESSION['temp_attachments'])) {
            foreach ($_SESSION['temp_attachments'] as $attachID => $name) {
                ...
                [$_FILES['attachment']['width'][], $_FILES['attachment']['height'][]] = @getimagesize($modSettings['attachmentUploadDir'] . '/' . $attachID);
        ...
        foreach ($_FILES['attachment']['tmp_name'] as $n => $dummy) {
            ...
            if (createAttachment($attachmentOptions)) {
                ...
                if (in_array('could_not_upload', $attachmentOptions['errors'], true)) {
                    fatal_lang_error('smf124');
                }

                if (in_array('too_large', $attachmentOptions['errors'], true)) {
                    fatal_lang_error('smf122', false, [$modSettings['attachmentSizeLimit']]);
                }

                if (in_array('bad_extension', $attachmentOptions['errors'], true)) {
                    fatal_error($attachmentOptions['name'] . '.<br>' . $txt['smf123'] . ' ' . $modSettings['attachmentExtensions'] . '.', false);
                }

                if (in_array('directory_full', $attachmentOptions['errors'], true)) {
                    fatal_lang_error('smf126');
                }

                if (in_array('bad_filename', $attachmentOptions['errors'], true)) {
                    fatal_error(basename($attachmentOptions['name']) . '.<br>' . $txt['smf130b'] . '.');
                }

                if (in_array('taken_filename', $attachmentOptions['errors'], true)) {
    ...
    if (isset($_REQUEST['poll'])) {
        ...
        db_query(
			...
			VALUES (SUBSTRING('$_POST[question]', 1, 255), $_POST[poll_hide], $_POST[poll_max_votes],
				" . (empty($_POST['poll_expire']) ? '0' : time() + $_POST['poll_expire'] * 3600 * 24) . ", $ID_MEMBER, SUBSTRING('$_POST[guestname]', 1, 255), $_POST[poll_change_vote])",
            __FILE__,
            __LINE__
        );
        ...
        db_query(
			...
			VALUES" . mb_substr($setString, 0, -1),
            __FILE__,
            __LINE__
        );
    } else {
    ...
    if (isset($_POST['calendar']) && (!isset($_REQUEST['eventid']) || -1 == $_REQUEST['eventid'])) {
        require_once $sourcedir . '/Calendar.php';

        calendarCanLink();

        calendarInsertEvent($board, $topic, $_POST['evtitle'], $ID_MEMBER, $_POST['month'], $_POST['day'], $_POST['year'], $_POST['span'] ?? null);
        ...
        if (!allowedTo('calendar_edit_any')) {
            ...
            $row2 = $GLOBALS['xoopsDB']->fetchArray($request);
    ...
    if (!empty($moderationAction)) {
        logAction('modify', ['topic' => $topic, 'message' => (int)$_REQUEST['msg'], 'member' => $row['ID_MEMBER']]);
    }

    if (isset($_POST['lock']) && 2 != $_POST['lock']) {
        logAction('lock', ['topic' => $topicOptions['id']]);
    }

    if (isset($_POST['sticky']) && !empty($modSettings['enableStickyTopics'])) {
        logAction('sticky', ['topic' => $topicOptions['id']]);
    }
...
function AnnouncementSelectMembergroup()
{
    ...
    [$context['topic_subject']] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function AnnouncementSend()
{
    ...
    foreach ($_POST['who'] as $id => $mg) {
        $_POST['who'][$id] = in_array((int)$mg, $groups, true) ? (int)$mg : 0;
    }
    ...
    [$ID_MSG, $context['topic_subject'], $message] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    $message = trim(un_htmlspecialchars(strip_tags(strtr(parse_bbc($message, false, $ID_MSG), ['<br>' => "\n", '</div>' => "\n", '</li>' => "\n", '&#91;' => '[', '&#93;' => ']']))));
...
function notifyMembersBoard()
{
    ...
    $message = trim(un_htmlspecialchars(strip_tags(strtr(parse_bbc($message, false), ['<br>' => "\n", '</div>' => "\n", '</li>' => "\n", '&#91;' => '[', '&#93;' => ']']))));
    ...
    while (false !== ($rowmember = $GLOBALS['xoopsDB']->fetchArray($members))) {
        if (1 != $rowmember['ID_GROUP']) {
            ...
            if (0 == count(array_intersect($allowed, $rowmember['additionalGroups']))) {
                continue;
            }
        }
        ...
        if (!empty($rowmember['notifyOnce']) && empty($rowmember['sent'])) {
            sendmail(
                ...
                . $txt['notify_boardsUnsubscribe']
                . ': '
                . $scripturl
                . '?action=notifyboard;board='
                . $board
                . ".0\n\n"
                . $txt[130],
                null,
...
function QuoteFast()
{
    ...
    $context['close_window'] = 0 == $GLOBALS['xoopsDB']->getRowsNum($request);
    ...
    if (0 != $GLOBALS['xoopsDB']->getRowsNum($request)) {
        ...
        $context['quote']['mozilla'] = strtr($func['htmlspecialchars']($context['quote']['text']), ['&quot;' => '"']);
    }
...
function JavaScriptModify()
{
    ...
    if (isset($_POST['lock'])) {
        if (!allowedTo(['lock_any', 'lock_own']) || (!allowedTo('lock_any') && $ID_MEMBER != $row['ID_MEMBER'])) {
            unset($_POST['lock']);
        } elseif (!allowedTo('lock_any')) {
            if (1 == $row['locked']) {
                unset($_POST['lock']);
    ...
    if (empty($post_errors)) {
        ...
        if ($moderationAction) {
            logAction('modify', ['topic' => $topic, 'message' => $row['ID_MSG'], 'member' => $row['ID_MEMBER_STARTED']]);
        }
    }
    ...
    if (isset($_REQUEST['xml'])) {
        ...
        elseif (empty($post_errors) && isset($msgOptions['subject'])) {
            ...
            $context['message'] = [
                'id' => $row['ID_MSG'],
                'errors' => [],
                'error_in_subject' => in_array('no_subject', $post_errors, true),
                'error_in_body' => in_array('no_message', $post_errors, true) || in_array('long_message', $post_errors, true),
            ];
