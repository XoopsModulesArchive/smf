...
function ManageAvatarSettings()
{
    ...
    if (!empty($_POST['avatarSettings'])) {
        ...
        save_inline_permissions(['profile_server_avatar', 'profile_upload_avatar', 'profile_remote_avatar']);
    }

    init_inline_permissions(['profile_server_avatar', 'profile_upload_avatar', 'profile_remote_avatar'], [-1]);
...
function BrowseFiles()
{
    ...
    [$context['num_avatars']] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    $sort_methods = [
        'name' => 'a.filename',
        'date' => 'avatars' == $context['browse_type'] ? 'mem.lastLogin' : 'm.ID_MSG',
        'size' => 'a.size',
        'member' => 'mem.realName',
    ];
    ...
    $context['page_index'] = constructPageIndex(
        $scripturl . '?action=manageattachments;sa=' . $_REQUEST['sa'] . ('attachments' == $context['browse_type'] ? '' : ';' . $context['browse_type']) . ';sort=' . $context['sort_by'] . ('down' == $context['sort_direction'] ? ';desc' : ''),
        $_REQUEST['start'],
        $context['num_' . $context['browse_type']],
    ...
    if ('avatars' == $context['browse_type']) {
        $request = db_query(
            "
			SELECT
        ...
        $request = db_query(
			...
			WHERE a.ID_MSG = m.ID_MSG
				AND a.attachmentType = " . ('attachments' == $context['browse_type'] ? '0' : '3') . "
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        $context['posts'][] = [
            ...
            'attachment' => [
                ...
                'href' => 1 == $row['attachmentType'] ? $modSettings['custom_avatar_url'] . '/' . $row['filename'] : ($scripturl . '?action=dlattach;' . ('avatars' == $context['browse_type'] ? 'type=avatar;' : 'topic=' . $row['ID_TOPIC'] . '.0;') . 'id=' . $row['ID_ATTACH']),
                'link' => '<a href="'
                               . (1 == $row['attachmentType'] ? $modSettings['custom_avatar_url'] . '/' . $row['filename'] : ($scripturl . '?action=dlattach;' . ('avatars' == $context['browse_type'] ? 'type=avatar;' : 'topic=' . $row['ID_TOPIC'] . '.0;') . 'id=' . $row['ID_ATTACH']))
                               . '"'
                               . (empty($row['width']) || empty($row['height']) ? '' : ' onclick="return reqWin(this.href + \';image\', ' . ($row['width'] + 20) . ', ' . ($row['height'] + 20) . ', true);"')
                               . '>'
                               . $row['filename']
                               . '</a>',
            ],
...
function MaintainFiles()
{
    ...
    [$context['num_attachments']] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function MoveAvatars()
{
    ...
    if (!empty($updatedAvatars)) {
        db_query(
			...
			WHERE ID_ATTACH IN (" . implode(', ', $updatedAvatars) . ')',
            __FILE__,
            __LINE__
        );
    }
...
function RemoveAttachment()
{
    ...
    if (!empty($_POST['remove'])) {
        ...
        if ('avatars' == $_REQUEST['type'] && !empty($attachments)) {
...
function removeAttachments($condition, $query_type = '', $return_affected_messages = false, $autoThumbRemoval = true)
{
    ...
    $request = db_query(
        '
		SELECT
			a.filename, a.attachmentType, a.ID_ATTACH, a.ID_MEMBER' . ('messages' == $query_type ? ', m.ID_MSG' : ', a.ID_MSG') . ",
			IFNULL(thumb.ID_ATTACH, 0) AS ID_THUMB, thumb.filename AS thumb_filename, thumb_parent.ID_ATTACH AS ID_PARENT
		FROM ({$db_prefix}attachments AS a" . ('members' == $query_type ? ", {$db_prefix}members AS mem" : ('messages' == $query_type ? ", {$db_prefix}messages AS m" : '')) . ")
		...
		WHERE $condition" . ('messages' == $query_type ? '
			AND m.ID_MSG = a.ID_MSG' : '') . ('members' == $query_type ? '
...
function RepairAttachments()
{
    ...
    $context['repair_errors'] = $_SESSION['attachments_to_fix2'] ?? [];
    ...
    if ($_GET['step'] <= 0) {
        ...
        [$thumbnails] = $GLOBALS['xoopsDB']->fetchRow($result);
        ...
        for (; $_GET['substep'] < $thumbnails; $_GET['substep'] += 500) {
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
                ...
                if ($fix_errors && in_array('missing_thumbnail_parent', $to_fix, true)) {
                    $filename = getAttachmentFilename($row['filename'], $row['ID_ATTACH']);
            ...
            if ($fix_errors && !empty($to_remove) && in_array('missing_thumbnail_parent', $to_fix, true)) {
                db_query(
                    "
					DELETE FROM {$db_prefix}attachments
    ...
    if ($_GET['step'] <= 1) {
        ...
        for (; $_GET['substep'] < $thumbnails; $_GET['substep'] += 500) {
            ...
            if ($fix_errors && !empty($to_update) && in_array('parent_missing_thumbnail', $to_fix, true)) {
                db_query(
                    "
					UPDATE {$db_prefix}attachments
					SET ID_THUMB = 0
					WHERE ID_ATTACH IN (" . implode(', ', $to_update) . ')',
                    __FILE__,
                    __LINE__
                );
            }
        ...
        for (; $_GET['substep'] < $thumbnails; $_GET['substep'] += 250) {
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
                ...
                if (!file_exists($filename)) {
                    ...
                    if ($fix_errors && in_array('file_missing_on_disk', $to_fix, true)) {
                        $to_remove[] = $row['ID_ATTACH'];
                    }
                    ...
                    if ($fix_errors && in_array('file_size_of_zero', $to_fix, true)) {
                        $to_remove[] = $row['ID_ATTACH'];
                    ...
                    if ($fix_errors && in_array('file_wrong_size', $to_fix, true)) {
                        db_query(
                            "
							UPDATE {$db_prefix}attachments
            ...
            if (in_array('file_missing_on_disk', $errors_found, true)) {
                $to_fix[] = 'file_missing_on_disk';
            }

            if (in_array('file_size_of_zero', $errors_found, true)) {
                $to_fix[] = 'file_size_of_zero';
            }

            if (in_array('file_wrong_size', $errors_found, true)) {
            ...
            if ($fix_errors && !empty($to_remove)) {
                ...
                db_query(
					...
					UPDATE {$db_prefix}attachments
					SET ID_THUMB = 0
					WHERE ID_THUMB IN (" . implode(', ', $to_remove) . ')',
                    __FILE__,
                    __LINE__
                );
            }
    ...
    if ($_GET['step'] <= 3) {
        ...
        for (; $_GET['substep'] < $thumbnails; $_GET['substep'] += 500) {
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
                ...
                if ($fix_errors && in_array('avatar_no_member', $to_fix, true)) {
                    if (1 == $row['attachmentType']) {
                        $filename = $modSettings['custom_avatar_dir'] . '/' . $row['filename'];
                    } else {
                        $filename = getAttachmentFilename($row['filename'], $row['ID_ATTACH']);
    ...
    if ($_GET['step'] <= 4) {
        ...
        for (; $_GET['substep'] < $thumbnails; $_GET['substep'] += 500) {
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
                ...
                if ($fix_errors && in_array('attachment_no_msg', $to_fix, true)) {
                    $filename = getAttachmentFilename($row['filename'], $row['ID_ATTACH']);
            ...
            if ($fix_errors && !empty($to_remove) && in_array('attachment_no_msg', $to_fix, true)) {
                db_query(
                    "
					DELETE FROM {$db_prefix}attachments
					WHERE ID_ATTACH IN (" . implode(', ', $to_remove) . ')
