...
function cleanRequest()
{
    ...
    if ((false === mb_strpos(@ini_get('arg_separator.input'), ';') || -1 == @version_compare(PHP_VERSION, '4.2.0')) && !empty($_SERVER['QUERY_STRING'])) {
        ...
        $_SERVER['QUERY_STRING'] = urldecode('url=/' == mb_substr($_SERVER['QUERY_STRING'], 0, 5) ? $_SERVER['REDIRECT_QUERY_STRING'] : $_SERVER['QUERY_STRING']);

        // Replace ';' with '&' and '&something&' with '&something=&'.  (this is done for compatibility...)

        // !!! smflib

        parse_str(preg_replace('/&(\w+)(?=&|$)/', '&$1=', strtr($_SERVER['QUERY_STRING'], [';?' => '&', ';' => '&'])), $_GET);
    } elseif (false !== mb_strpos(@ini_get('arg_separator.input'), ';')) {
        $_GET = urldecode__recursive($_GET);

        if (function_exists('get_magic_quotes_gpc') && 0 != @get_magic_quotes_gpc() && empty($modSettings['integrate_magic_quotes'])) {
        ...
        foreach ($_GET as $k => $v) {
            if (is_string($v) && false !== mb_strpos($k, ';')) {
                $temp = explode(';', $v);
    ...
    if (!empty($_SERVER['REQUEST_URI'])) {
        ...
        if ('.htm' == mb_substr($_SERVER['REQUEST_URI'], mb_strrpos($_SERVER['REQUEST_URI'], '.'), 4)) {
            $request = mb_substr($_SERVER['REQUEST_URI'], 0, mb_strrpos($_SERVER['REQUEST_URI'], '.'));
        } else {
        ...
        parse_str(mb_substr(preg_replace('/&(\w+)(?=&|$)/', '&$1=', strtr(preg_replace('~/([^,/]+),~', '/$1=', mb_substr($request, mb_strpos($request, basename($scripturl)) + mb_strlen(basename($scripturl)))), '/', '&')), 1), $temp);
    ...
    if (isset($_REQUEST['topic'])) {
        ...
        if (false !== mb_strpos($_REQUEST['topic'], '/')) {
            [$_REQUEST['topic'], $_REQUEST['start']] = explode('/', $_REQUEST['topic']);
        } // Dots are useful and fun ;).  This is ?topic=1.15.

        elseif (false !== mb_strpos($_REQUEST['topic'], '.')) {
            [$_REQUEST['topic'], $_REQUEST['start']] = explode('.', $_REQUEST['topic']);
        }
    ...
    $_SERVER['BAN_CHECK_IP'] = isset($_SERVER['REMOTE_ADDR']) && 1 === preg_match('~^((([1]?\d)?\d|2[0-4]\d|25[0-5])\.){3}(([1]?\d)?\d|2[0-4]\d|25[0-5])$~', $_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 'unknown';

    // Find the user's IP address. (but don't let it give you 'unknown'!)

    if (!empty($_SERVER['HTTP_X_FORWARDED_FOR']) && !empty($_SERVER['HTTP_CLIENT_IP']) && (0 == preg_match('~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~', $_SERVER['HTTP_CLIENT_IP']) || 0 != preg_match('~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~', $_SERVER['REMOTE_ADDR']))) {
        // We have both forwarded for AND client IP... check the first forwarded for as the block - only switch if it's better that way.

        if (strtok($_SERVER['HTTP_X_FORWARDED_FOR'], '.') != strtok($_SERVER['HTTP_CLIENT_IP'], '.') && '.' . strtok($_SERVER['HTTP_X_FORWARDED_FOR'], '.') == mb_strrchr($_SERVER['HTTP_CLIENT_IP'], '.')
            && (0 == preg_match(
                '~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~',
                $_SERVER['HTTP_X_FORWARDED_FOR']
            )
                || 0 != preg_match('~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~', $_SERVER['REMOTE_ADDR']))) {
    ...
    if (!empty($_SERVER['HTTP_CLIENT_IP']) && (0 == preg_match('~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~', $_SERVER['HTTP_CLIENT_IP']) || 0 != preg_match('~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~', $_SERVER['REMOTE_ADDR']))) {
        ...
        if (false !== mb_strpos($_SERVER['HTTP_X_FORWARDED_FOR'], ',')) {
            ...
            foreach ($ips as $i => $ip) {
                ...
                if (0 != preg_match('~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~', $ip) && 0 == preg_match('~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~', $_SERVER['REMOTE_ADDR'])) {
                    continue;
                }
        ...
        elseif (0 == preg_match('~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~', $_SERVER['HTTP_X_FORWARDED_FOR']) || 0 != preg_match('~^((0|10|172\.16|192\.168|255|127\.0)\.|unknown)~', $_SERVER['REMOTE_ADDR'])) {
    ...
    } elseif (1 == preg_match('~^([^/]+//[^/]+)~', $scripturl, $match)) {
    ...
    if (0 === preg_match('~^((([1]?\d)?\d|2[0-4]\d|25[0-5])\.){3}(([1]?\d)?\d|2[0-4]\d|25[0-5])$~', $_SERVER['REMOTE_ADDR'])) {
        $_SERVER['REMOTE_ADDR'] = '';
    }
}
...
function validate_unicode__recursive($var)
{
    ...
    for ($i = 0, $iMax = mb_strlen($var); $i < $iMax; $i++) {
        $c = ord($var[$i]);

        if (in_array($c, $cleanup, true)) {
            $var = mb_substr($var, 0, $i) . mb_substr($var, $i + 1);
...
function ob_sessrewrite($buffer)
{
    ...
    if (empty($_COOKIE) && SID != '' && empty($context['browser']['possibly_robot']) && -1 != @version_compare(PHP_VERSION, '4.3.0')) {
    ...
    if (!empty($modSettings['queryless_urls']) && (!$context['server']['is_cgi'] || 1 == @ini_get('cgi.fix_pathinfo')) && $context['server']['is_apache']) {
