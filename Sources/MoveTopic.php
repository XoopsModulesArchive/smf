...
function MoveTopic()
{
    ...
    [$ID_MEMBER_STARTED, $context['subject']] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function MoveTopic2()
{
    ...
    [$ID_MEMBER_STARTED, $ID_FIRST_MSG] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    [$pcounter, $board_name, $subject] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    [$pcounter_from] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if ($pcounter_from != $pcounter) {
        ...
        if (empty($pcounter_from)) {
            updateMemberData($posters, ['posts' => '-']);
    ...
    if (!empty($modSettings['cache_enable']) && 3 == $modSettings['cache_enable']) {
...
function moveTopics($topics, $toBoard)
{
    ...
    $request = db_query(
		...
		WHERE b.ID_BOARD = $toBoard",
        __FILE__,
        __LINE__
    );

    [$isSeen] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (!empty($isSeen) && !$user_info['is_guest']) {
        db_query(
			...
			VALUES ($toBoard, $ID_MEMBER, $modSettings[maxMsgID])",
            __FILE__,
            __LINE__
        );
    }
