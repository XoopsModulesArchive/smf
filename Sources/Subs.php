...
/*	This file has all the main functions in it that relate to, well,
    ...
    int db_insert_id()
        - should always be used in place of $GLOBALS['xoopsDB']->getInsertId.
...
function db_query($db_string, $file, $line)
{
    ...
    if (isset($db_show_debug) && true === $db_show_debug) {
    ...
    if (empty($modSettings['disableQueryCheck'])) {
        ...
        while (true) {
            $pos = mb_strpos($db_string, '\'', $pos + 1);

            if (false === $pos) {
                break;
            }

            $clean .= mb_substr($db_string, $old_pos, $pos - $old_pos);

            while (true) {
                $pos1 = mb_strpos($db_string, '\'', $pos + 1);

                $pos2 = mb_strpos($db_string, '\\', $pos + 1);
        ...
        $clean .= mb_substr($db_string, $old_pos);

        $clean = trim(mb_strtolower(preg_replace(['~\s+~s', '~/\*!40001 SQL_NO_CACHE \*/~', '~/\*!40000 USE INDEX \([A-Za-z\_]+?\) \*/~'], [' ', '', ''], $clean)));

        // We don't use UNION in SMF, at least so far.  But it's useful for injections.

        if (false !== mb_strpos($clean, 'union') && 0 != preg_match('~(^|[^a-z])union($|[^[a-z])~s', $clean)) {
            $fail = true;
        } // Comments?  We don't use comments in our queries, we leave 'em outside!

        elseif (mb_strpos($clean, '/*') > 2 || false !== mb_strpos($clean, '--') || false !== mb_strpos($clean, ';')) {
            $fail = true;
        } // Trying to change passwords, slow us down, or something?

        elseif (false !== mb_strpos($clean, 'set password') && 0 != preg_match('~(^|[^a-z])set password($|[^[a-z])~s', $clean)) {
            $fail = true;
        } elseif (false !== mb_strpos($clean, 'benchmark') && 0 != preg_match('~(^|[^a-z])benchmark($|[^[a-z])~s', $clean)) {
            $fail = true;
        } // Sub selects?  We don't use those either.

        elseif (0 != preg_match('~\([^)]*?select~s', $clean)) {
            $fail = true;
        }
    ...
    if (false !== mb_strpos($db_string, '_smf_members')) {
        //echo $db_string."<hr>";

        $db_string = str_replace('_smf_members', '_users', $db_string);
    }

    if (false !== mb_strpos($db_string, '_smf_sessions')) {
        //echo $db_string."<hr>";

        $db_string = str_replace('_smf_sessions', '_session', $db_string);

        $db_string = str_replace('data', 'sess_data', $db_string);

        $db_string = str_replace('session_id', 'sess_id', $db_string);

        $db_string = str_replace('last_update', 'sess_updated', $db_string);
    }

    // SIMPLE-XOOPS END

    $ret = $GLOBALS['xoopsDB']->queryF($db_string, $db_connection);

    if (false === $ret && false !== $file) {
...
function db_affected_rows()
{
    ...
    return $GLOBALS['xoopsDB']->getAffectedRows($db_connection);
}
...
function db_insert_id()
{
    ...
    return $GLOBALS['xoopsDB']->getInsertId($db_connection);
}
...
function updateStats($type, $parameter1 = null, $parameter2 = null)
{
    ...
    switch ($type) {
        case 'member':
            ...
            if (!empty($modSettings['registration_method']) && 2 == $modSettings['registration_method']) {
                ...
                [$changes['totalMembers'], $changes['latestMember']] = $GLOBALS['xoopsDB']->fetchRow($result);
                ...
                [$changes['latestRealName']] = $GLOBALS['xoopsDB']->fetchRow($result);
                ...
                [$changes['unapprovedMembers']] = $GLOBALS['xoopsDB']->fetchRow($result);

                $GLOBALS['xoopsDB']->freeRecordSet($result);
            } // If $parameter1 is a number, it's the new ID_MEMBER and #2 is the real name for a new registration.
            elseif (null !== $parameter1 && false !== $parameter1) {
            ...
            break;
        case 'message':
            if (true === $parameter1 && null !== $parameter2) {
        ...
        case 'topic':
            if (true === $parameter1) {
                ...
                updateSettings(['totalTopics' => $row['totalTopics']]);
            }
            break;
        case 'calendar':
        ...
        case 'postgroups':
            ...
            if (null !== $parameter2 && !in_array('posts', $parameter2, true)) {
                return;
            }

            if (null === ($postgroups = cache_get_data('updateStats:postgroups', 360))) {
...
function updateMemberData($members, $data)
{
    ...
    if (isset($modSettings['integrate_change_member_data']) && function_exists($modSettings['integrate_change_member_data'])) {
        ...
        if (0 != count($vars_to_integrate)) {
            // Fetch a list of memberNames if necessary

            if ((!is_array($members) && $members === $ID_MEMBER) || (is_array($members) && 1 == count($members) && in_array($ID_MEMBER, $members, true))) {
                $memberNames = [$user_info['username']];
            } else {
                $memberNames = [];
    ...
    if ($update) {
        foreach ($changeArray as $variable => $value) {
            db_query(
				...
				SET value = " . (true === $value ? 'value + 1' : (false === $value ? 'value - 1' : "'$value'")) . "
            ...
            $modSettings[$variable] = true === $value ? $modSettings[$variable] + 1 : (false === $value ? $modSettings[$variable] - 1 : stripslashes($value));
        }
...
function constructPageIndex($base_url, &$start, $max_value, $num_per_page, $flexible_start = false)
{
    ...
    elseif ($start >= $max_value) {
        $start = max(0, (int)$max_value - (0 == ((int)$max_value % (int)$num_per_page) ? $num_per_page : ((int)$max_value % (int)$num_per_page)));
    ...
    $base_link = '<a class="navPages" href="' . ($flexible_start ? $base_url : strtr($base_url, ['%' => '%%']) . ';start=%d') . '">%s</a> ';
    ...
    if (empty($modSettings['compactTopicPagesEnable'])) {
        ...
        $pageindex = 0 == $start ? ' ' : sprintf($base_link, $start - $num_per_page, '&#171;');
...
function comma_format($number, $override_decimal_count = false)
{
    ...
    if (null === $decimal_separator) {
        ...
        if (empty($modSettings['number_format']) || 1 != preg_match('~^1([^\d]*)?234([^\d]*)(0*?)$~', $modSettings['number_format'], $matches)) {
            return $number;
        }
        ...
        $decimal_count = mb_strlen($matches[3]);
    }

    // Format the string with our friend, number_format.

    return number_format($number, is_float($number) ? (false === $override_decimal_count ? $decimal_count : $override_decimal_count) : 0, $decimal_separator, $thousands_separator);
}
...
function timeformat($logTime, $show_today = true)
{
    ...
    if ($modSettings['todayMod'] >= 1 && true === $show_today) {
        ...
        $s = false === mb_strpos($user_info['time_format'], '%S') ? '' : ':%S';

        if (false === mb_strpos($user_info['time_format'], '%H') && false === mb_strpos($user_info['time_format'], '%T')) {
            $today_fmt = '%I:%M' . $s . ' %p';
        } else {
            $today_fmt = '%H:%M' . $s;
        }
        ...
        if ('2' == $modSettings['todayMod'] && (($now['yday'] - 1 == $then['yday'] && $then['year'] == $now['year']) || (0 == $now['yday'] && $now['year'] - 1 == $then['year']) && 12 == $then['mon'] && 31 == $then['mday'])) {
    ...
    if (setlocale(LC_TIME, $txt['lang_locale'])) {
        ...
        foreach (['%a' => 'days_short', '%A' => 'days', '%b' => 'months_short', '%B' => 'months'] as $token => $text_label) {
            if (false !== mb_strpos($str, $token)) {
                $str = str_replace($token, $txt[$text_label][(int)strftime('%a' === $token || '%A' === $token ? '%w' : '%m', $time)], $str);
            }
        }
...
function parse_bbc($message, $smileys = true, $cache_id = '')
{
    ...
    if (WIRELESS) {
        $smileys = false;
    } elseif (null !== $smileys && ('1' == $smileys || '0' == $smileys)) {
        $smileys = (bool)$smileys;
    }

    if (empty($modSettings['enableBBC']) && false !== $message) {
        if (true === $smileys) {
    ...
    if (!isset($context['utf8'])) {
        $context['utf8'] = 'UTF-8' === (empty($modSettings['global_character_set']) ? $txt['lang_character_set'] : $modSettings['global_character_set']);
    }

    // Sift out the bbc for a performance improvement.

    if (empty($bbc_codes) || false === $message) {
        if (!empty($modSettings['disabledBBC'])) {
            $temp = explode(',', mb_strtolower($modSettings['disabledBBC']));
        ...
        */
        ...
        $codes = [
            ...
            [
                ...
                'validate' => isset($disabled['code']) ? null : create_function(
                    '&$tag, &$data, $disabled',
                    '
					global $context;
					...
					{
						$php_parts = preg_preg_split(\'~(&lt;\?php|\?&gt;)~\', $data, -1, PREG_SPLIT_DELIM_CAPTURE);
            ...
            [
                ...
                'validate' => isset($disabled['code']) ? null : create_function(
					...
					if (!isset($disabled[\'code\']))
					{
						$php_parts = preg_preg_split(\'~(&lt;\?php|\?&gt;)~\', $data[0], -1, PREG_SPLIT_DELIM_CAPTURE);
            ...
            [
                ...
                'validate' => isset($disabled['php']) ? null : create_function(
                    '&$tag, &$data, $disabled',
                    '
            ...
            [
                ...
                'after' => '</span>',
                'validate' => $context['browser']['is_ie'] ? create_function(
                    '&$tag, &$data, $disabled',
                    '
					if ($data[1] == \'left\')
						$data[1] = 270;
        ...
        foreach ($codes as $c) {
            $bbc_codes[mb_substr($c['tag'], 0, 1)][] = $c;
        }
    ...
    if ('' != $cache_id && !empty($modSettings['cache_enable']) && (($modSettings['cache_enable'] >= 2 && mb_strlen($message) > 1000) || mb_strlen($message) > 2400)) {
        ...
        if (null != ($temp = cache_get_data($cache_key, 240))) {
            return $temp;
        }
    ...
    while (false !== $pos) {
        ...
        $pos = mb_strpos($message, '[', $pos + 1);

        // Failsafe.

        if (false === $pos || $last_pos > $pos) {
            $pos = mb_strlen($message) + 1;
        }
        ...
        if ($last_pos < $pos - 1) {
            ...
            $data = mb_substr($message, $last_pos, $pos - $last_pos + 1);

            // Take care of some HTML!

            if (!empty($modSettings['enablePostHTML']) && false !== mb_strpos($data, '&lt;')) {
                ...
                foreach ($closable_tags as $tag) {
                    $diff = mb_substr_count($data, '&lt;' . $tag . '&gt;') - mb_substr_count($data, '&lt;/' . $tag . '&gt;');
                ...
                if (!empty($matches[0])) {
                    ...
                    foreach ($matches[1] as $match => $imgtag) {
                        ...
                        if (0 != preg_match('~action(=|%3d)(?!dlattach)~i', $imgtag)) {
            ...
            if (!empty($modSettings['autoLinkUrls'])) {
                ...
                if (!empty($open_tags)) {
                    foreach ($open_tags as $open_tag) {
                        if (in_array($open_tag['tag'], $no_autolink_tags, true)) {
                ...
                if (!$no_autolink_area) {
                    ...
                    if (!isset($disabled['url']) && (false !== mb_strpos($data, '://') || false !== mb_strpos($data, 'www.'))) {
                        ...
                        $data = preg_replace(
                            [
                                '~(?<=[\s>\.(;\'"]|^)((?:http|https|ftp|ftps)://[\w\-_%@:|]+(?:\.[\w\-_%]+)*(?::\d+)?(?:/[\w\-_\~%\.@,\?&;=#+:\'\\\\]*|[\(\{][\w\-_\~%\.@,\?&;=#(){}+:\'\\\\]*)*[/\w\-_\~%@\?;=#}\\\\])~i',
                                '~(?<=[\s>(\'<]|^)(www(?:\.[\w\-_]+)+(?::\d+)?(?:/[\w\-_\~%\.@,\?&;=#+:\'\\\\]*|[\(\{][\w\-_\~%\.@,\?&;=#(){}+:\'\\\\]*)*[/\w\-_\~%@\?;=#}\\\\])~i',
                            ],
                            ['[url]$1[/url]', '[url=http://$1]$1[/url]'],
                            $data
                        );
                    ...
                    if (!isset($disabled['email']) && false !== mb_strpos($data, '@')) {
                        $data = preg_replace(
                            '~(?<=[\?\s' . $non_breaking_space . '\[\]()*\\\;>]|^)([\w\-\.]{1,80}@[\w\-]+\.[\w\-\.]+[\w\-])(?=[?,\s' . $non_breaking_space . '\[\]()*\\\]|$|<br>|&nbsp;|&gt;|&lt;|&quot;|&#039;|\.(?:\.|;|&nbsp;|\s|$|<br>))~' . ($context['utf8'] ? 'u' : ''),
                            '[email]$1[/email]',
                            $data
                        );
            ...
            if (!empty($modSettings['fixLongWords']) && $modSettings['fixLongWords'] > 5) {
                ...
                if (mb_strlen($data) > $modSettings['fixLongWords']) {
            ...
            if ($data != mb_substr($message, $last_pos, $pos - $last_pos + 1)) {
                $message = mb_substr($message, 0, $last_pos) . $data . mb_substr($message, $pos + 1);

                // Since we changed it, look again incase we added or removed a tag.  But we don't want to skip any.

                $old_pos = mb_strlen($data) + $last_pos - 1;

                $pos = mb_strpos($message, '[', $last_pos);

                $pos = false === $pos ? $old_pos : min($pos, $old_pos);
            }
        }

        // Are we there yet?  Are we there yet?

        if ($pos >= mb_strlen($message) - 1) {
            break;
        }

        $tags = mb_strtolower(mb_substr($message, $pos + 1, 1));
        ...
        if ('/' == $tags && !empty($open_tags)) {
            ...
            $look_for = mb_strtolower(mb_substr($message, $pos + 2, $pos2 - $pos - 2));
            ...
            do {
                ...
                if (!empty($tag['block_level'])) {
                    ...
                    if (mb_strlen($look_for) > 0 && isset($bbc_codes[$look_for[0]])) {
            ...
            if ((empty($open_tags) && (empty($tag) || $tag['tag'] != $look_for))) {
                ...
                if (null === $block_level && isset($look_for[0], $bbc_codes[$look_for[0]])) {
                    foreach ($bbc_codes[$look_for[0]] as $temp) {
                        if ($temp['tag'] == $look_for) {
                            $block_level = !empty($temp['block_level']);
            ...
            foreach ($to_close as $tag) {
                $message = mb_substr($message, 0, $pos) . $tag['after'] . mb_substr($message, $pos2 + 1);

                $pos += mb_strlen($tag['after']);

                $pos2 = $pos - 1;

                // See the comment at the end of the big loop - just eating whitespace ;).

                if (!empty($tag['block_level']) && '<br>' == mb_substr($message, $pos, 6)) {
                    $message = mb_substr($message, 0, $pos) . mb_substr($message, $pos + 6);
                }

                if (!empty($tag['trim']) && 'inside' != $tag['trim'] && 0 != preg_match('~(<br>|&nbsp;|\s)*~', mb_substr($message, $pos), $matches)) {
                    $message = mb_substr($message, 0, $pos) . mb_substr($message, $pos + mb_strlen($matches[0]));
                }
            }
        ...
        foreach ($bbc_codes[$tags] as $possible) {
            ...
            if (mb_strtolower(mb_substr($message, $pos + 1, mb_strlen($possible['tag']))) != $possible['tag']) {
                continue;
            }

            $next_c = mb_substr($message, $pos + 1 + mb_strlen($possible['tag']), 1);

            // A test validation?

            if (isset($possible['test']) && 0 == preg_match('~^' . $possible['test'] . '~', mb_substr($message, $pos + 1 + mb_strlen($possible['tag']) + 1))) {
                continue;
            ...
            elseif (!empty($possible['parameters'])) {
                ...
                if (in_array($possible['type'], ['unparsed_equals', 'unparsed_commas', 'unparsed_commas_content', 'unparsed_equals_content', 'parsed_equals'], true) && '=' != $next_c) {
                    continue;
                }

                // Maybe we just want a /...

                if ('closed' == $possible['type'] && ']' != $next_c && '/]' != mb_substr($message, $pos + 1 + mb_strlen($possible['tag']), 2) && ' /]' != mb_substr($message, $pos + 1 + mb_strlen($possible['tag']), 3)) {
                    continue;
                }

                // An immediate ]?

                if ('unparsed_content' == $possible['type'] && ']' != $next_c) {
                    continue;
                }
            ...
            if (isset($possible['require_parents']) && (null === $inside || !in_array($inside['tag'], $possible['require_parents'], true))) {
                continue;
            } elseif (isset($inside['require_children']) && !in_array($possible['tag'], $inside['require_children'], true)) {
                continue;
            } // If this is in the list of disallowed child tags, don't parse it.

            elseif (isset($inside['disallow_children']) && in_array($possible['tag'], $inside['disallow_children'], true)) {
                continue;
            }

            $pos1 = $pos + 1 + mb_strlen($possible['tag']) + 1;
            ...
            if (!empty($possible['parameters'])) {
                ...
                foreach ($possible['parameters'] as $p => $info) {
                    $preg[] = '(\s+' . $p . '=' . (empty($info['quoted']) ? '' : '&quot;') . ($info['match'] ?? '(.+?)') . (empty($info['quoted']) ? '' : '&quot;') . ')' . (empty($info['optional']) ? '' : '?');
                }
                ...
                foreach ($orders as $p) {
                    if (0 != preg_match('~^' . implode('', $p) . '\]~i', mb_substr($message, $pos1 - 1), $matches)) {
                        $match = true;
        ...
        if (false !== $smileys && null === $tag && isset($itemcodes[mb_substr($message, $pos + 1, 1)]) && ']' == mb_substr($message, $pos + 2, 1) && !isset($disabled['list']) && !isset($disabled['li'])) {
            if ('0' == mb_substr($message, $pos + 1, 1) && !in_array(mb_substr($message, $pos - 1, 1), [';', ' ', "\t", '>'], true)) {
                continue;
            }

            $tag = $itemcodes[mb_substr($message, $pos + 1, 1)];

            // First let's set up the tree: it needs to be in a list, or after an li.

            if (null === $inside || ('list' != $inside['tag'] && 'li' != $inside['tag'])) {
                $open_tags[] = [
                    'tag' => 'list',
                    'after' => '</ul>',
                    'block_level' => true,
                    'require_children' => ['li'],
                    'disallow_children' => $inside['disallow_children'] ?? null,
                ];

                $code = '<ul style="margin-top: 0; margin-bottom: 0;">';
            } // We're in a list item already: another itemcode?  Close it first.

            elseif ('li' == $inside['tag']) {
                array_pop($open_tags);
            ...
            $message = mb_substr($message, 0, $pos) . $code . mb_substr($message, $pos + 3);

            $pos += mb_strlen($code) - 1;

            // Next, find the next break (if any.)  If there's more itemcode after it, keep it going - otherwise close!

            $pos2 = mb_strpos($message, '<br>', $pos);

            $pos3 = mb_strpos($message, '[/', $pos);

            if (false !== $pos2 && ($pos2 <= $pos3 || false === $pos3)) {
                preg_match('~^(<br>|&nbsp;|\s|\[)+~', mb_substr($message, $pos2 + 6), $matches);

                $message = mb_substr($message, 0, $pos2) . (!empty($matches[0]) && '[' == mb_substr($matches[0], -1) ? '[/li]' : '[/li][/list]') . mb_substr($message, $pos2);
        ...
        if (null === $tag && null !== $inside && !empty($inside['require_children'])) {
            array_pop($open_tags);

            $message = mb_substr($message, 0, $pos) . $inside['after'] . mb_substr($message, $pos);

            $pos += mb_strlen($inside['after']) - 1;
        }
        ...
        if (isset($disabled[$tag['tag']])) {
            if (!isset($tag['disabled_before']) && !isset($tag['disabled_after']) && !isset($tag['disabled_content'])) {
                ...
                $tag['content'] = isset($tag['type']) && 'closed' == $tag['type'] ? '' : (!empty($tag['block_level']) ? '<div>$1</div>' : '$1');
        ...
        if (!empty($tag['block_level']) && 'html' != $tag['tag'] && empty($inside['block_level'])) {
            $n = count($open_tags) - 1;
            ...
            for ($i = count($open_tags) - 1; $i > $n; $i--) {
                $message = mb_substr($message, 0, $pos) . $open_tags[$i]['after'] . mb_substr($message, $pos);

                $pos += mb_strlen($open_tags[$i]['after']);

                $pos1 += mb_strlen($open_tags[$i]['after']);

                // Trim or eat trailing stuff... see comment at the end of the big loop.

                if (!empty($open_tags[$i]['block_level']) && '<br>' == mb_substr($message, $pos, 6)) {
                    $message = mb_substr($message, 0, $pos) . mb_substr($message, $pos + 6);
                }

                if (!empty($open_tags[$i]['trim']) && 'inside' != $tag['trim'] && 0 != preg_match('~(<br>|&nbsp;|\s)*~', mb_substr($message, $pos), $matches)) {
                    $message = mb_substr($message, 0, $pos) . mb_substr($message, $pos + mb_strlen($matches[0]));
                }
        ...
        if (!isset($tag['type'])) {
            ...
            $message = mb_substr($message, 0, $pos) . $tag['before'] . mb_substr($message, $pos1);

            $pos += mb_strlen($tag['before']) - 1;
        } // Don't parse the content, just skip it.

        elseif ('unparsed_content' == $tag['type']) {
            $pos2 = mb_stripos($message, '[/' . mb_substr($message, $pos + 1, mb_strlen($tag['tag'])) . ']', $pos1);

            if (false === $pos2) {
                continue;
            }

            $data = mb_substr($message, $pos1, $pos2 - $pos1);

            if (!empty($tag['block_level']) && '<br>' == mb_substr($data, 0, 6)) {
                $data = mb_substr($data, 6);
            }
            ...
            $message = mb_substr($message, 0, $pos) . $code . mb_substr($message, $pos2 + 3 + mb_strlen($tag['tag']));

            $pos += mb_strlen($code) - 1;
        } // Don't parse the content, just skip it.

        elseif ('unparsed_equals_content' == $tag['type']) {
            // The value may be quoted for some tags - check.

            if (isset($tag['quoted'])) {
                $quoted = '&quot;' == mb_substr($message, $pos1, 6);

                if ('optional' != $tag['quoted'] && !$quoted) {
                    continue;
                }
            ...
            $pos2 = mb_strpos($message, false === $quoted ? ']' : '&quot;]', $pos1);

            if (false === $pos2) {
                continue;
            }

            $pos3 = mb_stripos($message, '[/' . mb_substr($message, $pos + 1, mb_strlen($tag['tag'])) . ']', $pos2);
            ...
            $data = [
                mb_substr($message, $pos2 + (false === $quoted ? 1 : 7), $pos3 - ($pos2 + (false === $quoted ? 1 : 7))),
                mb_substr($message, $pos1, $pos2 - $pos1),
            ];

            if (!empty($tag['block_level']) && '<br>' == mb_substr($data[0], 0, 6)) {
                $data[0] = mb_substr($data[0], 6);
            }
            ...
            $message = mb_substr($message, 0, $pos) . $code . mb_substr($message, $pos3 + 3 + mb_strlen($tag['tag']));

            $pos += mb_strlen($code) - 1;
        } // A closed tag, with no content or value.

        elseif ('closed' == $tag['type']) {
            $pos2 = mb_strpos($message, ']', $pos);

            $message = mb_substr($message, 0, $pos) . $tag['content'] . mb_substr($message, $pos2 + 1);

            $pos += mb_strlen($tag['content']) - 1;
        } // This one is sorta ugly... :/.  Unforunately, it's needed for flash.

        elseif ('unparsed_commas_content' == $tag['type']) {
            $pos2 = mb_strpos($message, ']', $pos1);
            ...
            $data = explode(',', ',' . mb_substr($message, $pos1, $pos2 - $pos1));

            $data[0] = mb_substr($message, $pos2 + 1, $pos3 - $pos2 - 1);
        ...
        elseif ('unparsed_commas' == $tag['type']) {
            $pos2 = mb_strpos($message, ']', $pos1);
            ...
            $message = mb_substr($message, 0, $pos) . $code . mb_substr($message, $pos2 + 1);

            $pos += mb_strlen($code) - 1;
        } // A tag set to a value, parsed or not.

        elseif ('unparsed_equals' == $tag['type'] || 'parsed_equals' == $tag['type']) {
            // The value may be quoted for some tags - check.
            ...
            $message = mb_substr($message, 0, $pos) . $code . mb_substr($message, $pos2 + (false === $quoted ? 1 : 7));
        ...
        if (!empty($tag['block_level']) && '<br>' == mb_substr($message, $pos + 1, 6)) {
            $message = mb_substr($message, 0, $pos + 1) . mb_substr($message, $pos + 7);
        }

        // Are we trimming outside this tag?

        if (!empty($tag['trim']) && 'outside' != $tag['trim'] && 0 != preg_match('~(<br>|&nbsp;|\s)*~', mb_substr($message, $pos + 1), $matches)) {
            $message = mb_substr($message, 0, $pos + 1) . mb_substr($message, $pos + 1 + mb_strlen($matches[0]));
        }
    }
...
function parsesmileys(&$message)
{
    ...
    if (empty($smileyfromcache)) {
        ...
        if (empty($modSettings['smiley_enable'])) {
            ...
            if (null === ($temp = cache_get_data('parsing_smileys', 480))) {
                $result = db_query(
                    "
                ...
                [$smileysfrom, $smileysto, $smileysdescs] = $temp;
            }
        }
        ...
        for ($i = 0, $n = count($smileysfrom); $i < $n; $i++) {
            ...
            $smileytocache[] = '<img src="' . $modSettings['smileys_url'] . '/' . $user_info['smiley_set'] . '/' . $smileysto[$i] . '" alt="' . strtr(htmlspecialchars($smileysdescs[$i], ENT_QUOTES | ENT_HTML5), [':' => '&#58;', '(' => '&#40;', ')' => '&#41;', '$' => '&#36;', '[' => '&#091;']) . '" border="0">';
        }
    }
...
function highlight_php_code($code)
{
    ...
    if (-1 == @version_compare(PHP_VERSION, '4.2.0')) {
        ob_start();
...
function writeLog($force = false)
{
    ...
    if (!empty($modSettings['who_enabled'])) {
        $serialized = $_GET + ['USER_AGENT' => $_SERVER['HTTP_USER_AGENT']];
    ...
    if (empty($_SESSION['log_time'])) {
        if ($do_delete || !empty($ID_MEMBER)) {
            db_query(
                "
				DELETE FROM {$db_prefix}log_online
				WHERE " . ($do_delete ? 'logTime < NOW() - INTERVAL ' . ($modSettings['lastActive'] * 60) . ' SECOND' : '') . ($do_delete && !empty($ID_MEMBER) ? ' OR ' : '') . (empty($ID_MEMBER) ? '' : "ID_MEMBER = $ID_MEMBER"),
                __FILE__,
                __LINE__
            );
        }
        ...
        db_query(
			...
			VALUES ('$session_id', $ID_MEMBER, NOW(), IFNULL(INET_ATON('$user_info[ip]'), 0), '$serialized')",
            __FILE__,
            __LINE__
        );
    }
    ...
    if (SMF != 'SSI' && !empty($user_info['last_login']) && $user_info['last_login'] < time() - 60) {
        ...
        updateMemberData($ID_MEMBER, ['lastLogin' => time(), 'memberIP' => '\'' . $user_info['ip'] . '\'', 'memberIP2' => '\'' . $_SERVER['BAN_CHECK_IP'] . '\'', 'totalTimeLoggedIn' => $user_settings['totalTimeLoggedIn']]);
...
function redirectexit($setLocation = '', $refresh = false)
{
    ...
    $add = 0 == preg_match('~^(ftp|http)[s]?://~', $setLocation) && 'about:' != mb_substr($setLocation, 0, 6);
    ...
    if (WIRELESS) {
        ...
        $char = false === mb_strpos($setLocation, '?') ? '?' : ';';

        if (mb_strpos($setLocation, '#') == !false) {
            $setLocation = strtr($setLocation, ['#' => $char . WIRELESS_PROTOCOL . '#']);
        } else {
    ...
    if (!empty($modSettings['queryless_urls']) && (empty($context['server']['is_cgi']) || 1 == @ini_get('cgi.fix_pathinfo')) && !empty($context['server']['is_apache'])) {
        if (defined('SID') && SID != '') {
    ...
    if ($refresh) {
        header('Refresh: 0; URL=' . strtr($setLocation, [' ' => '%20', ';' => '%3b']));
    } else {
...
function obExit($header = null, $do_footer = null, $from_index = false)
{
    ...
    if ($do_header) {
        ...
        if ((isset($_REQUEST['debug']) || isset($_REQUEST['xml']) || (WIRELESS && WIRELESS_PROTOCOL == 'wap')) && in_array($txt['lang_locale'], ['UTF-8', 'ISO-8859-1'], true)) {
    ...
    if ($do_footer) {
        ...
        loadSubTemplate($context['sub_template'] ?? 'main');
    ...
    if (false === mb_strpos($_SERVER['REQUEST_URL'], 'action=dlattach')) {
    ...
    if (isset($modSettings['integrate_exit'], $context['template_layers']) && in_array('main', $context['template_layers'], true) && function_exists($modSettings['integrate_exit'])) {
...
function adminIndex($area)
{
    ...
    if (allowedTo(['edit_news', 'send_mail', 'admin_forum'])) {
    ...
    if (allowedTo(['manage_boards', 'admin_forum', 'manage_smileys', 'manage_attachments', 'moderate_forum'])) {
        $context['admin_areas']['layout'] = [
        ...
        if (allowedTo(['admin_forum', 'moderate_forum'])) {
    ...
    if (allowedTo(['moderate_forum', 'manage_membergroups', 'manage_bans', 'manage_permissions', 'admin_forum'])) {
        $context['admin_areas']['members'] = [
            'title' => $txt[426],
            'areas' => [],
        ];
    ...
    if (allowedTo('admin_forum')) {
        $context['admin_areas']['maintenance'] = [
            'title' => $txt[501],
            'areas' => [
    ...
    if (!empty($modSettings['modlog_enabled'])) {
        db_query(
			...
			VALUES (" . time() . ", $ID_MEMBER, SUBSTRING('$user_info[ip]', 1, 16), SUBSTRING('$action', 1, 30),
				SUBSTRING('" . addslashes(serialize($extra)) . "', 1, 65534))",
            __FILE__,
            __LINE__
        );
...
function trackStats($stats = [])
{
    ...
    db_query(
		...
		SET" . mb_substr($setStringUpdate, 0, -1) . "
		WHERE date = '$date'
		LIMIT 1",
        __FILE__,
        __LINE__
    ...
    if (0 == db_affected_rows()) {
        db_query(
			...
			VALUES ('$date', " . implode(', ', $cache_stats) . ')',
            __FILE__,
            __LINE__
        );
    }
...
function spamProtection($error_type)
{
    ...
    if ('spam' == $error_type && !allowedTo('moderate_board')) {
        db_query(
            "
			DELETE FROM {$db_prefix}log_floodcontrol
			WHERE logTime < " . (time() - $modSettings['spamWaitTime']),
            __FILE__,
            __LINE__
        );
    } else {
        db_query(
			...
			WHERE (logTime < " . (time() - 2) . " AND ip = '$user_info[ip]')
				OR logTime < " . (time() - $modSettings['spamWaitTime']),
            __FILE__,
            __LINE__
        );
    }
    ...
    db_query(
		...
		REPLACE INTO {$db_prefix}log_floodcontrol
			(ip, logTime)
		VALUES (SUBSTRING('$user_info[ip]', 1, 16), " . time() . ')',
        __FILE__,
        __LINE__
    );
    ...
    if (1 != db_affected_rows()) {
        ...
        fatal_lang_error($error_type . 'WaitTime_broken', false, [$modSettings['spamWaitTime']]);
...
function url_image_size($url)
{
    ...
    if (null !== ($temp = cache_get_data('url_image_size-' . md5($url), 240))) {
        return $temp;
    }
    ...
    if ('' == $url || 'http://' == $url || 'https://' == $url) {
        ...
        if (false !== $fp) {
            ...
            if ($test < 4) {
                ...
                if (false === $size && function_exists('imagecreatefromstring')) {
...
function setupThemeContext()
{
    ...
    for ($i = 0, $n = count($context['news_lines']); $i < $n; $i++) {
        ...
        $context['fader_news_lines'][$i] = strtr(addslashes($context['news_lines'][$i]), ['/' => '\/', '<a href=' => '<a hre" + "f=']);
    }
    ...
    if (!$user_info['is_guest']) {
        ...
        if ($user_info['unread_messages'] > ($_SESSION['unread_messages'] ?? 0)) {
        ...
        if (allowedTo('moderate_forum')) {
            $context['unapproved_members'] = !empty($modSettings['registration_method']) && 2 == $modSettings['registration_method'] ? $modSettings['unapprovedMembers'] : 0;
        }
        ...
        elseif ('http://' == mb_substr($user_info['avatar']['url'], 0, 7)) {
            $context['user']['avatar']['href'] = $user_info['avatar']['url'];

            if ('option_html_resize' == $modSettings['avatar_action_too_large'] || 'option_js_resize' == $modSettings['avatar_action_too_large']) {
        ...
        elseif ('' != $user_info['avatar']['url']) {
            $context['user']['avatar']['href'] = $modSettings['avatar_url'] . '/' . htmlspecialchars($user_info['avatar']['url'], ENT_QUOTES | ENT_HTML5);
        }

        if (!empty($context['user']['avatar'])) {
            $context['user']['avatar']['image'] = '<img src="'
                                                  . $context['user']['avatar']['href']
                                                  . '"'
                                                  . (isset($context['user']['avatar']['width']) ? ' width="' . $context['user']['avatar']['width'] . '"' : '')
                                                  . (isset($context['user']['avatar']['height']) ? ' height="'
        ...
        if (!empty($modSettings['registration_method']) && 1 == $modSettings['registration_method']) {
            $txt['welcome_guest'] .= $txt['welcome_guest_activate'];
        }

        // If we've upgraded recently, go easy on the passwords.

        if (!empty($modSettings['disableHashTime']) && (1 == $modSettings['disableHashTime'] || time() < $modSettings['disableHashTime'])) {
    ...
    $context['allow_edit_profile'] = !$user_info['is_guest'] && allowedTo(['profile_view_own', 'profile_view_any', 'profile_identity_own', 'profile_identity_any', 'profile_extra_own', 'profile_extra_any', 'profile_remove_own', 'profile_remove_any', 'moderate_forum', 'manage_membergroups']);
    ...
    $context['current_action'] = $_GET['action'] ?? '';
    ...
    if ($context['user']['popup_messages'] && !empty($options['popup_messages']) && (!isset($_REQUEST['action']) || 'pm' != $_REQUEST['action'])) {
    ...
    if ('option_js_resize' == $modSettings['avatar_action_too_large'] && (!empty($modSettings['avatar_max_width_external']) || !empty($modSettings['avatar_max_height_external']))) {
        $context['html_headers'] .= '
	<script language="JavaScript" type="text/javascript"><!-- // --><![CDATA[
...
function template_header()
{
    ...
    foreach ($context['template_layers'] as $layer) {
        ...
        if ('main' == $layer && allowedTo('admin_forum') && !$user_info['is_guest']) {
            $securityFiles = ['install.php', 'webinstall.php', 'upgrade.php', 'convert.php', 'repair_paths.php', 'repair_settings.php'];
        ...
        elseif ('main' == $layer && isset($_SESSION['ban']['cannot_post'])) {
            echo '
...
function theme_copyright($get_it = false)
{
    ...
    if (!isset($forum_version) || false === mb_strpos($forum_version, 'SMF') || isset($_GET['checkcopyright'])) {
        $data = mb_substr(file_get_contents(__FILE__), 0, 4096);

        if (0 == preg_match('~\*\s*Software\s+Version:\s+(SMF\s+.+?)[\s]{2}~i', $data, $match)) {
            $match = ['', 'SMF'];
        }
    ...
    $forum_copyright = str_replace(['Lewis Media', 'href="http://www.lewismedia.com/"', '2001-'], ['Simple Machines LLC', 'href="http://www.simplemachines.org/about/copyright.php" title="Free Forum Software"', ''], $forum_copyright);
    ...
    elseif (isset($modSettings['copyright_key']) && '1d01885ece7a9355bdeb22ed107f0ffa8c323026' == sha1($modSettings['copyright_key'] . 'banjo')) {
        $found = true;

        return;
    } elseif ((false !== mb_strpos($forum_copyright, '<a href="http://www.simplemachines.org/" title="Simple Machines Forum" target="_blank">Powered by SMF')
               || false !== mb_strpos(
                   $forum_copyright,
                   '<a href="http://www.simplemachines.org/" onclick="this.href += \'referer.php?forum=' . urlencode(
                          $context['forum_name'] . '|' . $boardurl . '|' . $forum_version
                      ) . '\';" target="_blank">SMF'
               )
               || false !== mb_strpos($forum_copyright, '<a href="http://www.simplemachines.org/" target="_blank">SMF')
               || false !== mb_strpos($forum_copyright, '<a href="http://www.simplemachines.org/" title="Simple Machines Forum" target="_blank">SMF'))
              && ((false !== mb_strpos($forum_copyright, '<a href="http://www.simplemachines.org/about/copyright.php" title="Free Forum Software" target="_blank">SMF &copy;') && (false !== mb_strpos($forum_copyright, 'Lewis Media</a>') || false !== mb_strpos($forum_copyright, 'Simple Machines LLC</a>')))
                  || false !== mb_strpos($forum_copyright, '<a href="http://www.lewismedia.com/">Lewis Media</a>')
                  || false !== mb_strpos($forum_copyright, '<a href="http://www.lewismedia.com/" target="_blank">Lewis Media</a>')
                  || (false !== mb_strpos($forum_copyright, '<a href="http://www.simplemachines.org/about/copyright.php"') && false !== mb_strpos($forum_copyright, 'Simple Machines LLC')))) {
        $found = true;
...
function template_footer()
{
    ...
    if ($modSettings['ob_googlebot_stats']) {
        $ob_googlebot_start = $context['start'] ?? '0';
        ...
        if (0 == $GLOBALS['xoopsDB']->getRowsNum($result)) {    //No stats yet.
            $lastvisit = $frequency = $visits = 0;
        } else {
            [$lastvisit, $frequency, $visits] = $GLOBALS['xoopsDB']->fetchRow($result);
        }
        ...
        if (ob_googlebot_getAgent($_SERVER['HTTP_USER_AGENT'], $spider_name, $agent)) {
            if ('Googlebot' == $agent) {
                ...
                if (1 == $visits) {    // First time, insert new record
                    db_query(
                        "
                    ...
                    }    /* This is the SECOND time Google visits this page */ else {
        ...
        if ($visits > 0) {
            $context['ob_googlebot_stats'] = [
                'Googlebot' => ['frequency' => $frequency, 'visits' => $visits, 'lastvisit' => $lastvisit],
            ];
        }
    }
...
function db_debug_junk()
{
    ...
    if (!isset($db_show_debug) || true !== $db_show_debug || (isset($_GET['action']) && 'viewquery' == $_GET['action']) || WIRELESS) {
        return;
    }
    ...
    for ($i = 0, $n = count($files); $i < $n; $i++) {
        ...
        $files[$i] = strtr($files[$i], [$boarddir => '.']);
    }
...
<div class="smalltext" style="text-align: left; margin: 1ex;">
	...
	Files included: ', count($files), ' - ', round(
        $total_size / 1024
    ), 'KB. (<a href="javascript:void(0);" onclick="document.getElementById(\'debug_include_info\').style.display = \'inline\'; this.style.display = \'none\'; return false;">show</a><span id="debug_include_info" style="display: none;"><i>', implode('</i>, <i>', $files), '</i></span>)<br>';
    ...
    if (!empty($modSettings['cache_enable']) && !empty($cache_hits)) {
        ...
        echo '
	Cache hits: ', $cache_count, ': ', comma_format($total_t, 5), 's for ', comma_format(
            $total_s
        ), ' bytes (<a href="javascript:void(0);" onclick="document.getElementById(\'debug_cache_info\').style.display = \'inline\'; this.style.display = \'none\'; return false;">show</a><span id="debug_cache_info" style="display: none;"><i>', implode('</i>, <i>', $entries), '</i></span>)<br>';
    }

    echo '
	<a href="', $scripturl, '?action=viewquery" target="_blank">Queries used: ', $db_count, 0 == $warnings ? '' : ', ' . $warnings . ' warning(s)', '</a>.<br>
	<br>';

    if (1 == $_SESSION['view_queries']) {
        foreach ($db_cache as $q => $qq) {
            $is_select = 'SELECT' == mb_substr(trim($qq['q']), 0, 6) || 0 != preg_match('~^INSERT(?: IGNORE)? INTO \w+(?:\s+\([^)]+\))?\s+SELECT .+$~s', trim($qq['q']));

            echo '
	<b>', $is_select ? '<a href="' . $scripturl . '?action=viewquery;qq=' . ($q + 1) . '#qq' . $q . '" target="_blank" style="text-decoration: none;">' : '', nl2br(str_replace("\t", '&nbsp;&nbsp;&nbsp;', htmlspecialchars(ltrim($qq['q'], "\n\r"), ENT_QUOTES | ENT_HTML5))) . ($is_select ? '</a></b>' : '</b>') . '<br>
	&nbsp;&nbsp;&nbsp;';
...
function getAttachmentFilename($filename, $attachment_id, $new = false)
{
    ...
    if (false === $attachment_id || ($new && empty($modSettings['attachmentEncryptFilenames']))) {
        return $clean_name;
    } elseif ($new) {
        return $enc_name;
    }
...
function host_from_ip($ip)
{
    ...
    if (null !== ($host = cache_get_data('hostlookup-' . $ip, 600))) {
        return $host;
    }
    ...
    if (!isset($host) && (false === mb_strpos(mb_strtolower(PHP_OS), 'win') || false !== mb_strpos(mb_strtolower(PHP_OS), 'darwin')) && 1 == mt_rand(0, 1)) {
        ...
        if (false !== mb_strpos($test, 'not found')) {
            $host = '';
        } // Invalid server option?

        elseif (mb_strpos($test, 'invalid option') && !isset($modSettings['host_to_dis'])) {
            updateSettings(['host_to_dis' => 1]);
        } // Maybe it found something, after all?

        elseif (1 == preg_match('~\s([^\s]+?)\.\s~', $test, $match)) {
            $host = $match[1];
        }
    }
    ...
    if (!isset($host) && false !== mb_strpos(mb_strtolower(PHP_OS), 'win') && false === mb_strpos(mb_strtolower(PHP_OS), 'darwin') && 1 == mt_rand(0, 1)) {
        ...
        if (false !== mb_strpos($test, 'Non-existent domain')) {
            $host = '';
        } elseif (1 == preg_match('~Name:\s+([^\s]+)~', $test, $match)) {
            $host = $match[1];
        }
    }
...
function text2words($text, $max_chars = 20, $encrypt = false)
{
    ...
    if ($encrypt) {
        ...
        foreach ($words as $word) {
            if ('' !== ($word = trim($word, '-_\''))) {
                $encrypted = mb_substr(crypt($word, 'uk'), 2, $max_chars);
                ...
                $returned_ints[] = 4 == $max_chars ? min($total, 16777215) : $total;
            }
        }
    ...
    foreach ($words as $word) {
        if ('' !== ($word = trim($word, '-_\''))) {
            $returned_words[] = addslashes(null === $max_chars ? $word : mb_substr($word, 0, $max_chars));
        }
    }
...
function ob_googlebot_getAgent($user_agent, &$spider_name, &$result)
{
    ...
    foreach ($known_spiders as $poss) {
        if (false !== mb_strpos(mb_strtolower($user_agent), mb_strtolower($poss['agent']))) {
            $spider_name = $poss['spidername'] ?? $poss['agent'];
