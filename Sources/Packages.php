...
function PackageInstallTest()
{
    ...
    if (!mktree($boarddir . '/Packages/temp', 0755)) {
        ...
        if (!mktree($boarddir . '/Packages/temp', 0777)) {
            ...
            packageRequireFTP($scripturl . '?action=packages;sa=' . $_REQUEST['sa'] . ';package=' . $_REQUEST['package'], [$boarddir . '/Packages/temp/delme.tmp']);
    ...
    $context['uninstalling'] = 'uninstall' == $_REQUEST['sa'];
    ...
    if (is_file($boarddir . '/Packages/' . $context['filename'])) {
        ...
        if ($context['extracted_files'] && !file_exists($boarddir . '/Packages/temp/package-info.xml')) {
            foreach ($context['extracted_files'] as $file) {
                if ('package-info.xml' == basename($file['filename'])) {
    ...
    $context['package_name'] = $packageInfo['name'] ?? $context['filename'];
    ...
    foreach ($actions as $action) {
        ...
        elseif ('redirect' == $action['type']) {
            continue;
        } elseif ('error' == $action['type']) {
            $context['has_failure'] = true;
        } elseif ('modification' == $action['type']) {
            if (!file_exists($boarddir . '/Packages/temp/' . $context['base_path'] . $action['filename'])) {
                $context['has_failure'] = true;

                $context['actions'][] = [
                    'type' => $txt['package56'],
                    'action' => strtr($action['filename'], [$boarddir => '.']),
            ...
            foreach ($mod_actions as $mod_action) {
                if ('opened' == $mod_action['type']) {
                    $failed = false;
                } elseif ('failure' == $mod_action['type']) {
                    $context['has_failure'] = true;

                    $failed = true;
                } elseif ('chmod' == $mod_action['type']) {
                    $context['ftp_needed'] = true;

                    $chmod_files[] = $mod_action['filename'];
                } elseif ('saved' == $mod_action['type']) {
                    $context['actions'][] = [
                        'type' => $txt['package56'],
                        'action' => strtr($mod_action['filename'], [$boarddir => '.']),
                        'description' => $failed ? $txt['package_action_failure'] : $txt['package_action_success'],
                    ];
                } elseif ('skipping' == $mod_action['type']) {
                    $context['actions'][] = [
                        'type' => $txt['package56'],
                        'action' => strtr($mod_action['filename'], [$boarddir => '.']),
                        'description' => $txt['package_action_skipping'],
                    ];
                } elseif ('missing' == $mod_action['type']) {
                    $context['has_failure'] = true;
            ...
            $thisAction = [];
        } elseif ('code' == $action['type']) {
            $thisAction = [
                'type' => $txt['package57'],
                'action' => $action['filename'],
            ];
        } elseif (in_array($action['type'], ['create-dir', 'create-file'], true)) {
            $thisAction = [
                'type' => $txt['package50'] . ' ' . ('create-dir' == $action['type'] ? $txt['package55'] : $txt['package54']),
                'action' => strtr($action['destination'], [$boarddir => '.']),
            ];
        } elseif (in_array($action['type'], ['require-dir', 'require-file'], true)) {
            $thisAction = [
                'type' => $txt['package53'] . ' ' . ('require-dir' == $action['type'] ? $txt['package55'] : $txt['package54']),
                'action' => strtr($action['destination'], [$boarddir => '.']),
            ];
        } elseif (in_array($action['type'], ['move-dir', 'move-file'], true)) {
            $thisAction = [
                'type' => $txt['package51'] . ' ' . ('move-dir' == $action['type'] ? $txt['package55'] : $txt['package54']),
                'action' => strtr($action['source'], [$boarddir => '.']) . ' => ' . strtr($action['destination'], [$boarddir => '.']),
            ];
        } elseif (in_array($action['type'], ['remove-dir', 'remove-file'], true)) {
            $thisAction = [
                'type' => $txt['package52'] . ' ' . ('remove-dir' == $action['type'] ? $txt['package55'] : $txt['package54']),
                'action' => strtr($action['filename'], [$boarddir => '.']),
            ];
        }
...
function PackageInstall()
{
    ...
    $context['uninstalling'] = 'uninstall2' == $_REQUEST['sa'];
    ...
    if (!empty($modSettings['package_make_backups']) && (!isset($_SESSION['last_backup_for']) || $context['filename'] . ($context['uninstalling'] ? '$$' : '$') != $_SESSION['last_backup_for'])) {
    ...
    if (!empty($install_log)) {
        foreach ($install_log as $action) {
            if ('modification' == $action['type'] && !empty($action['filename'])) {
                if ($action['boardmod']) {
            ...
            } elseif ('code' == $action['type'] && !empty($action['filename'])) {
            ...
            elseif ('redirect' == $action['type'] && !empty($action['redirect_url'])) {
...
function ExamineFile()
{
    ...
    if (in_array(mb_strtolower(mb_strrchr($_REQUEST['file'], '.')), ['.bmp', '.gif', '.jpeg', '.jpg', '.png'], true)) {
        ...
        if (is_file($boarddir . '/Packages/' . $_REQUEST['package'])) {
            $context['filedata'] = htmlspecialchars(read_tgz_file($boarddir . '/Packages/' . $_REQUEST['package'], $_REQUEST['file'], true), ENT_QUOTES | ENT_HTML5);
        } elseif (is_dir($boarddir . '/Packages/' . $_REQUEST['package'])) {
            $context['filedata'] = htmlspecialchars(file_get_contents($boarddir . '/Packages/' . $_REQUEST['package'] . '/' . $_REQUEST['file']), ENT_QUOTES | ENT_HTML5);
        }

        if ('.php' == mb_strtolower(mb_strrchr($_REQUEST['file'], '.'))) {
...
function PackageRemove()
{
    ...
    if (!isset($_GET['package']) || 'index.php' == $_GET['package'] || 'installed.list' == $_GET['package']) {
    ...
    if (file_exists($boarddir . '/Packages/' . $_GET['package'])) {
        packageRequireFTP($scripturl . '?action=packages;sa=remove;package=' . $_GET['package'], [$boarddir . '/Packages/' . $_GET['package']]);
...
function PackageBrowse()
{
    ...
    if (isset($_GET['version_emulate'])) {
        if (0 == $_GET['version_emulate'] && isset($_SESSION['version_emulate'])) {
            unset($_SESSION['version_emulate']);
        } elseif (0 != $_GET['version_emulate']) {
    ...
    $context['available_mods'] = [];

    $context['available_avatars'] = [];

    $context['available_languages'] = [];

    $context['available_other'] = [];

    $context['available_all'] = [];

    if ($dir = @opendir($boarddir . '/Packages')) {
        $dirs = [];

        while ($package = readdir($dir)) {
            if ('.' == $package || '..' == $package || 'temp' == $package
                || (!(is_dir($boarddir . '/Packages/' . $package) && file_exists($boarddir . '/Packages/' . $package . '/package-info.xml')) && '.tar.gz' != mb_substr($package, -7) && '.tgz' != mb_substr($package, -4)
                    && '.zip' != mb_substr($package, -4))) {
                continue;
            }
            ...
            if (is_dir($boarddir . '/Packages/' . $package)) {
                ...
                $dirs[] = $package;
            } elseif ('.tar.gz' == mb_substr($package, -7)) {
                if (in_array(mb_substr($package, 0, -7), $dirs, true)) {
                    continue;
                }

                $dirs[] = mb_substr($package, 0, -7);
            } elseif ('.zip' == mb_substr($package, -4) || '.tgz' == mb_substr($package, -4)) {
                if (in_array(mb_substr($package, 0, -4), $dirs, true)) {
                    continue;
                }

                $dirs[] = mb_substr($package, 0, -4);
            }
            ...
            if ('modification' == $packageInfo['type'] || 'mod' == $packageInfo['type']) {
                $context['available_mods'][] = $packageInfo;
            } // Avatar package.

            elseif ('avatar' == $packageInfo['type']) {
                $context['available_avatars'][] = $packageInfo;
            } // Language package.

            elseif ('language' == $packageInfo['type']) {
