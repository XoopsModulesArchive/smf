...
/*	The purpose of this file is... errors. (hard to guess, huh?)  It takes
    ...
    void errorHandler(int error_level, string error_string, string filename,
            int line)
...
function log_error($error_message, $file = null, $line = null)
{
    ...
    $error_message = strtr($error_message, ['&lt;br /&gt;' => '<br>', '&lt;b&gt;' => '<b>', '&lt;/b&gt;' => '</b>', "\n" => '<br>']);
    ...
    $query_string = empty($_SERVER['QUERY_STRING']) ? '' : addslashes(htmlspecialchars('?' . preg_replace(['~;sesc=[^&;]+~', '~' . session_name() . '=' . session_id() . '[&;]~'], [';sesc', ''], $_SERVER['QUERY_STRING']), ENT_QUOTES | ENT_HTML5));
    ...
    db_query(
		...
		VALUES ($ID_MEMBER, " . time() . ", SUBSTRING('$user_info[ip]', 1, 16), SUBSTRING('$query_string', 1, 65534), SUBSTRING('" . addslashes($error_message) . "', 1, 65534), '$sc')",
        false,
        false
    ) || die($error_message);
...
function db_error($db_string, $file, $line)
{
    ...
    $query_error = $GLOBALS['xoopsDB']->error($db_connection);

    $query_errno = $GLOBALS['xoopsDB']->errno($db_connection);
    ...
    if (1213 != $query_errno && 1205 != $query_errno) {
        log_error($txt[1001] . ': ' . $query_error, $file, $line);
    }

    // Database error auto fixing ;).

    if (!isset($modSettings['autoFixDatabase']) || '1' == $modSettings['autoFixDatabase']) {
        ...
        if (null !== ($temp = cache_get_data('db_last_error', 600))) {
        ...
        if (@$db_last_error < time() - 3600 * 24 * 3) {
            ...
            if (1030 == $query_errno && false !== mb_strpos($query_error, ' 127 ')) {
            ...
            elseif (1016 == $query_errno) {
                if (0 != preg_match('~\'([^\.\']+)~', $query_error, $match)) {
                    $fix_tables = ['`' . $match[1] . '`'];
                }
            } // Indexes crashed.  Should be easy to fix!

            elseif (1034 == $query_errno || 1035 == $query_errno) {
        ...
        if (!empty($fix_tables)) {
            ...
            if (null === ($temp = cache_get_data('db_last_error', 600))) {
                updateSettingsFile(['db_last_error' => time()]);
            }
        ...
        if (in_array($query_errno, [1205, 1213, 2006, 2013], true)) {
            ...
            if ($db_connection) {
                ...
                for ($n = 0; $n < 4; $n++) {
                    ...
                    $new_errno = $GLOBALS['xoopsDB']->errno($db_connection);

                    if (false !== $ret || in_array($new_errno, [1205, 1213], true)) {
                        break;
                    }
                }
        ...
        elseif (1030 == $query_errno && (false !== mb_strpos($query_error, ' -1 ') || false !== mb_strpos($query_error, ' 28 ') || false !== mb_strpos($query_error, ' 12 '))) {
            if (!isset($txt)) {
    ...
    if (allowedTo('admin_forum') && !empty($forum_version) && $forum_version != 'SMF ' . @$modSettings['smfVersion'] && false === mb_strpos($forum_version, 'Demo') && false === mb_strpos($forum_version, 'CVS')) {
        $context['error_message'] .= '<br><br>' . $txt['database_error_versions'];
    }

    if (allowedTo('admin_forum') && isset($db_show_debug) && true === $db_show_debug) {
...
function fatal_error($error, $log = true)
{
    ...
    if (!isset($context['error_title'])) {
        ...
        $context['error_message'] = $log || (!empty($modSettings['enableErrorLogging']) && 2 == $modSettings['enableErrorLogging']) ? log_error($error) : $error;
    }
...
function fatal_lang_error($error, $log = true, $sprintf = [])
{
    global $txt;
...
function errorHandler($error_level, $error_string, $file, $line)
{
    global $settings, $modSettings, $db_show_debug;

    // Ignore errors if we're ignoring them or they are strict notices from PHP 5 (which cannot be solved without breaking PHP 4.)

    if (0 == error_reporting() || (defined('E_STRICT') && E_STRICT == $error_level && (empty($modSettings['enableErrorLogging']) || 2 != $modSettings['enableErrorLogging']))) {
        return;
    }

    if (false !== mb_strpos($file, 'eval()') && !empty($settings['current_include_filename'])) {
        if (function_exists('debug_backtrace')) {
            $array = debug_backtrace();

            for ($i = 0, $iMax = count($array); $i < $iMax; $i++) {
                if ('loadSubTemplate' != $array[$i]['function']) {
                    continue;
                }
    ...
    if (isset($db_show_debug) && true === $db_show_debug) {
        ...
        if (E_ERROR != $error_level % 255) {
            $temporary = ob_get_contents();

            if ('="' == mb_substr($temporary, -2)) {
                echo '"';
            }
        }
        ...
        echo '<br>
<b>', E_ERROR == $error_level % 255 ? 'Error' : (E_WARNING == $error_level % 255 ? 'Warning' : 'Notice'), '</b>: ', $error_string, ' in <b>', $file, '</b> on line <b>', $line, '</b><br>';
    }
    ...
    if (E_ERROR == $error_level % 255 || E_WARNING == $error_level % 255) {
