...
function RemindMail()
{
    ...
    $password = mb_substr(preg_replace('/\W/', '', md5(mt_rand())), 0, 10);

    // Set the password in the database.

    updateMemberData($row['ID_MEMBER'], ['validation_code' => "'" . mb_substr(md5($password), 0, 10) . "'"]);

    require_once $sourcedir . '/Subs-Post.php';

    sendmail(
        $row['emailAddress'],
        $txt['reminder_subject'],
...
function setPassword2()
{
    ...
    [$realCode, $username, $email] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (empty($_POST['code']) || mb_substr($realCode, 0, 10) != mb_substr(md5($_POST['code']), 0, 10)) {
        fatal_error($txt['invalid_activation_code'], false);
    }

    // User validated.  Update the database!

    updateMemberData($_POST['u'], ['validation_code' => '\'\'', 'passwd' => '\'' . sha1(mb_strtolower($username) . $_POST['passwrd1']) . '\'']);
...
function secretAnswer2()
{
    ...
    if (0 === mb_strlen(trim($_POST['passwrd1']))) {
    ...
    updateMemberData($row['ID_MEMBER'], ['passwd' => '\'' . sha1(mb_strtolower($row['memberName']) . $_POST['passwrd1']) . '\'']);
