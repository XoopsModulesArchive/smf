...
function ShowXmlFeed()
{
    ...
    if (!empty($_REQUEST['c']) && empty($board)) {
        ...
        if (1 == count($_REQUEST['c'])) {
            ...
            [$feed_title] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    $xml_format = isset($_GET['type']) && in_array($_GET['type'], ['smf', 'rss', 'rss2', 'atom', 'rdf'], true) ? $_GET['type'] : 'smf';
    ...
    $feed_title = htmlspecialchars(strip_tags($context['forum_name']), ENT_QUOTES | ENT_HTML5) . ($feed_title ?? '');
    ...
    if ('rss' == $xml_format || 'rss2' == $xml_format) {
        // Start with an RSS 2.0 header.

        echo '
<rss version=', 'rss2' == $xml_format ? '"2.0"' : '"0.92"', ' xml:lang="', strtr($txt['lang_locale'], '_', '-'), '">
	<channel>
...
function fix_possible_url($val)
{
    ...
    if (mb_substr($val, 0, mb_strlen($scripturl)) != $scripturl) {
        return $val;
    }
    ...
    if (empty($modSettings['queryless_urls']) || ($context['server']['is_cgi'] && 0 == @ini_get('cgi.fix_pathinfo')) || !$context['server']['is_apache']) {
        return $val;
    }
...
function cdata_parse($data, $ns = '')
{
    ...
    for ($pos = 0, $n = $func['strlen']($data); $pos < $n; null) {
        ...
        if ($func['substr']($data, $pos, 1) == '<') {
            ...
            } elseif (in_array($ent, ['amp', 'lt', 'gt', 'quot'], true)) {
                $cdata .= ']]>' . $func['substr']($data, $pos, $pos2 - $pos + 1) . '<![CDATA[';
            }
...
function dumpTags($data, $i, $tag = null, $xml_format = '')
{
    ...
    foreach ($data as $key => $val) {
        ...
        if ('' == $val) {
            ...
            elseif (false !== mb_strpos($val, "\n") || false !== mb_strpos($val, '<br>')) {
...
function getXmlNews($xml_format)
{
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        if (!empty($modSettings['xmlnews_maxlen']) && $modSettings['xmlnews_maxlen'] < $func['strlen'](str_replace('<br>', "\n", $row['body']))) {
...
function getXmlProfile($xml_format)
{
    ...
    if (empty($_GET['u']) || false === loadMemberData((int)$_GET['u'])) {
        return [];
    }
    ...
    if ('rss' == $xml_format || 'rss2' == $xml_format) {
        $data = [
            [
                ...
                'description' => cdata_parse($profile['group'] ?? $profile['post_group']),
        ...
        $data[] = [
            'title' => cdata_parse($profile['name']),
            'link' => $scripturl . '?action=profile;u=' . $profile['id'],
            'summary' => cdata_parse($profile['group'] ?? $profile['post_group']),
        ...
        if (!empty($profile['birth_date']) && '0000' != mb_substr($profile['birth_date'], 0, 4)) {
            [$birth_year, $birth_month, $birth_day] = sscanf($profile['birth_date'], '%d-%d-%d');
