...
function Vote()
{
    ...
    if (-1 != $row['selected'] && empty($row['changeVote'])) {
        fatal_lang_error('smf27', false);
    ...
    elseif (!empty($row['changeVote'])) {
        ...
        $request = db_query(
			...
			WHERE ID_MEMBER = $ID_MEMBER
				AND ID_POLL = $row[ID_POLL]",
            __FILE__,
            __LINE__
        );

        while (false !== ($choice = $GLOBALS['xoopsDB']->fetchRow($request))) {
    ...
    if (count($_REQUEST['options']) > $row['maxVotes']) {
        fatal_lang_error('poll_error1', false, [$row['maxVotes']]);
    }
...
function LockVoting()
{
    ...
    [$memberID, $pollID, $votingLocked] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    elseif ('2' == $votingLocked && allowedTo('moderate_board')) {
        $votingLocked = '0';
    ...
    elseif ('0' == $votingLocked && allowedTo('moderate_board')) {
        $votingLocked = '2';
...
function EditPoll()
{
    ...
    if ($context['is_edit'] && !allowedTo('poll_edit_any')) {
        isAllowedTo('poll_edit_' . ($ID_MEMBER == $pollinfo['ID_MEMBER_STARTED'] || (0 != $pollinfo['pollStarter'] && $ID_MEMBER == $pollinfo['pollStarter']) ? 'own' : 'any'));
    ...
    if (isset($_POST['preview'])) {
        ...
        if ($context['is_edit']) {
            $request = db_query(
				...
				WHERE ID_POLL = $pollinfo[ID_POLL]",
                __FILE__,
                __LINE__
            );
...
function EditPoll2()
{
    ...
    $bcinfo = $GLOBALS['xoopsDB']->fetchArray($request);
    ...
    if ($isEdit && !allowedTo('poll_edit_any')) {
        isAllowedTo('poll_edit_' . ($ID_MEMBER == $bcinfo['ID_MEMBER_STARTED'] || (0 != $bcinfo['pollStarter'] && $ID_MEMBER == $bcinfo['pollStarter']) ? 'own' : 'any'));
    ...
    if (!$isEdit || allowedTo('moderate_board')) {
        if (empty($_POST['poll_expire']) && 2 == $_POST['poll_hide']) {
            $_POST['poll_hide'] = 1;
        } else {
    ...
    if ($isEdit) {
        ...
        db_query(
			...
			VALUES (SUBSTRING('$_POST[question]', 1, 255), $_POST[poll_hide], $_POST[poll_max_votes], $_POST[poll_expire], $ID_MEMBER, SUBSTRING('$user_info[username]', 1, 255), $_POST[poll_change_vote])",
            __FILE__,
            __LINE__
        );
    ...
    if (!empty($delete_options)) {
        db_query(
			...
			DELETE FROM {$db_prefix}log_polls
			WHERE ID_POLL = $bcinfo[ID_POLL]
				AND ID_CHOICE IN (" . implode(', ', $delete_options) . ')',
            __FILE__,
            __LINE__
        );
...
function RemovePoll()
{
    ...
    if (!allowedTo('poll_remove_any')) {
        ...
        [$topicStarter, $pollStarter] = $GLOBALS['xoopsDB']->fetchRow($request);

        $GLOBALS['xoopsDB']->freeRecordSet($request);

        isAllowedTo('poll_remove_' . ($topicStarter == $ID_MEMBER || (0 != $pollStarter && $ID_MEMBER == $pollStarter) ? 'own' : 'any'));
    }
    ...
    [$pollID] = $GLOBALS['xoopsDB']->fetchRow($request);
