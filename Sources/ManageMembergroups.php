...
function ModifyMembergroups()
{
    ...
    $subActions = [
        ...
        'members' => ['MembergroupMembers', 'manage_membergroups'],
        'settings' => ['ModifyMembergroupSettings', 'admin_forum'],
    ];
    ...
    if (allowedTo('manage_membergroups')) {
        $context['admin_tabs']['tabs']['index'] = [
            ...
            'is_selected' => 'add' != $_REQUEST['sa'] && 'settings' != $_REQUEST['sa'],
        ];

        $context['admin_tabs']['tabs']['add_cat'] = [
...
function MembergroupIndex()
{
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($query))) {
        ...
        $context['groups'][-1 == $row['minPosts'] ? 'regular' : 'post'][$row['ID_GROUP']] = [
            'id' => $row['ID_GROUP'],
            'name' => $row['groupName'],
            ...
            'min_posts' => -1 == $row['minPosts'] ? '-' : $row['minPosts'],
...
function AddMembergroup()
{
    ...
    if (!empty($_POST['group_name'])) {
        ...
        [$ID_GROUP] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        elseif ('copy' == $_POST['perm_type']) {
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
                if (empty($context['illegal_permissions']) || !in_array($row['permission'], $context['illegal_permissions'], true)) {
                    $setString .= "
            ...
            if ($_POST['copyperm'] > 0) {
                ...
                $group_info = $GLOBALS['xoopsDB']->fetchArray($request);
        ...
        $_POST['boardaccess'] = empty($_POST['boardaccess']) || !is_array($_POST['boardaccess']) ? [] : $_POST['boardaccess'];
...
function EditMembergroup()
{
    ...
    elseif (isset($_POST['submit'])) {
        ...
        $_POST['min_posts'] = isset($_POST['min_posts']) && '1' == $_POST['post_group'] && $_REQUEST['group'] > 3 ? abs($_POST['min_posts']) : (4 == $_REQUEST['group'] ? 0 : -1);
        ...
        if (2 == $_REQUEST['group'] || $_REQUEST['group'] > 3) {
            $_POST['boardaccess'] = empty($_POST['boardaccess']) || !is_array($_POST['boardaccess']) ? [] : $_POST['boardaccess'];
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
                db_query(
                    "
					UPDATE {$db_prefix}boards
					SET memberGroups = '" . implode(',', array_diff(explode(',', $row['memberGroups']), [$_REQUEST['group']])) . "'
            ...
            if (!empty($_POST['boardaccess'])) {
                db_query(
					...
					WHERE ID_BOARD IN (" . implode(', ', $_POST['boardaccess']) . ')
						AND NOT FIND_IN_SET(' . (int)$_REQUEST['group'] . ', memberGroups)',
                    __FILE__,
                    __LINE__
                );
            }
        ...
        if (-1 != $_POST['min_posts']) {
            ...
            $request = db_query(
				...
				FROM {$db_prefix}members
				WHERE FIND_IN_SET(" . (int)$_REQUEST['group'] . ', additionalGroups)',
                __FILE__,
                __LINE__
            );
            ...
            foreach ($updates as $additionalGroups => $memberArray) {
                updateMemberData($memberArray, ['additionalGroups' => '\'' . implode(',', array_diff(explode(',', $additionalGroups), [(int)$_REQUEST['group']])) . '\'']);
            }
        }
    ...
    $context['group'] = [
        'id' => $_REQUEST['group'],
        'name' => $row['groupName'],
        'editable_name' => htmlspecialchars($row['groupName'], ENT_QUOTES | ENT_HTML5),
        'color' => $row['onlineColor'],
        'min_posts' => $row['minPosts'],
        'max_messages' => $row['maxMessages'],
        'star_count' => (int)$row['stars'][0],
        'star_image' => $row['stars'][1] ?? '',
        'is_post_group' => -1 != $row['minPosts'],
        'allow_post_group' => 2 == $_REQUEST['group'] || $_REQUEST['group'] > 4,
        'allow_delete' => 2 == $_REQUEST['group'] || $_REQUEST['group'] > 4,
    ];
...
function MembergroupMembers()
{
    ...
    if (in_array($_REQUEST['group'], [-1, 0, 3], true)) {
        fatal_lang_error('membergroup_does_not_exist', false);
    }
    ...
    $context['group'] = $GLOBALS['xoopsDB']->fetchArray($request);

    $GLOBALS['xoopsDB']->freeRecordSet($request);

    // Non-admins cannot assign admins.

    if (1 == $context['group']['id'] && !allowedTo('admin_forum')) {
    ...
    elseif (isset($_REQUEST['add']) && !empty($_REQUEST['toAdd']) && $context['group']['assignable']) {
        ...
        $_REQUEST['toAdd'] = strtr($func['htmlspecialchars'](stripslashes($_REQUEST['toAdd']), ENT_QUOTES), ['&quot;' => '"']);
        ...
        foreach ($memberNames as $index => $memberName) {
            ...
            if (0 == mb_strlen($memberNames[$index])) {
    ...
    [$context['total_members']] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    $request = db_query(
		...
		FROM {$db_prefix}members
		WHERE " . (empty($context['group']['is_post_group']) ? 'ID_GROUP = ' . (int)$_REQUEST['group'] . ' OR FIND_IN_SET(' . (int)$_REQUEST['group'] . ', additionalGroups)' : 'ID_POST_GROUP = ' . (int)$_REQUEST['group']) . "
		ORDER BY $querySort " . ('down' == $context['sort_direction'] ? 'DESC' : 'ASC') . "
...
function ModifyMembergroupSettings()
{
    ...
    if (!empty($_POST['save_settings'])) {
        ...
        save_inline_permissions(['manage_membergroups']);
    }

    // Initialize permissions.

    init_inline_permissions(['manage_membergroups'], [-1]);
}
