...
function PlushSearch1()
{
    ...
    if (isset($_REQUEST['params'])) {
        $temp_params = explode('|"|', base64_decode(strtr($_REQUEST['params'], [' ' => '+']), true));
    ...
    if (isset($context['search_params']['search'])) {
        $context['search_params']['search'] = htmlspecialchars($context['search_params']['search'], ENT_QUOTES | ENT_HTML5);
    }

    if (isset($context['search_params']['userspec'])) {
        $context['search_params']['userspec'] = htmlspecialchars(stripslashes($context['search_params']['userspec']), ENT_QUOTES | ENT_HTML5);
    }
    ...
    $context['num_boards'] = $GLOBALS['xoopsDB']->getRowsNum($request);
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        $context['categories'][$row['ID_CAT']]['boards'][$row['ID_BOARD']] = [
            ...
            'selected' => (empty($context['search_params']['brd']) && (empty($modSettings['recycle_enable']) || $row['ID_BOARD'] != $modSettings['recycle_board'])) || (!empty($context['search_params']['brd']) && in_array($row['ID_BOARD'], $context['search_params']['brd'], true)),
        ];
    }
    ...
    if (!empty($context['search_params']['topic'])) {
        ...
        [$context['search_topic']['subject']] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function PlushSearch2()
{
    ...
    if (!empty($modSettings['search_index']) && 'fulltext' == $modSettings['search_index']) {
        ...
        if (false !== $request && 1 == $GLOBALS['xoopsDB']->getRowsNum($request)) {
            [, $min_word_length] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        $canDoBooleanSearch = 1 == version_compare($GLOBALS['xoopsDB']->getServerVersion($db_connection), '4.0.1', '>=');

        // Get a list of banned fulltext words.

        $banned_words = empty($modSettings['search_banned_words']) ? [] : explode(',', addslashes($modSettings['search_banned_words']));
    } elseif (!empty($modSettings['search_index']) && 'custom' == $modSettings['search_index'] && !empty($modSettings['search_custom_index_config'])) {
        $customIndexSettings = unserialize($modSettings['search_custom_index_config']);

        $min_word_length = $customIndexSettings['bytes_per_word'];

        $banned_words = empty($modSettings['search_stopwords']) ? [] : explode(',', addslashes($modSettings['search_stopwords']));
    } else {
    ...
    if (!empty($search_params['searchtype']) || (!empty($_REQUEST['searchtype']) && 2 == $_REQUEST['searchtype'])) {
    ...
    if (!empty($search_params['maxage']) || (!empty($_REQUEST['maxage']) && 9999 != $_REQUEST['maxage'])) {
    ...
    if (!empty($search_params['minage']) || !empty($search_params['maxage'])) {
        ...
        [$minMsgID, $maxMsgID] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (!empty($search_params['userspec']) || (!empty($_REQUEST['userspec']) && '*' != $_REQUEST['userspec'])) {
        $search_params['userspec'] = $search_params['userspec'] ?? $_REQUEST['userspec'];
    }
    ...
    if (empty($search_params['userspec'])) {
        ...
        $userString = strtr($func['htmlspecialchars'](stripslashes($search_params['userspec']), ENT_QUOTES), ['&quot;' => '"']);
        ...
        for ($k = 0, $n = count($possible_users); $k < $n; $k++) {
            ...
            if (0 == mb_strlen($possible_users[$k])) {
        ...
        $request = db_query(
			...
			WHERE realName LIKE '" . implode("' OR realName LIKE '", $possible_users) . "'",
            __FILE__,
            __LINE__
        );
        ...
        if ($GLOBALS['xoopsDB']->getRowsNum($request) > $maxMembersToSearch) {
            $userQuery = '';
        } elseif (0 == $GLOBALS['xoopsDB']->getRowsNum($request)) {
    ...
    if (!empty($search_params['topic'])) {
        ...
        [$search_params['brd'][0]] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    elseif ($user_info['is_admin'] && (!empty($search_params['advanced']) || !empty($_REQUEST['brd']))) {
        ...
        $request = db_query(
			...
			WHERE $user_info[query_see_board]" . (empty($_REQUEST['brd']) ? (!empty($modSettings['recycle_enable']) && $modSettings['recycle_board'] > 0 ? "
				...
				AND b.ID_BOARD IN (' . implode(', ', $_REQUEST['brd']) . ')'),
            __FILE__,
            __LINE__
        );
    ...
    if (0 != count($search_params['brd'])) {
        ...
        [$num_boards] = $GLOBALS['xoopsDB']->fetchRow($request);

        $GLOBALS['xoopsDB']->freeRecordSet($request);

        if (count($search_params['brd']) == $num_boards) {
            $boardQuery = '';
        } elseif (count($search_params['brd']) == $num_boards - 1 && !empty($modSettings['recycle_board']) && !in_array($modSettings['recycle_board'], $search_params['brd'], true)) {
    ...
    if (empty($search_params['sort']) && !empty($_REQUEST['sort'])) {
        [$search_params['sort'], $search_params['sort_dir']] = array_pad(explode('|', $_REQUEST['sort']), 2, '');
    }

    $search_params['sort'] = !empty($search_params['sort']) && in_array($search_params['sort'], $sort_columns, true) ? $search_params['sort'] : 'relevance';

    if (!empty($search_params['topic']) && 'numReplies' === $search_params['sort']) {
        $search_params['sort'] = 'ID_MSG';
    }

    // Sorting direction: descending unless stated otherwise.

    $search_params['sort_dir'] = !empty($search_params['sort_dir']) && 'asc' == $search_params['sort_dir'] ? 'asc' : 'desc';
    ...
    $no_regexp = 1 === preg_match('~&#(\d{1,7}|x[0-9a-fA-F]{1,6});~', $stripped_query);
    ...
    foreach ($matches[1] as $index => $word) {
        if ('-' === $word) {
            if ('' !== ($word = trim($phraseArray[$index], '-_\' ')) && !in_array($word, $blacklisted_words, true)) {
    ...
    foreach ($wordArray as $index => $word) {
        if (0 === mb_strpos(trim($word), '-')) {
            if ('' !== ($word = trim($word, '-_\' ')) && !in_array($word, $blacklisted_words, true)) {
                $excludedWords[] = addslashes($word);
            }
    ...
    foreach ($searchArray as $index => $value) {
        if (($searchArray[$index] = trim($value, '-_\' ')) === '' || in_array($searchArray[$index], $blacklisted_words, true)) {
    ...
    foreach ($orParts as $orIndex => $andParts) {
        ...
        foreach ($orParts[$orIndex] as $word) {
            ...
            if (!$is_excluded || 1 === count($subjectWords)) {
            ...
            if (!empty($modSettings['search_index'])) {
                $subwords = text2words(stripslashes($word), 'fulltext' === $modSettings['search_index'] ? null : $min_word_length, 'custom' === $modSettings['search_index']);

                if (('custom' === $modSettings['search_index'] || ('fulltext' === $modSettings['search_index'] && !$canDoBooleanSearch && count($subwords) > 1)) && empty($modSettings['search_force_index'])) {
                    $searchWords[$orIndex]['words'][] = $word;
                }

                if ('fulltext' === $modSettings['search_index'] && $canDoBooleanSearch) {
                    $fulltextWord = 1 === count($subwords) ? $word : '"' . $word . '"';
                ...
                elseif (count($subwords) > 1 && $is_excluded) {
                    ...
                    foreach ($subwords as $subword) {
                        if (('custom' === $modSettings['search_index'] || mb_strlen(stripslashes($subword)) >= $min_word_length) && !in_array($subword, $banned_words, true)) {
                            $searchWords[$orIndex]['indexed_words'][] = $subword;

                            if ($is_excluded) {
                                $excludedIndexWords[] = $subword;
                            }
                        } elseif (!in_array($subword, $banned_words, true)) {
                            $relyOnIndex = false;
                        }
                    }

                    if ('fulltext' === $modSettings['search_index'] && $canDoBooleanSearch && !$relyOnIndex && empty($modSettings['search_force_index'])) {
                        $searchWords[$orIndex]['words'][] = $word;
                    }
                }
            }
    ...
    if ($context['show_spellchecking']) {
        ...
        $pspell_link = pspell_new($txt['lang_dictionary'], $txt['lang_spelling'], '', strtr($txt['lang_character_set'], ['iso-' => 'iso', 'ISO-' => 'iso']), PSPELL_FAST | PSPELL_RUN_TOGETHER);
        ...
        if ($search_params['subject_only']) {
            foreach ($searchWords as $orIndex => $words) {
                ...
                foreach ($words['subject_words'] as $subjectWord) {
                    ...
                    if (in_array($subjectWord, $excludedSubjectWords, true)) {
                        ...
                        $subject_query['where'][] = "subj$numTables.ID_TOPIC = " . (0 === $prev_join ? 't' : 'subj' . $prev_join) . '.ID_TOPIC';
                ...
                if (!empty($userQuery)) {
                    if (!in_array("{$db_prefix}messages AS m", $subject_query['from'], true)) {
                ...
                if (!empty($excludedPhrases)) {
                    ...
                    foreach ($excludedPhrases as $phrase) {
                        $subject_query['where'][] = 'm.subject NOT ' . (empty($modSettings['search_match_words']) || $no_regexp ? " LIKE '%" . strtr($phrase, ['_' => '\\_', '%' => '\\%']) . "%'" : " RLIKE '[[:<:]]"
                                                                                                                                                                                                     . addcslashes(preg_replace(['/([\[\]$.+*?|{}()])/'], ['[$1]'], $phrase), '\\\'')
                                                                                                                                                                                                     . "[[:>:]]'");
                    }
                }
            ...
            if (empty($search_params['topic'])) {
                ...
                $createTemporary = false !== db_query(
                    "
                ...
                foreach ($searchWords as $orIndex => $words) {
                    ...
                    foreach ($words['subject_words'] as $subjectWord) {
                        ...
                        if (in_array($subjectWord, $excludedSubjectWords, true)) {
                            ...
                            $subject_query['where'][] = 'm.body NOT ' . (empty($modSettings['search_match_words']) || $no_regexp ? " LIKE '%" . strtr($subjectWord, ['_' => '\\_', '%' => '\\%']) . "%'" : " RLIKE '[[:<:]]" . addcslashes(
                                preg_replace(['/([\[\]$.+*?|{}()])/'], ['[$1]'], $subjectWord),
                                '\\\''
                            ) . "[[:>:]]'");
                        } else {
                            $subject_query['from'][] = "{$db_prefix}log_search_subjects AS subj$numTables";
                    ...
                    if (!empty($excludedPhrases)) {
                        ...
                        foreach ($excludedPhrases as $phrase) {
                            ...
                            $subject_query['where'][] = 'm.body NOT ' . (empty($modSettings['search_match_words']) || $no_regexp ? " LIKE '%" . strtr($phrase, ['_' => '\\_', '%' => '\\%']) . "%'" : " RLIKE '[[:<:]]"
                                                                                                                                                                                                      . addcslashes(preg_replace(['/([\[\]$.+*?|{}()])/'], ['[$1]'], $phrase), '\\\'')
                                                                                                                                                                                                      . "[[:>:]]'");
                        }
                    }
                ...
                if (0 !== $numSubjectResults) {
                    $main_query['weights']['subject'] = 'IF(lst.ID_TOPIC IS NULL, 0, 1)';

                    $main_query['left_join'][] = (string)($db_prefix) . ($createTemporary ? 'tmp_' : '') . 'log_search_topics AS lst ON (' . ($createTemporary ? '' : 'lst.ID_SEARCH = ' . $_SESSION['search_cache']['ID_SEARCH'] . ' AND ') . 'lst.ID_TOPIC = t.ID_TOPIC)';
                }
            }
            ...
            if (!empty($modSettings['search_index'])) {
                ...
                foreach ($searchWords as $orIndex => $words) {
                    ...
                    if (!empty($words['indexed_words']) && 'fulltext' == $modSettings['search_index']) {
                        $fulltext_query = [
                        ...
                        if (empty($modSettings['search_simple_fulltext'])) {
                            foreach ($words['words'] as $regularWord) {
                                $fulltext_query['where'][] = 'body' . (in_array($regularWord, $excludedWords, true) ? ' NOT' : '') . (empty($modSettings['search_match_words']) || $no_regexp ? " LIKE '%" . strtr($regularWord, ['_' => '\\_', '%' => '\\%']) . "%'" : " RLIKE '[[:<:]]" . addcslashes(
                                    preg_replace(['/([\[\]$.+*?|{}()])/'], ['[$1]'], $regularWord),
                                    '\\\''
                                ) . "[[:>:]]'");
                            }
                        }
                        ...
                        if (!empty($userQuery)) {
                            $fulltext_query['where'][] = strtr($userQuery, ['m.' => '']);
                        }
                        ...
                        if (!empty($excludedPhrases) && empty($modSettings['search_force_index'])) {
                            foreach ($excludedPhrases as $phrase) {
                                $fulltext_query['where'][] = 'subject NOT ' . (empty($modSettings['search_match_words']) || $no_regexp ? " LIKE '%" . strtr($phrase, ['_' => '\\_', '%' => '\\%']) . "%'" : " RLIKE '[[:<:]]" . addcslashes(
                                    preg_replace(['/([\[\]$.+*?|{}()])/'], ['[$1]'], $phrase),
                                    '\\\''
                                ) . "[[:>:]]'");
                            }
                        }
                        ...
                        if (!empty($excludedSubjectWords) && empty($modSettings['search_force_index'])) {
                            foreach ($excludedSubjectWords as $excludedWord) {
                                $fulltext_query['where'][] = 'subject NOT ' . (empty($modSettings['search_match_words']) || $no_regexp ? " LIKE '%" . strtr($excludedWord, ['_' => '\\_', '%' => '\\%']) . "%'" : " RLIKE '[[:<:]]" . addcslashes(
                                    preg_replace(['/([\[\]$.+*?|{}()])/'], ['[$1]'], $excludedWord),
                                    '\\\''
                                ) . "[[:>:]]'");
                            }
                        }
                        ...
                        if (!empty($modSettings['search_simple_fulltext'])) {
                            ...
                            foreach ($words['indexed_words'] as $fulltextWord) {
                                $where .= (in_array($fulltextWord, $excludedIndexWords, true) ? '-' : '+') . $fulltextWord . ' ';
                            }

                            $fulltext_query['where'][] = mb_substr($where, 0, -1) . "' IN BOOLEAN MODE)";
                        } else {
                            foreach ($words['indexed_words'] as $fulltextWord) {
                                $fulltext_query['where'][] = (in_array($fulltextWord, $excludedIndexWords, true) ? 'NOT ' : '') . "MATCH (body) AGAINST ('$fulltextWord')";
                            }
                        }
                        ...
                        db_query(
							...
							WHERE " . implode(
                                '
								AND ',
                                $fulltext_query['where']
                            ) . (empty($maxMessageResults) ? '' : '
                    ...
                    elseif (!empty($words['indexed_words']) && 'custom' == $modSettings['search_index']) {
                        $custom_query = [
                            'insert_into' => $db_prefix . ($createTemporary ? 'tmp_' : '') . 'log_search_messages',
                            'select' => [
                                'ID_MSG' => 'm.ID_MSG',
                        ...
                        foreach ($words['words'] as $regularWord) {
                            $custom_query['where'][] = 'm.body' . (in_array($regularWord, $excludedWords, true) ? ' NOT' : '') . (empty($modSettings['search_match_words']) || $no_regexp ? " LIKE '%" . strtr($regularWord, ['_' => '\\_', '%' => '\\%']) . "%'" : " RLIKE '[[:<:]]" . addcslashes(
                                preg_replace(['/([\[\]$.+*?|{}()])/'], ['[$1]'], $regularWord),
                                '\\\''
                            ) . "[[:>:]]'");
                        }
                        ...
                        foreach ($words['indexed_words'] as $indexedWord) {
                            ...
                            if (in_array($indexedWord, $excludedIndexWords, true)) {
                                ...
                                $custom_query['where'][] = "lsw$numTables.ID_MSG = " . (0 === $prev_join ? 'm' : 'lsw' . $prev_join) . '.ID_MSG';
                ...
                foreach ($searchWords as $orIndex => $words) {
                    ...
                    foreach ($words['all_words'] as $regularWord) {
                        $where[] = 'm.body' . (in_array($regularWord, $excludedWords, true) ? ' NOT' : '') . (empty($modSettings['search_match_words']) || $no_regexp ? " LIKE '%" . strtr($regularWord, ['_' => '\\_', '%' => '\\%']) . "%'" : " RLIKE '[[:<:]]" . addcslashes(
                            preg_replace(['/([\[\]$.+*?|{}()])/'], ['[$1]'], $regularWord),
                            '\\\''
                        ) . "[[:>:]]'");

                        if (in_array($regularWord, $excludedWords, true)) {
                            $where[] = 'm.subject NOT' . (empty($modSettings['search_match_words']) || $no_regexp ? " LIKE '%" . strtr($regularWord, ['_' => '\\_', '%' => '\\%']) . "%'" : " RLIKE '[[:<:]]"
                                                                                                                                                                                            . addcslashes(preg_replace(['/([\[\]$.+*?|{}()])/'], ['[$1]'], $regularWord), '\\\'')
                                                                                                                                                                                            . "[[:>:]]'");
                        }
                    }
            ...
            if (!empty($indexedResults) || empty($modSettings['search_index'])) {
                ...
                $main_query['select']['relevance'] = mb_substr($relevance, 0, -3) . ") / $new_weight_total AS relevance";
                ...
                db_query(
					...
					FROM (' . implode(', ', $main_query['from']) . ')' . (empty($main_query['left_join']) ? '' : '
                        ...
                        '
						LEFT JOIN ',
                        $main_query['left_join']
                    )) . '
					WHERE ' . implode(
                            '
						AND ',
                            $main_query['where']
                        ) . (empty($main_query['group_by']) ? '' : '
            ...
            if ($_SESSION['search_cache']['num_results'] < $modSettings['search_max_results'] && 0 !== $numSubjectResults) {
                db_query(
                    "
					INSERT IGNORE INTO {$db_prefix}log_search_results
						(ID_SEARCH, ID_TOPIC, relevance, ID_MSG, num_matches)
                ...
                $_SESSION['search_cache']['num_results'] += db_affected_rows();
            } elseif (-1 == $_SESSION['search_cache']['num_results']) {
    ...
    $request = db_query(
		...
		FROM ({$db_prefix}log_search_results AS lsr" . ('numReplies' == $search_params['sort'] ? ", {$db_prefix}topics AS t" : '') . ')
		WHERE ID_SEARCH = ' . $_SESSION['search_cache']['ID_SEARCH'] . ('numReplies' == $search_params['sort'] ? '
    ...
    if (!empty($context['topics'])) {
        ...
        if (!empty($options['display_quick_mod']) && !empty($context['topics'])) {
            ...
            $context['can_lock'] = in_array(0, $boards_can['lock_any'], true);

            $context['can_sticky'] = in_array(0, $boards_can['make_sticky'], true) && !empty($modSettings['enableStickyTopics']);

            $context['can_move'] = in_array(0, $boards_can['move_any'], true);

            $context['can_remove'] = in_array(0, $boards_can['remove_any'], true);

            $context['can_merge'] = in_array(0, $boards_can['merge_any'], true);
        }
...
function prepareSearchContext($reset = false)
{
    ...
    if (null === $counter || $reset) {
        $counter = $_REQUEST['start'] + 1;
    }

    // If the query returned false, bail.

    if (false === $messages_request) {
        return false;
    }
    ...
    $message = $GLOBALS['xoopsDB']->fetchArray($messages_request);
    ...
    if ($context['compact']) {
        ...
        $message['body'] = strip_tags(strtr($message['body'], ['</div>' => '<br>']), '<br>');

        if (mb_strlen($message['body']) > $charLimit) {
            if (empty($context['key_words'])) {
                $message['body'] = $charLimit < $func['strlen']($message['body']) ? $func['substr']($message['body'], 0, $charLimit) . '<b>...</b>' : $message['body'];
            } else {
                $matchString = '';
                ...
                foreach ($context['key_words'] as $keyword) {
                    $keyword = preg_replace('~(&amp;#(\d{1,7}|x[0-9a-fA-F]{1,6});)~e', '$GLOBALS[\'func\'][\'entity_fix\'](\'\\2\')', strtr($keyword, ['\\\'' => '\'', '&' => '&amp;']));

                    if (0 != preg_match('~[\'\.,/@%&;:(){}\[\]_\-+\\\\]$~', $keyword) || 0 != preg_match('~^[\'\.,/@%&;:(){}\[\]_\-+\\\\]~', $keyword)) {
                ...
                $matchString = mb_substr($matchString, 0, -1);
                ...
                foreach ($matches[0] as $index => $match) {
                    $match = strtr(htmlspecialchars($match, ENT_QUOTES), ["\n" => '<br>']);
    ...
    $output = array_merge(
        ...
        [
            ...
            'can_reply' => in_array($message['ID_BOARD'], $boards_can['post_reply_any'], true) || in_array(0, $boards_can['post_reply_any'], true),
            'can_mark_notify' => in_array($message['ID_BOARD'], $boards_can['mark_any_notify'], true) || in_array(0, $boards_can['mark_any_notify'], true) && !$context['user']['is_guest'],
            'first_post' => [
            ...
            'last_post' => [
                ...
                'href' => $scripturl . '?topic=' . $message['ID_TOPIC'] . (0 == $message['numReplies'] ? '.0' : '.msg' . $message['last_msg']) . '#msg' . $message['last_msg'],
                'link' => '<a href="' . $scripturl . '?topic=' . $message['ID_TOPIC'] . (0 == $message['numReplies'] ? '.0' : '.msg' . $message['last_msg']) . '#msg' . $message['last_msg'] . '">' . $message['last_subject'] . '</a>',
    ...
    if (!empty($options['display_quick_mod'])) {
        ...
        $output['quick_mod'] = [
            'lock' => in_array(0, $boards_can['lock_any'], true) || in_array($output['board']['id'], $boards_can['lock_any'], true) || ($started && (in_array(0, $boards_can['lock_own'], true) || in_array($output['board']['id'], $boards_can['lock_own'], true))),
            'sticky' => (in_array(0, $boards_can['make_sticky'], true) || in_array($output['board']['id'], $boards_can['make_sticky'], true)) && !empty($modSettings['enableStickyTopics']),
            'move' => in_array(0, $boards_can['move_any'], true) || in_array($output['board']['id'], $boards_can['move_any'], true) || ($started && (in_array(0, $boards_can['move_own'], true) || in_array($output['board']['id'], $boards_can['move_own'], true))),
            'remove' => in_array(0, $boards_can['remove_any'], true) || in_array($output['board']['id'], $boards_can['remove_any'], true) || ($started && (in_array(0, $boards_can['remove_own'], true) || in_array($output['board']['id'], $boards_can['remove_own'], true))),
        ];
        ...
        $context['can_merge'] |= in_array($output['board']['id'], $boards_can['merge_any'], true);
    }

    foreach ($context['key_words'] as $query) {
        // Fix the international characters in the keyword too.

        $query = strtr($func['htmlspecialchars']($query), ['\\\'' => '\'']);

        $body_highlighted = preg_replace('/((<[^>]*)|' . preg_quote(strtr($query, ['\'' => '&#039;']), '/') . ')/ie' . ($context['utf8'] ? 'u' : ''), "'\$2' == '\$1' ? stripslashes('\$1') : '<b class=\"highlight\">\$1</b>'", $body_highlighted);
...
function searchSort($a, $b)
{
    ...
    $x = mb_strlen($a) - (in_array($a, $excludedWords, true) ? 1000 : 0);

    $y = mb_strlen($b) - (in_array($b, $excludedWords, true) ? 1000 : 0);

    if ('fulltext' == $modSettings['search_index']) {
