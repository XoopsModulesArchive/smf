...
function markBoardsRead($boards, $unread = false)
{
    ...
    [$lowest_topic] = $GLOBALS['xoopsDB']->fetchRow($result);
...
function MarkRead()
{
    ...
    if (isset($_REQUEST['sa']) && 'all' == $_REQUEST['sa']) {
        ...
        if (!empty($_SESSION['old_url']) && false !== mb_strpos($_SESSION['old_url'], 'action=unread')) {
        ...
        redirectexit();
    } elseif (isset($_REQUEST['sa']) && 'unreadreplies' == $_REQUEST['sa']) {
    ...
    elseif (isset($_REQUEST['sa']) && 'topic' == $_REQUEST['sa']) {
        if (!empty($_GET['t'])) {
            ...
            [$earlyMsg] = $GLOBALS['xoopsDB']->fetchRow($result);
        ...
        db_query(
			...
			REPLACE INTO {$db_prefix}log_topics
				(ID_MSG, ID_MEMBER, ID_TOPIC)
			VALUES ($earlyMsg, $ID_MEMBER, $topic)",
            __FILE__,
            __LINE__
        );
...
function getMsgMemberID($messageID)
{
    ...
    if ($GLOBALS['xoopsDB']->getRowsNum($result) > 0) {
        [$memberID] = $GLOBALS['xoopsDB']->fetchRow($result);
...
function QuickModeration()
{
    ...
    if (!empty($board)) {
        ...
        $redirect_url = $_POST['redirect_url'] ?? ($_SESSION['old_url'] ?? '');
    }
    ...
    if (!empty($_REQUEST['topics'])) {
        ...
        if (empty($_REQUEST['qaction']) || !in_array($_REQUEST['qaction'], $possibleActions, true)) {
            redirectexit($redirect_url);
        }

        // Merge requires all topics as one parameter and can be done at once.

        if ('merge' == $_REQUEST['qaction']) {
    ...
    if (!empty($_REQUEST['actions'])) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            if (!empty($board)) {
                ...
                if ('sticky' == $_REQUEST['actions'][$row['ID_TOPIC']] && !in_array(0, $boards_can['make_sticky'], true) && !in_array($row['ID_BOARD'], $boards_can['make_sticky'], true)) {
                    unset($_REQUEST['actions'][$row['ID_TOPIC']]);
                } elseif ('move' == $_REQUEST['actions'][$row['ID_TOPIC']] && !in_array(0, $boards_can['move_any'], true) && !in_array($row['ID_BOARD'], $boards_can['move_any'], true)
                          && ($row['ID_MEMBER_STARTED'] != $ID_MEMBER
                              || (!in_array(0, $boards_can['move_own'], true)
                                  && !in_array(
                                      $row['ID_BOARD'],
                                      $boards_can['move_own'], true
                                  )))) {
                    unset($_REQUEST['actions'][$row['ID_TOPIC']]);
                } elseif ('remove' == $_REQUEST['actions'][$row['ID_TOPIC']] && !in_array(0, $boards_can['remove_any'], true) && !in_array($row['ID_BOARD'], $boards_can['remove_any'], true)
                          && ($row['ID_MEMBER_STARTED'] != $ID_MEMBER
                              || (!in_array(0, $boards_can['remove_own'], true)
                                  && !in_array(
                                      $row['ID_BOARD'],
                                      $boards_can['remove_own'], true
                                  )))) {
                    unset($_REQUEST['actions'][$row['ID_TOPIC']]);
                } elseif ('lock' == $_REQUEST['actions'][$row['ID_TOPIC']] && !in_array(0, $boards_can['lock_any'], true) && !in_array($row['ID_BOARD'], $boards_can['lock_any'], true)
                          && ($row['ID_MEMBER_STARTED'] != $ID_MEMBER || 1 == $locked
                              || (!in_array(0, $boards_can['lock_own'], true)
                                  && !in_array(
                                      $row['ID_BOARD'],
                                      $boards_can['lock_own'], true
                                  )))) {
                    unset($_REQUEST['actions'][$row['ID_TOPIC']]);
                }
            }
    ...
    foreach ($_REQUEST['actions'] as $topic => $action) {
        ...
        if ('markread' == $action) {
            $markCache[] = $topic;
        } elseif ('sticky' == $action) {
            $stickyCache[] = $topic;
        } elseif ('move' == $action) {
            // $moveCache[0] is the topic, $moveCache[1] is the board to move to.

            $moveCache[1][$topic] = (int)($_REQUEST['move_tos'][$topic] ?? $_REQUEST['move_to']);
    ...
    if (!empty($moveCache[0])) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            ...
            if (!isset($affectedBoards[$to])) {
                $affectedBoards[$to] = [0, 0];
            }

            if (!isset($affectedBoards[$row['ID_BOARD']])) {
                $affectedBoards[$row['ID_BOARD']] = [0, 0];
            }
...
function QuickModeration2()
{
    ...
    elseif (allowedTo('delete_replies')) {
        ...
        [$starter] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        if (!$allowed_all && !empty($modSettings['edit_disable_time']) && time() > $row['posterTime'] + $modSettings['edit_disable_time'] * 60) {
            continue;
        }
    ...
    [$first_message, $last_message] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    foreach ($messages as $message => $info) {
        ...
        if (allowedTo('delete_any') && (!allowedTo('delete_own') || $info[1] != $ID_MEMBER)) {
            logAction('delete', ['topic' => $topic, 'subject' => $info[0], 'member' => $info[1]]);
        }
    }
...
function modifyBoard($board_id, &$boardOptions)
{
    ...
    if (isset($boardOptions['move_to'])) {
        ...
        elseif ('bottom' == $boardOptions['move_to']) {
            $ID_CAT = $boardOptions['target_category'];
        ...
        elseif ('child' == $boardOptions['move_to']) {
        ...
        elseif (in_array($boardOptions['move_to'], ['before', 'after'], true)) {
            $ID_CAT = $boards[$boardOptions['target_board']]['category'];
            ...
            $after = $boards[$boardOptions['target_board']]['order'] - ('before' == $boardOptions['move_to'] ? 1 : 0);
        } // Oops...?
        ...
        if (!empty($childList) && !empty($childUpdates)) {
            db_query(
				...
				WHERE ID_BOARD IN (' . implode(', ', $childList) . ')',
                __FILE__,
                __LINE__
            );
        }
    ...
    if (isset($boardOptions['moderators']) || isset($boardOptions['moderator_string'])) {
        ...
        if (isset($boardOptions['moderator_string']) && '' != trim($boardOptions['moderator_string'])) {
            ...
            $moderator_string = strtr($func['htmlspecialchars'](stripslashes($boardOptions['moderator_string']), ENT_QUOTES), ['&quot;' => '"']);
...
function createBoard($boardOptions)
{
    ...
    if (in_array($boardOptions['move_to'], ['child', 'before', 'after'], true) && !isset($boardOptions['target_board'])) {
    ...
    if ($boardOptions['inherit_permissions']) {
        ...
        if (empty($modSettings['permission_enable_by_board']) && !empty($boards[$board_id]['parent']) && empty($boards[$boards[$board_id]['parent']]['use_local_permissions'])) {
            ...
            [$boardOptions['permission_mode']] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function deleteBoards($boards_to_remove, $moveChildrenTo = null)
{
    ...
    if (!empty($modSettings['recycle_board']) && in_array($modSettings['recycle_board'], $boards_to_remove, true)) {
        updateSettings(['recycle_board' => 0, 'recycle_enable' => 0]);
    }
...
function deleteCategories($categories, $moveBoardsTo = null)
{
    ...
    elseif (in_array($moveBoardsTo, $categories, true)) {
        ...
        if (!empty($row['ID_BOARD'])) {
            ...
            if (empty($row['childLevel'])) {
                ...
                if (!isset($boards[$row['ID_PARENT']]['tree'])) {
                    fatal_lang_error('no_valid_parent', false, [$row['bName']]);
                }

                // Wrong childlevel...we can silently fix this...

                if ($row['childLevel'] - 1 != $boards[$row['ID_PARENT']]['tree']['node']['level']) {
                    db_query(
                        "
						UPDATE {$db_prefix}boards
