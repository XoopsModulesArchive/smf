...
function MessageMain()
{
    ...
    if (false === loadLanguage('PersonalMessage', '', false)) {
    ...
    if (WIRELESS) {
        ...
        if (false === loadTemplate('PersonalMessage', false)) {
    ...
    if (!$user_info['is_admin']) {
        ...
        [$maxMessage, $minMessage] = $GLOBALS['xoopsDB']->fetchRow($request);

        $GLOBALS['xoopsDB']->freeRecordSet($request);

        $context['message_limit'] = 0 == $minMessage ? 0 : $maxMessage;
    } else {
    ...
    $context['labels'] = '' == $user_settings['messageLabels'] ? [] : explode(',', $user_settings['messageLabels']);

    foreach ($context['labels'] as $k => $v) {
        $context['labels'][(int)$k] = ['id' => $k, 'name' => trim($v), 'messages' => 0, 'unread_messages' => 0];
    }

    $context['labels'][-1] = ['id' => -1, 'name' => $txt['pm_msg_label_inbox'], 'messages' => 0, 'unread_messages' => 0];
    ...
    $context['folder'] = !isset($_REQUEST['f']) || 'outbox' != $_REQUEST['f'] ? 'inbox' : 'outbox';
...
function messageIndexBar($area)
{
    ...
    $context['pm_areas'] = [
        'folders' => [
            ...
            'areas' => [
                'send' => ['link' => '<a href="' . $scripturl . '?action=pm;sa=send">' . $txt[321] . '</a>', 'href' => $scripturl . '?action=pm;sa=send'],
                '' => [],
                'inbox' => ['link' => '<a href="' . $scripturl . '?action=pm">' . $txt[316] . '</a>', 'href' => $scripturl . '?action=pm'],
                'outbox' => ['link' => '<a href="' . $scripturl . '?action=pm;f=outbox">' . $txt[320] . '</a>', 'href' => $scripturl . '?action=pm;f=outbox'],
            ],
        ],
        'labels' => [
            'title' => $txt['pm_labels'],
        ...
        'pref' => [
            ...
            'areas' => [
                'search' => ['link' => '<a href="' . $scripturl . '?action=pm;sa=search">' . $txt['pm_search_bar_title'] . '</a>', 'href' => $scripturl . '?action=pm;sa=search'],
                'manlabels' => ['link' => '<a href="' . $scripturl . '?action=pm;sa=manlabels">' . $txt['pm_manage_labels'] . '</a>', 'href' => $scripturl . '?action=pm;sa=manlabels'],
                'prune' => ['link' => '<a href="' . $scripturl . '?action=pm;sa=prune">' . $txt['pm_prune'] . '</a>', 'href' => $scripturl . '?action=pm;sa=prune'],
            ],
        ],
    ];
    ...
    if (empty($context['currently_using_labels'])) {
        ...
        foreach ($context['labels'] as $label) {
            ...
            $context['pm_areas']['labels']['areas']['label' . $label['id']] = [
    ...
    if (!empty($context['message_limit'])) {
        ...
        $context['pm_areas']['pref']['areas']['limit_bar'] = ['limit_bar' => true];
    }
...
function MessageFolder()
{
    ...
    if (isset($_GET['start']) && 'new' != $_GET['start']) {
    ...
    $context['from_or_to'] = 'outbox' != $context['folder'] ? 'from' : 'to';

    $context['get_pmessage'] = 'prepareMessageContext';

    $labelQuery = 'outbox' != $context['folder'] ? "
			AND FIND_IN_SET('$context[current_label_id]', pmr.labels)" : '';

    // Set the index bar correct!

    messageIndexBar(-1 == $context['current_label_id'] ? $context['folder'] : 'label' . $context['current_label_id']);
    ...
    if ('outbox' == $context['folder']) {
        isAllowedTo('pm_send');
    }

    // Set the text to resemble the current folder.

    $pmbox = 'outbox' != $context['folder'] ? $txt[316] : $txt[320];
    ...
    if ('outbox' != $context['folder'] && !empty($context['labels'][(int)$context['current_label_id']]['unread_messages'])) {
    ...
    [$max_messages] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (isset($_GET['pmid'])) {
        ...
        if ($max_messages <= $modSettings['defaultMaxMessages']) {
            ...
            [$_GET['start']] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    $context['links'] = [
        ...
        'next' => $max_messages > $_GET['start'] + $modSettings['defaultMaxMessages'] ? $scripturl . '?action=pm;start=' . ($_GET['start'] + $modSettings['defaultMaxMessages']) : '',
        'last' => $max_messages > $_GET['start'] + $modSettings['defaultMaxMessages'] ? $scripturl . '?action=pm;start=' . (floor(($max_messages - 1) / $modSettings['defaultMaxMessages']) * $modSettings['defaultMaxMessages']) : '',
        'up' => $scripturl,
    ];
    ...
    $request = db_query(
		...
		FROM ({$db_prefix}personal_messages AS pm" . ('outbox' == $context['folder'] ? ')' . ('name' == $context['sort_by'] ? "
			LEFT JOIN {$db_prefix}pm_recipients AS pmr ON (pmr.ID_PM = pm.ID_PM)" : '') : ", {$db_prefix}pm_recipients AS pmr)") . ('name' == $context['sort_by'] ? ("
			LEFT JOIN {$db_prefix}members AS mem ON (mem.ID_MEMBER = " . ('outbox' == $context['folder'] ? 'pmr.ID_MEMBER' : 'pm.ID_MEMBER_FROM') . ')') : '') . '
		WHERE ' . ('outbox' == $context['folder'] ? "pm.ID_MEMBER_FROM = $ID_MEMBER
		...
		ORDER BY ' . ('pm.ID_PM' == $_GET['sort'] && 'outbox' != $context['folder'] ? 'pmr.ID_PM' : $_GET['sort']) . ($descending ? ' DESC' : ' ASC') . (empty($_GET['pmsg']) ? "
    ...
    $posters = 'outbox' == $context['folder'] ? [$ID_MEMBER] : [];
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        if (!isset($recipients[$row['ID_PM']])) {
            ...
            if (!empty($row['ID_MEMBER_FROM']) && 'outbox' != $context['folder']) {
    ...
    if (!empty($pms)) {
        ...
        $request = db_query(
			...
			WHERE pmr.ID_PM IN (" . implode(', ', $pms) . ')',
            __FILE__,
            __LINE__
        );
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            if ('outbox' == $context['folder'] || empty($row['bcc'])) {
            ...
            if ($row['ID_MEMBER_TO'] == $ID_MEMBER && 'outbox' != $context['folder']) {
                ...
                foreach ($row['labels'] as $v) {
                    if (isset($context['labels'][(int)$v])) {
                        $context['message_labels'][$row['ID_PM']][(int)$v] = ['id' => $v, 'name' => $context['labels'][(int)$v]['name']];
                    }
                }
            }
        }
        ...
        $messages_request = db_query(
			...
			FROM {$db_prefix}personal_messages AS pm" . ('outbox' == $context['folder'] ? "
				LEFT JOIN {$db_prefix}pm_recipients AS pmr ON (pmr.ID_PM = pm.ID_PM)" : '') . ('name' == $context['sort_by'] ? "
				LEFT JOIN {$db_prefix}members AS mem ON (mem.ID_MEMBER = " . ('outbox' == $context['folder'] ? 'pmr.ID_MEMBER' : 'pm.ID_MEMBER_FROM') . ')' : '') . '
			WHERE pm.ID_PM IN (' . implode(',', $pms) . ')' . ('outbox' == $context['folder'] ? '
			GROUP BY pm.ID_PM' : '') . "
...
function prepareMessageContext($reset = false)
{
    ...
    $output = [
        ...
        'is_selected' => !empty($temp_pm_selected) && in_array($message['ID_PM'], $temp_pm_selected, true),
    ];
...
function MessageSearch()
{
    ...
    foreach ($context['labels'] as $label) {
        $context['search_labels'][] = [
            'id' => $label['id'],
            'name' => $label['name'],
            'checked' => !empty($searchedLabels) ? in_array($label['id'], $searchedLabels, true) : true,
        ];
    }
...
function MessageSearch2()
{
    ...
    $search_params['sort'] = !empty($search_params['sort']) && in_array($search_params['sort'], $sort_columns, true) ? $search_params['sort'] : 'ID_PM';
    ...
    if ('inbox' == $context['folder'] && !empty($search_params['advanced']) && $context['currently_using_labels']) {
    ...
    foreach ($tempSearch as $index => $word) {
        if (0 === mb_strpos(trim($word), '-')) {
            $word = mb_substr($func['strtolower'](trim($word)), 1);
    ...
    if (isset($context['search_params']['userspec'])) {
        $context['search_params']['userspec'] = htmlspecialchars($context['search_params']['userspec'], ENT_QUOTES | ENT_HTML5);
    }
    ...
    foreach ($searchWords as $index => $word) {
        ...
        if ($search_params['subject_only']) {
            $andQueryParts[] = 'pm.subject' . (in_array($word, $excludedWords, true) ? ' NOT' : '') . " LIKE '%" . strtr($word, ['_' => '\\_', '%' => '\\%']) . "%'";
        } else {
            $andQueryParts[] = '(pm.subject' . (in_array($word, $excludedWords, true) ? ' NOT' : '') . " LIKE '%" . strtr($word, ['_' => '\\_', '%' => '\\%']) . "%' " . (in_array($word, $excludedWords, true) ? 'AND pm.body NOT' : 'OR pm.body') . " LIKE '%" . strtr($word, ['_' => '\\_', '%' => '\\%']) . "%')";
        }
    }
    ...
    if (!empty($andQueryParts)) {
        $searchQuery = implode(!empty($search_params['searchtype']) && 2 == $search_params['searchtype'] ? ' OR ' : ' AND ', $andQueryParts);
    }
    ...
    $request = db_query(
		...
		WHERE pm.ID_PM = pmr.ID_PM" . ('inbox' == $context['folder'] ? "
			AND pmr.ID_MEMBER = $ID_MEMBER
			AND pmr.deleted = 0" : "
    ...
    if (!empty($foundMessages)) {
        ...
        $request = db_query(
			...
			FROM {$db_prefix}pm_recipients AS pmr
				LEFT JOIN {$db_prefix}members AS mem_to ON (mem_to.ID_MEMBER = pmr.ID_MEMBER)
			WHERE pmr.ID_PM IN (" . implode(', ', $foundMessages) . ')',
            __FILE__,
            __LINE__
        );
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            ...
            if ($row['ID_MEMBER_TO'] == $ID_MEMBER && 'outbox' != $context['folder']) {
                ...
                foreach ($row['labels'] as $v) {
                    ...
                    if (!isset($context['first_label'][$row['ID_PM']]) && !in_array('-1', $row['labels'], true)) {
...
function MessagePost()
{
    ...
    if (!empty($modSettings['pm_posts_per_hour']) && !allowedTo(['admin_forum', 'moderate_forum', 'send_mail'])) {
        ...
        $request = db_query(
			...
			WHERE pm.ID_MEMBER_FROM = $ID_MEMBER
				...
				AND pr.ID_PM = pm.ID_PM',
            __FILE__,
            __LINE__
        );

        [$postCount] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (!empty($_REQUEST['pmsg'])) {
        ...
        $request = db_query(
			...
			FROM ({$db_prefix}personal_messages AS pm" . ('outbox' == $context['folder'] ? '' : ", {$db_prefix}pm_recipients AS pmr") . ")
				LEFT JOIN {$db_prefix}members AS mem ON (mem.ID_MEMBER = pm.ID_MEMBER_FROM)
			WHERE pm.ID_PM = $_REQUEST[pmsg]" . ('outbox' == $context['folder'] ? "
				AND pm.ID_MEMBER_FROM = $ID_MEMBER" : "
        ...
        $row_quoted = $GLOBALS['xoopsDB']->fetchArray($request);
    ...
    if (isset($_REQUEST['u'])) {
        ...
        if ('all' == $_REQUEST['u'] && isset($row_quoted)) {
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
                $membersTo[] = '&quot;' . htmlspecialchars($row['realName'], ENT_QUOTES | ENT_HTML5) . '&quot;';
            }
    ...
    if ($context['visual_verification']) {
        $context['use_graphic_library'] = in_array('gd', get_loaded_extensions(), true);

        $context['verificiation_image_href'] = $scripturl . '?action=verificationcode;rand=' . md5(mt_rand());

        // Skip I, J, L, O, Q, S and Z.

        $character_range = array_merge(range('A', 'H'), ['K', 'M', 'N', 'P'], range('R', 'Z'));
...
function messagePostError($error_types, $to, $bcc)
{
    ...
    $context['message'] = isset($_REQUEST['message']) ? str_replace(['  '], ['&nbsp; '], $func['htmlspecialchars'](stripslashes($_REQUEST['message']))) : '';
    ...
    if ($context['reply']) {
        ...
        $request = db_query(
			...
			FROM ({$db_prefix}personal_messages AS pm" . ('outbox' == $context['folder'] ? '' : ", {$db_prefix}pm_recipients AS pmr") . ")
				LEFT JOIN {$db_prefix}members AS mem ON (mem.ID_MEMBER = pm.ID_MEMBER_FROM)
			WHERE pm.ID_PM = $_REQUEST[replied_to]" . ('outbox' == $context['folder'] ? "
				AND pm.ID_MEMBER_FROM = $ID_MEMBER" : "
...
function MessagePost2()
{
    ...
    } elseif (!empty($modSettings['max_messageLength']) && $modSettings['max_messageLength'] < $func['strlen']($_REQUEST['message'])) {
    ...
    if (!$user_info['is_admin'] && !empty($modSettings['pm_posts_verification']) && $user_info['posts'] < $modSettings['pm_posts_verification'] && (empty($_REQUEST['visual_verification_code']) || mb_strtoupper($_REQUEST['visual_verification_code']) !== $_SESSION['visual_verification_code'])) {
    ...
    if (isset($_REQUEST['preview'])) {
        ...
        return messagePostError([], $func['htmlspecialchars']($_REQUEST['to']), $func['htmlspecialchars']($_REQUEST['bcc']));
    }
    ...
    if (empty($_REQUEST['u'])) {
        ...
        if (!empty($_REQUEST['bcc'])) {
            ...
            $_REQUEST['bcc'] = strtr($_REQUEST['bcc'], ['\\"' => '"']);
        ...
        foreach ($foundMembers as $member) {
            ...
            $member['name'] = strtr($member['name'], ['&#039;' => '\'']);

            foreach ($input as $rec_type => $to_members) {
                if (array_intersect([$func['strtolower']($member['username']), $func['strtolower']($member['name']), $func['strtolower']($member['email'])], $to_members)) {
                    $recipients[$rec_type][] = $member['id'];

                    // Get rid of this username. The ones that remain were not found.

                    $input[$rec_type] = array_diff($input[$rec_type], [$func['strtolower']($member['username']), $func['strtolower']($member['name']), $func['strtolower']($member['email'])]);
                }
            }
        }
    } else {
    ...
    if (!empty($modSettings['max_pm_recipients']) && count($recipients['to']) + count($recipients['bcc']) > $modSettings['max_pm_recipients'] && !allowedTo(['moderate_forum', 'send_mail', 'admin_forum'])) {
        $context['send_log'] = [
            'sent' => [],
    ...
    foreach ($input as $rec_type => $rec) {
        ...
        if (!empty($rec) && !in_array('bad_' . $rec_type, $post_errors, true)) {
    ...
    if (!empty($context['send_log']['sent']) && !empty($_REQUEST['replied_to']) && isset($_REQUEST['f']) && 'inbox' == $_REQUEST['f']) {
        db_query(
            "
...
function WirelessAddBuddy()
{
    ...
    $current_buddies = empty($_REQUEST['u']) ? [] : explode(',', $_REQUEST['u']);
    ...
    if (!empty($user_info['buddies'])) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            $context['buddies'][] = [
                'id' => $row['ID_MEMBER'],
                'name' => $row['realName'],
                'selected' => in_array($row['ID_MEMBER'], $current_buddies, true),
...
function MessageActionsApply()
{
    ...
    foreach ($_REQUEST['pm_actions'] as $pm => $action) {
        if ('delete' === $action) {
            ...
            if ('add_' == mb_substr($action, 0, 4)) {
                $type = 'add';

                $action = mb_substr($action, 4);
            } elseif ('rem_' == mb_substr($action, 0, 4)) {
                $type = 'rem';
            ...
            if ('-1' == $action || '0' == $action || (int)$action > 0) {
    ...
    if (!empty($to_label) && 'inbox' == $context['folder']) {
        $updateErrors = 0;
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            ...
            $ID_LABEL = array_search($to_label[$row['ID_PM']], $labels, true);

            if (false !== $ID_LABEL && 'add' !== $label_type[$row['ID_PM']]) {
                unset($labels[$ID_LABEL]);
            } elseif ('rem' !== $label_type[$row['ID_PM']]) {
    ...
    redirectexit($context['current_label_redirect'] . (1 == count($to_label) ? '#' . $_SESSION['pm_selected'][0] : ''), 1 == count($to_label) && $context['browser']['is_ie']);
}
...
function MessageKillAllQuery()
{
    ...
    $context['delete_all'] = 'all' == $_REQUEST['f'];

    // And set the folder name...

    $txt[412] = str_replace('PMBOX', 'outbox' != $context['folder'] ? $txt[316] : $txt[320], $txt[412]);
}
...
function MessageKillAll()
{
    ...
    if ('all' == $_REQUEST['f']) {
        ...
        deleteMessages(null, 'outbox' != $_REQUEST['f'] ? 'inbox' : 'outbox');
    }
...
function deleteMessages($personal_messages, $folder = null, $owner = null)
{
    ...
    if ('outbox' != $folder || null === $folder) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            if ($row['is_read']) {
                updateMemberData($row['ID_MEMBER'], ['instantMessages' => '' == $where ? 0 : "instantMessages - $row[numDeletedMessages]"]);
            } else {
                updateMemberData($row['ID_MEMBER'], ['instantMessages' => '' == $where ? 0 : "instantMessages - $row[numDeletedMessages]", 'unreadMessages' => '' == $where ? 0 : "unreadMessages - $row[numDeletedMessages]"]);
            }
...
function markMessages($personal_messages = null, $label = null, $owner = null)
{
    ...
    db_query(
		...
		WHERE ID_MEMBER = $owner
			AND NOT (is_read & 1)" . (null === $label ? '' : "
			AND FIND_IN_SET($label, labels)") . (null !== $personal_messages ? '
			AND ID_PM IN (' . implode(', ', $personal_messages) . ')
		LIMIT ' . count($personal_messages) : ''),
        __FILE__,
        __LINE__
    );
    ...
    if (db_affected_rows() > 0) {
        ...
        updateMemberData($owner, ['unreadMessages' => $total_unread]);
...
function ManageLabels()
{
    ...
    if (isset($_GET['sesc'])) {
        ...
        if (isset($_POST['add'])) {
            $_POST['label'] = strtr($func['htmlspecialchars'](trim($_POST['label'])), [',' => '&#044;']);
        ...
        elseif (isset($_POST['save']) && !empty($_POST['label_name'])) {
            ...
            foreach ($the_labels as $id => $name) {
                if (-1 == $id) {
                    ...
                    $_POST['label_name'][$id] = trim(strtr($func['htmlspecialchars']($_POST['label_name'][$id]), [',' => '&#044;']));
        ...
        updateMemberData($ID_MEMBER, ['messageLabels' => "'" . implode(',', $the_labels) . "'"]);
        ...
        if (!empty($message_changes)) {
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
                ...
                foreach ($toChange as $key => $value) {
                    if (in_array($value, $searchArray, true)) {
...
function ReportMessage()
{
    ...
    if (!isset($_REQUEST['report'])) {
        ...
        [$subject, $body, $time, $memberFromID, $memberFromName] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            ...
            if (!isset($messagesToSend[$cur_language])) {
                if (false === loadLanguage('PersonalMessage', $cur_language, false)) {
