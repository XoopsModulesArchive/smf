...
function RemoveTopic2()
{
    ...
    [$starter, $subject] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (allowedTo('remove_any') && (!allowedTo('remove_own') || $starter != $ID_MEMBER)) {
        logAction('remove', ['topic' => $topic, 'subject' => $subject, 'member' => $starter]);
    }
...
function DeleteMessage()
{
    ...
    [$starter, $poster, $subject, $post_time] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (allowedTo('delete_any') && (!allowedTo('delete_own') || $poster != $ID_MEMBER)) {
        logAction('delete', ['topic' => $topic, 'subject' => $subject, 'member' => $starter]);
    }
...
function RemoveOldTopics2()
{
    ...
    if ('moved' == $_POST['delete_type']) {
        $condition .= '
			AND m.icon = \'moved\'
			AND t.locked = 1';
    } // Otherwise, maybe locked topics only?

    elseif ('locked' == $_POST['delete_type']) {
        $condition .= '
			AND t.locked = 1';
    }
    ...
    $request = db_query(
		...
		WHERE m.ID_MSG = t.ID_LAST_MSG
			...
			AND t.ID_BOARD IN (" . implode(', ', array_keys($_POST['boards'])) . ')',
        __FILE__,
        __LINE__
    );
...
function removeTopics($topics, $decreasePostCount = true, $ignoreRecycling = false)
{
    ...
    if ($decreasePostCount) {
        ...
        if ($GLOBALS['xoopsDB']->getRowsNum($requestMembers) > 0) {
            while (false !== ($rowMembers = $GLOBALS['xoopsDB']->fetchArray($requestMembers))) {
                updateMemberData($rowMembers['ID_MEMBER'], ['posts' => 'posts - ' . $rowMembers['posts']]);
            }
        }

        $GLOBALS['xoopsDB']->freeRecordSet($requestMembers);
    }
    ...
    if (!empty($modSettings['recycle_enable']) && $modSettings['recycle_board'] > 0 && !$ignoreRecycling) {
        ...
        if ($GLOBALS['xoopsDB']->getRowsNum($request) > 0) {
            ...
            db_query(
				...
				SET icon = 'recycled'
				WHERE ID_TOPIC IN (" . implode(', ', $recycleTopics) . ')',
                __FILE__,
                __LINE__
            );
    ...
    if (!empty($polls)) {
        $pollCondition = 1 == count($polls) ? '= ' . $polls[0] : 'IN (' . implode(', ', $polls) . ')';
        ...
        if (!empty($words) && !empty($messages)) {
            db_query(
				...
				WHERE ID_WORD IN (" . implode(', ', $words) . ')
					AND ID_MSG IN (' . implode(', ', $messages) . ')',
                __FILE__,
                __LINE__
            );
        }
...
function removeMessage($message, $decreasePostCount = true)
{
    ...
    if (empty($board) || $row['ID_BOARD'] != $board) {
        ...
        if (!in_array(0, $delete_any, true) && !in_array($row['ID_BOARD'], $delete_any, true)) {
            $delete_own = boardsAllowedTo('delete_own');

            $delete_own = in_array(0, $delete_own, true) || in_array($row['ID_BOARD'], $delete_own, true);

            $delete_replies = boardsAllowedTo('delete_replies');

            $delete_replies = in_array(0, $delete_replies, true) || in_array($row['ID_BOARD'], $delete_replies, true);
            ...
            if ($row['ID_MEMBER'] == $ID_MEMBER) {
                ...
                } elseif (($row['ID_MEMBER_POSTER'] != $ID_MEMBER || !$delete_replies) && !empty($modSettings['edit_disable_time']) && time() > $row['posterTime'] + $modSettings['edit_disable_time'] * 60) {
                    fatal_lang_error('modify_post_time_passed', false);
                }
        ...
        if ($row['ID_MEMBER'] == $ID_MEMBER) {
            ...
            } elseif (!allowedTo('delete_any') && ($row['ID_MEMBER_POSTER'] != $ID_MEMBER || !allowedTo('delete_replies')) && !empty($modSettings['edit_disable_time']) && time() > $row['posterTime'] + $modSettings['edit_disable_time'] * 60) {
                fatal_lang_error('modify_post_time_passed', false);
            }
    ...
    if ($row['ID_FIRST_MSG'] == $message) {
        if (empty($board) || $row['ID_BOARD'] != $board) {
            ...
            $remove_any = in_array(0, $remove_any, true) || in_array($row['ID_BOARD'], $remove_any, true);

            if (!$remove_any) {
                $remove_own = boardsAllowedTo('remove_own');

                $remove_own = in_array(0, $remove_own, true) || in_array($row['ID_BOARD'], $remove_own, true);
            }
    ...
    if (!empty($modSettings['recycle_enable']) && $row['ID_BOARD'] != $modSettings['recycle_board'] && 'recycled' != $row['icon']) {
        ...
        $request = db_query(
			...
			WHERE b.ID_BOARD = $modSettings[recycle_board]",
            __FILE__,
            __LINE__
        );
        ...
        [$isRead] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        db_query(
			...
			VALUES ($modSettings[recycle_board], $row[ID_MEMBER], $row[ID_MEMBER], $message, $message)",
            __FILE__,
            __LINE__
        );
        ...
        if ($topicID > 0) {
            ...
            if (!$user_info['is_guest']) {
                db_query(
					...
					VALUES ($topicID, $ID_MEMBER, $modSettings[maxMsgID])",
                    __FILE__,
                    __LINE__
                );
            }
            ...
            if (!empty($isRead) && !$user_info['is_guest']) {
                db_query(
					...
					VALUES ($modSettings[recycle_board], $ID_MEMBER, $modSettings[maxMsgID])",
                    __FILE__,
                    __LINE__
                );
            }
