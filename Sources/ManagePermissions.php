...
function ModifyPermissions()
{
    ...
    $subActions = [
        ...
        'settings' => ['GeneralPermissionSettings', 'admin_forum'],
        'switch' => ['SwitchBoard', 'manage_permissions'],
    ];
...
function PermissionIndex()
{
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($query))) {
        $row['stars'] = explode('#', $row['stars']);

        $context['groups'][$row['ID_GROUP']] = [
            'id' => $row['ID_GROUP'],
            'name' => $row['groupName'],
            'num_members' => 3 != $row['ID_GROUP'] ? 0 : $txt['membergroups_guests_na'],
            'allow_delete' => $row['ID_GROUP'] > 4,
            'allow_modify' => $row['ID_GROUP'] > 1,
            'can_search' => 3 != $row['ID_GROUP'],
            'href' => $scripturl . '?action=viewmembers;sa=query;params=' . base64_encode(-1 == $row['minPosts'] ? "ID_GROUP = $row[ID_GROUP] OR FIND_IN_SET($row[ID_GROUP], additionalGroups)" : "ID_POST_GROUP = $row[ID_GROUP]"),
            'is_post_group' => -1 != $row['minPosts'],
            ...
            'num_permissions' => [
                'allowed' => 1 == $row['ID_GROUP'] ? '(' . $txt['permissions_all'] . ')' : 0,
                'denied' => 1 == $row['ID_GROUP'] ? '(' . $txt['permissions_none'] . ')' : 0,
            ],
            'access' => false,
        ];
    ...
    */
    ...
    if (empty($_REQUEST['boardid'])) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            if (isset($context['groups'][(int)$row['ID_GROUP']]) && (!empty($row['addDeny']) || -1 != $row['ID_GROUP'])) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            ...
            } elseif (isset($context['boards'][$row['ID_BOARD']]) && isset($context['boards'][$row['ID_BOARD']]['groups'][(int)$row['ID_GROUP']]) && (!empty($row['addDeny']) || -1 != $row['ID_GROUP'])) {
        ...
        [$context['board']['name'], $groups] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function PermissionByBoard()
{
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        $context['boards'][$row['ID_BOARD']] = [
            ...
            'public' => in_array(0, $row['memberGroups'], true) || in_array(-1, $row['memberGroups'], true),
            'membergroups' => $row['memberGroups'],
            'use_local_permissions' => !empty($modSettings['permission_enable_by_board']) && 1 == $row['permission_mode'],
            'permission_mode' => empty($modSettings['permission_enable_by_board']) ? (empty($row['permission_mode']) ? 'normal' : (2 == $row['permission_mode'] ? 'no_polls' : (3 == $row['permission_mode'] ? 'reply_only' : 'read_only'))) : 'normal',
            //			'groups' => $board_groups
        ];
    }
...
function SetQuickGroups()
{
    ...
    if ((!empty($_POST['predefined']) && ((isset($_POST['copy_from']) && 'empty' != $_POST['copy_from']) || !empty($_POST['permissions']))) || (!empty($_POST['copy_from']) && 'empty' != $_POST['copy_from'] && !empty($_POST['permissions']))) {
    ...
    if (!empty($_POST['predefined'])) {
        ...
        if (!in_array($_POST['predefined'], ['restrict', 'standard', 'moderator', 'maintenance'], true)) {
            redirectexit('action=permissions;boardid=' . $_REQUEST['boardid']);
        }
    ...
    elseif (isset($_POST['from_board']) && 'empty' != $_POST['from_board']) {
        ...
        $request = db_query(
			...
			WHERE ID_BOARD = $_POST[from_board]
				AND ID_GROUP IN (" . implode(',', $_POST['group']) . ')',
            __FILE__,
            __LINE__
        );
        ...
        if (empty($_REQUEST['boardid'])) {
            ...
            foreach ($_POST['group'] as $group_id) {
                foreach ($target_perm as $perm => $addDeny) {
                    ...
                    if (!empty($context['illegal_permissions']) && in_array($perm, $context['illegal_permissions'], true)) {
                        continue;
                    }
            ...
            if (!empty($insert_string)) {
                ...
                $insert_string = mb_substr($insert_string, 0, -1);
    ...
    elseif (!empty($_POST['permissions'])) {
        ...
        [$permissionType, $permission] = explode('/', $_POST['permissions']);

        // Check whether our input is within expected range.

        if (!in_array($_POST['add_remove'], ['add', 'clear', 'deny'], true) || !in_array($permissionType, ['membergroup', 'board'], true)) {
            redirectexit('action=permissions;boardid=' . $_REQUEST['boardid']);
        }

        if ('clear' == $_POST['add_remove']) {
            if ('membergroup' == $permissionType) {
                db_query(
                    "
					DELETE FROM {$db_prefix}permissions
					WHERE ID_GROUP IN (" . implode(', ', $_POST['group']) . ")
            ...
            $addDeny = 'add' == $_POST['add_remove'] ? '1' : '0';

            if ('membergroup' == $permissionType && (empty($context['illegal_permissions']) || !in_array($permission, $context['illegal_permissions'], true))) {
                db_query(
                    "
            ...
            elseif ('membergroup' != $permissionType) {
                db_query(
                    "
...
function SwitchBoard()
{
    ...
    $request = db_query(
		...
		WHERE ID_BOARD = $_GET[boardid]
			AND permission_mode = " . ('local' == $_GET['to'] ? '0' : '1') . '
		LIMIT 1',
        __FILE__,
        __LINE__
    );
    ...
    db_query(
        "
		UPDATE {$db_prefix}boards
		SET permission_mode = " . ('local' == $_GET['to'] ? '1' : '0') . "
		WHERE ID_BOARD = $_GET[boardid]
		LIMIT 1",
        __FILE__,
        __LINE__
...
function ModifyMembergroup()
{
    ...
    if ((empty($modSettings['permission_enable_by_board']) && !empty($_REQUEST['boardid'])) || 1 == $_GET['group']) {
        redirectexit('action=permissions');
    }
    ...
    if ($context['group']['id'] > 0) {
        ...
        [$context['group']['name']] = $GLOBALS['xoopsDB']->fetchRow($result);

        $GLOBALS['xoopsDB']->freeRecordSet($result);
    } elseif (-1 == $context['group']['id']) {
    ...
    if ($context['local']) {
        ...
        [$context['board']['name']] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (3 != $context['group']['id'] && !$context['local']) {
        $result = db_query(
            "
			SELECT permission, addDeny
			FROM {$db_prefix}permissions
        ...
        foreach ($tmp['columns'] as $position => $permissionGroups) {
            foreach ($permissionGroups as $permissionGroup => $permissionArray) {
                foreach ($permissionArray['permissions'] as $perm) {
                    ...
                    if ($perm['has_own_any']) {
                        $curPerm['any']['select'] = in_array($perm['id'] . '_any', $permissions[$permissionType]['allowed'], true) ? 'on' : (in_array($perm['id'] . '_any', $permissions[$permissionType]['denied'], true) ? 'denied' : 'off');

                        $curPerm['own']['select'] = in_array($perm['id'] . '_own', $permissions[$permissionType]['allowed'], true) ? 'on' : (in_array($perm['id'] . '_own', $permissions[$permissionType]['denied'], true) ? 'denied' : 'off');
                    } else {
                        $curPerm['select'] = in_array($perm['id'], $permissions[$permissionType]['denied'], true) ? 'denied' : (in_array($perm['id'], $permissions[$permissionType]['allowed'], true) ? 'on' : 'off');
                    }
                }
            }
        }
...
function ModifyMembergroup2()
{
        ...
        foreach ($_POST['perm'] as $perm_type => $perm_array) {
            if (is_array($perm_array)) {
                foreach ($perm_array as $permission => $value) {
                    if ('on' == $value || 'deny' == $value) {
                        // Don't allow people to escalate themselves!

                        if (!empty($context['illegal_permissions']) && in_array($permission, $context['illegal_permissions'], true)) {
                            continue;
                        }

                        $givePerms[$perm_type][] = "$permission', " . ('deny' == $value ? '0' : '1');
                    }
                }
            }
        }
    ...
    if (3 != $_GET['group'] && empty($_GET['boardid'])) {
        db_query(
            "
			DELETE FROM {$db_prefix}permissions
			WHERE ID_GROUP = $_GET[group]
    ...
    if (!empty($_POST['save_settings'])) {
        ...
        elseif (empty($modSettings['permission_enable_by_board']) && !empty($_POST['permission_enable_by_board'])) {
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
                ...
                foreach ($perm as $ID_GROUP => $perm_array) {
                    if (!in_array('moderate_board', $perm_array[1], true)) {
                        if (4 == $row['permission_mode']) {
                            $perm_array[1] = array_diff($perm_array[1], ['post_reply_own', 'post_reply_any']);
                        }

                        if ($row['permission_mode'] >= 3) {
                            $perm_array[1] = array_diff($perm_array[1], ['post_new']);
                        }

                        if ($row['permission_mode'] >= 2) {
                            $perm_array[1] = array_diff($perm_array[1], ['poll_post']);
                        }
                    }
        ...
        if (empty($modSettings['permission_enable_postgroups'])) {
            ...
            db_query(
                "
				DELETE FROM {$db_prefix}permissions
				WHERE ID_GROUP IN (" . implode(', ', $post_groups) . ')',
                __FILE__,
                __LINE__
            );
        ...
        save_inline_permissions(['manage_permissions']);
    }

    init_inline_permissions(['manage_permissions'], [-1]);
...
function setPermissionLevel($level, $group, $board = 'null')
{
    ...
    $groupLevels['global']['standard'] = array_merge(
        $groupLevels['global']['restrict'],
        [
            'view_mlist',
            'karma_edit',
    ...
    $groupLevels['board']['standard'] = array_merge(
        $groupLevels['board']['restrict'],
        [
            'poll_vote',
            'poll_edit_own',
    ...
    $groupLevels['global']['moderator'] = array_merge(
        $groupLevels['global']['standard'],
        [
            'calendar_post',
            'calendar_edit_own',
        ]
    ...
    $groupLevels['board']['moderator'] = array_merge(
        $groupLevels['board']['standard'],
        [
            'make_sticky',
            'poll_edit_any',
            'delete_any',
    ...
    $groupLevels['global']['maintenance'] = array_merge(
        $groupLevels['global']['moderator'],
        [
            'manage_attachments',
            'manage_smileys',
            'manage_boards',
    ...
    $groupLevels['board']['maintenance'] = array_merge(
        $groupLevels['board']['moderator'],
        []
    );
    ...
    $boardLevels['publish'] = array_merge(
        $boardLevels['locked'],
        [
            'post_new',
            'post_reply_own',
...
function loadAllPermissions()
{
    ...
    */
    ...
    foreach ($permissionList as $permissionType => $permissionGroups) {
        ...
        foreach ($permissionGroups as $permissionGroup => $permissionArray) {
            $position = in_array($permissionGroup, $leftPermissionGroups, true) ? 'left' : 'right';

            $context['permissions'][$permissionType]['columns'][$position][$permissionGroup] = [
                'type' => $permissionType,
                'id' => $permissionGroup,
                'name' => &$txt['permissiongroup_' . $permissionGroup],
                'icon' => $txt['permissionicon_' . $permissionGroup] ?? $txt['permissionicon'],
                'help' => $txt['permissionhelp_' . $permissionGroup] ?? '',
                'permissions' => [],
            ];

            foreach ($permissionArray as $perm => $has_own_any) {
                if (isset($context['group']['id']) && -1 == $context['group']['id'] && in_array($perm, $non_guest_permissions, true)) {
                    continue;
                }

                $context['permissions'][$permissionType]['columns'][$position][$permissionGroup]['permissions'][$perm] = [
                    'id' => $perm,
        ...
        foreach ($permissions as $permission) {
            if (!isset($context[$permission][$row['ID_GROUP']])) {
                $context[$permission][$row['ID_GROUP']] = [
                    'id' => $row['ID_GROUP'],
                    'name' => $row['groupName'],
                    'is_postgroup' => -1 != $row['minPosts'],
                    'status' => 'off',
                ];
            }
        }

        $context[$row['permission']][$row['ID_GROUP']]['status'] = empty($row['status']) ? 'deny' : (1 == $row['status'] ? 'on' : 'off');
    }
...
function save_inline_permissions($permissions)
{
    ...
    foreach ($permissions as $permission) {
        ...
        foreach ($_POST[$permission] as $ID_GROUP => $value) {
            if (in_array($value, ['on', 'deny'], true) && (empty($context['illegal_permissions']) || !in_array($permission, $context['illegal_permissions'], true))) {
                $insertRows .= ', (' . (int)$ID_GROUP . ", '$permission', " . ('on' == $value ? '1' : '0') . ')';
            }
        }
    }
