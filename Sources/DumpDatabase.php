...
function DumpDatabase2()
{
    ...
    if (isset($_REQUEST['compress']) && function_exists('gzencode')) {
        ...
        elseif (function_exists('ob_clean') && 0 != ob_get_length()) {
            ob_clean();
        }
    ...
    while (false !== ($tableName = $GLOBALS['xoopsDB']->fetchRow($queryTables))) {
        ...
        if (isset($_GET['struct'])) {
            echo $crlf, '#', $crlf, '# Table structure for table `', $tableName[0], '`', $crlf, '#', $crlf, $crlf, 'DROP TABLE IF EXISTS `', $tableName[0], '`;', $crlf, $crlf, getTableSQLData($tableName[0]), ';', $crlf;
        }

        // How about the data?

        if (!isset($_GET['data']) || 'log_errors' == mb_substr($tableName[0], -10)) {
            continue;
        }
        ...
        echo $crlf, '#', $crlf, '# Dumping data in `', $tableName[0], '`', $crlf, '#', $crlf, $crlf, $get_rows, '# --------------------------------------------------------', $crlf;
    }
...
function getTableContent($tableName)
{
    ...
    $fields = array_keys($GLOBALS['xoopsDB']->fetchArray($result));
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchRow($result))) {
        ...
        for ($j = 0; $j < mysqli_num_fields($result); $j++) {
            ...
            if (!isset($row[$j])) {
                ...
                $field_list[] = "'" . $GLOBALS['xoopsDB']->escape($row[$j]) . "'";
            }
        }
        ...
        if ($current_row > 249 && 0 == $current_row % 250) {
...
function getTableSQLData($tableName)
{
    ...
    while (false !== ($row = @$GLOBALS['xoopsDB']->fetchArray($result))) {
        ...
        $schema_create .= '  ' . $row['Field'] . ' ' . $row['Type'] . ('YES' != $row['Null'] ? ' NOT NULL' : '');
        ...
        if (isset($row['Default'])) {
            ...
            if ('CURRENT_TIMESTAMP' == $row['Default']) {
                ...
                $schema_create .= ' default ' . (is_numeric($row['Default']) ? $row['Default'] : "'" . $GLOBALS['xoopsDB']->escape($row['Default']) . "'");
            }
        }
    ...
    $schema_create = mb_substr($schema_create, 0, -mb_strlen($crlf) - 1);
    ...
    while (false !== ($row = @$GLOBALS['xoopsDB']->fetchArray($result))) {
        ...
        $row['Key_name'] = 'PRIMARY' == $row['Key_name'] ? 'PRIMARY KEY' : (empty($row['Non_unique']) ? 'UNIQUE ' : ('FULLTEXT' == $row['Comment'] || (isset($row['Index_type']) && 'FULLTEXT' == $row['Index_type']) ? 'FULLTEXT ' : 'KEY ')) . $row['Key_name'];
    ...
    $schema_create .= $crlf . ') TYPE=' . ($row['Type'] ?? $row['Engine']) . ('' != $row['Comment'] ? ' COMMENT="' . $row['Comment'] . '"' : '');
