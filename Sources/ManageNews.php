...
function ManageNews()
{
    ...
    if (allowedTo('edit_news')) {
        $context['admin_tabs']['tabs'][] = [
            ...
            'is_selected' => 'editnews' == $_REQUEST['sa'],
        ];
    }
    ...
    if (allowedTo('send_mail')) {
        $context['admin_tabs']['tabs'][] = [
            ...
            'is_selected' => 'mailing' == mb_substr($_REQUEST['sa'], 0, 7),
        ];
    }
...
function EditNews()
{
    ...
    if (!empty($_POST['delete_selection']) && !empty($_POST['remove'])) {
        ...
        foreach ($temp_news as $i => $news) {
            if (in_array($i, $_POST['remove'], true)) {
                unset($temp_news[$i]);
            }
        }
        ...
        updateSettings(['news' => addslashes(implode("\n", $temp_news))]);
...
function SelectMailingMembers()
{
    ...
    [$context['groups'][3]['member_count']] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function ComposeMailing()
{
    ...
    if (!empty($_POST['who']) && in_array(3, $_POST['who'], true)) {
        $request = db_query(
            '
    ...
    if (!empty($_POST['who']) && in_array(0, $_POST['who'], true)) {
        $request = db_query(
            '
...
function SendMailing()
{
    ...
    $context['subject'] = htmlspecialchars(stripslashes($_POST['subject']), ENT_QUOTES | ENT_HTML5);

    $context['message'] = htmlspecialchars(stripslashes($_POST['message']), ENT_QUOTES | ENT_HTML5);
    ...
    if (!empty($_POST['send_html']) && 0 == preg_match('~\<html~i', $_POST['message'])) {
        if (0 == preg_match('~\<body~i', $_POST['message'])) {
...
function ModifyNewsSettings()
{
    ...
    if (!empty($_POST['save_settings'])) {
        ...
        save_inline_permissions(['edit_news', 'send_mail']);
    }

    // Initialize permissions.

    init_inline_permissions(['edit_news', 'send_mail'], [-1]);
}
