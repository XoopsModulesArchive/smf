...
function SplitIndex()
{
    ...
    [$_REQUEST['subname'], $numReplies, $ID_FIRST_MSG] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function SplitSelectTopics()
{
    ...
    if (isset($_REQUEST['xml'])) {
        ...
        if (empty($context['not_selected']['start']) && $GLOBALS['xoopsDB']->getRowsNum($request) <= 1 && 'down' == $_REQUEST['move']) {
            $_REQUEST['move'] = '';
        }
    ...
    if (!empty($_REQUEST['move'])) {
        ...
        if ('reset' == $_REQUEST['move']) {
            $_SESSION['split_selection'][$topic] = [];
        } elseif ('up' == $_REQUEST['move']) {
    ...
    if ($context['selected']['start'] >= $context['selected']['num_messages']) {
        $context['selected']['start'] = $context['selected']['num_messages'] <= $modSettings['defaultMaxMessages'] ? 0 : ($context['selected']['num_messages'] - (0
                                                                                                                                                                  == ($context['selected']['num_messages'] % $modSettings['defaultMaxMessages']) ? $modSettings['defaultMaxMessages'] : ($context['selected']['num_messages'] % $modSettings['defaultMaxMessages'])));
    }

    // Build a page list of the not-selected topics...

    $context['not_selected']['page_index'] = constructPageIndex(
        $scripturl . '?action=splittopics;sa=selectTopics;subname=' . strtr(urlencode($_REQUEST['subname']), ['%' => '%%']) . ';topic=' . $topic . '.%d;start2=' . $context['selected']['start'],
        $context['not_selected']['start'],
        $context['not_selected']['num_messages'],
    ...
    $context['selected']['page_index'] = constructPageIndex(
        $scripturl . '?action=splittopics;sa=selectTopics;subname=' . strtr(urlencode($_REQUEST['subname']), ['%' => '%%']) . ';topic=' . $topic . '.' . $context['not_selected']['start'] . ';start2=%d',
        $context['selected']['start'],
        $context['selected']['num_messages'],
        $modSettings['defaultMaxMessages'],
        true
    );
...
function splitTopic($split1_ID_TOPIC, $splitMessages, $new_subject)
{
    ...
    if ($split1_ID_TOPIC == $topic) {
        ...
        [$ID_BOARD] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    [$split1_firstMsg, $split1_lastMsg, $split1_replies, $isSticky] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    [$split2_firstMsg, $split2_lastMsg, $split2_replies] = $GLOBALS['xoopsDB']->fetchRow($result);
    ...
    db_query(
		...
		VALUES ($ID_BOARD, $split2_firstMem, $split2_lastMem, 0, 0, $split2_replies, $isSticky)",
        __FILE__,
        __LINE__
    );
    ...
    if (!$user_info['is_guest']) {
        db_query(
			...
			VALUES ($modSettings[maxMsgID], $ID_MEMBER, $split2_ID_TOPIC)",
            __FILE__,
            __LINE__
        );
    }
    ...
    logAction('split', ['topic' => $split1_ID_TOPIC, 'new_topic' => $split2_ID_TOPIC]);
...
function MergeIndex()
{
    ...
    [$topiccount] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    [$subject] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    $request = db_query(
		...
		WHERE $user_info[query_see_board]" . (!in_array(0, $merge_boards, true) ? '
...
function MergeExecute($topics = [])
{
    ...
    if (empty($topics) || !is_array($topics) || 1 == count($topics)) {
        fatal_lang_error('merge_need_more_topics');
    }
    ...
    if ($GLOBALS['xoopsDB']->getRowsNum($request) < 2) {
    ...
    $request = db_query(
		...
		WHERE b.ID_BOARD IN (" . implode(', ', $boards) . ")
			AND $user_info[query_see_board]" . (!in_array(0, $merge_boards, true) ? '
			AND b.ID_BOARD IN (' . implode(', ', $merge_boards) . ')' : '') . '
		LIMIT ' . count($boards),
        __FILE__,
        __LINE__
    ...
    if (empty($_REQUEST['sa']) || 'options' == $_REQUEST['sa']) {
        if (count($polls) > 1) {
            $request = db_query(
                "
    ...
    if (!in_array($target_board, $boards, true)) {
        fatal_lang_error('smf232');
    }

    // Determine which poll will survive and which polls won't.

    $target_poll = count($polls) > 1 ? (int)$_POST['poll'] : (1 == count($polls) ? $polls[0] : 0);

    if ($target_poll > 0 && !in_array($target_poll, $polls, true)) {
        fatal_lang_error(1, false);
    }
    ...
    [$first_msg, $last_msg, $num_replies] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    [$member_started] = $GLOBALS['xoopsDB']->fetchRow($request);

    [$member_updated] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    logAction('merge', ['topic' => $ID_TOPIC]);
