        ...
        if ($setString != '') {
            ...
            if (!empty($old_settings)) {
                db_query(
					...
					WHERE ID_THEME != 1
						AND ID_MEMBER = -1
						AND variable IN ('" . implode("', '", $old_settings) . "')",
                    __FILE__,
                    __LINE__
                );
            }
...
function SetThemeSettings()
{
    ...
    foreach ($settings as $setting => $dummy) {
        if (!in_array($setting, ['theme_url', 'theme_dir', 'images_url'])) {
            $settings[$setting] = htmlspecialchars($settings[$setting], ENT_QUOTES | ENT_HTML5);
        }
    }
...
function PickTheme()
{
    ...
    if (isset($_GET['th'])) {
        ...
        if (!isset($_REQUEST['u']) || !allowedTo('admin_forum')) {
            updateMemberData($ID_MEMBER, ['ID_THEME' => (int)$_GET['th']]);
    ...
    elseif ($_REQUEST['u'] == '-1') {
        ...
        [$context['current_theme']] = $GLOBALS['xoopsDB']->fetchRow($request);
        $GLOBALS['xoopsDB']->freeRecordSet($request);
    }
    ...
    if (!empty($modSettings['knownThemes'])) {
        ...
        $request = db_query(
			...
			WHERE variable IN ('name', 'theme_url', 'theme_dir', 'images_url')" . (empty($modSettings['theme_default']) && !allowedTo('admin_forum') ? "
				...
				AND ID_THEME != 1" : '') . '
				AND ID_THEME != 0
			LIMIT ' . substr_count($modSettings['knownThemes'], ',') + 1 * 8,
            __FILE__,
            __LINE__
        );
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
...
function ThemeInstall()
{
    ...
    if (isset($_GET['theme_id'])) {
        ...
        [$theme_name] = $GLOBALS['xoopsDB']->fetchRow($result);
    ...
    if (!empty($_REQUEST['copy']) && $method == 'copy') {
        ...
        $to_copy = ['/style.css', '/index.php', '/index.template.php'];
        ...
        $theme_name = preg_replace(['/\s/', '/\.[\.]+/', '/[^\w_\.\-]/'], ['_', '.', ''], $theme_name);
    ...
    if ($theme_dir != '' && basename($theme_dir) != 'Themes') {
        ...
        $install_info = [
            ...
            'images_url' => $images_url ?? ($boardurl . '/Themes/' . basename($theme_dir) . '/images'),
            'theme_dir'  => $theme_dir,
            'name'       => $theme_name,
        ];
        ...
        if (isset($install_info['based_on'])) {
            if ($install_info['based_on'] == 'default') {
                ...
                $temp    = $GLOBALS['xoopsDB']->fetchArray($request);
                $GLOBALS['xoopsDB']->freeRecordSet($request);
        ...
        [$ID_THEME] = $GLOBALS['xoopsDB']->fetchRow($result);
        $GLOBALS['xoopsDB']->freeRecordSet($result);
...
function SetJavaScript()
{
    ...
    db_query(
		...
		REPLACE INTO {$db_prefix}themes
			(ID_THEME, ID_MEMBER, variable, value)
		VALUES ($settings[theme_id], $ID_MEMBER, SUBSTRING('$_GET[var]', 1, 255), SUBSTRING('" . (is_array($_GET['val']) ? implode(',', $_GET['val']) : $_GET['val']) . "', 1, 65534))",
        __FILE__,
        __LINE__
    );
...
function EditTheme()
{
    ...
    [$theme_dir, $context['theme_id']] = $GLOBALS['xoopsDB']->fetchRow($request);
    $GLOBALS['xoopsDB']->freeRecordSet($request);
    ...
    if (isset($_POST['submit'])) {
        if (checkSession('post', '', false) == '') {
            ...
            $_POST['entire_file'] = rtrim(strtr(stripslashes($_POST['entire_file']), ["\r" => '', '   ' => "\t"]));
            ...
            if (substr($_REQUEST['filename'], -13) == '.template.php' && is_writable($theme_dir) && @ini_get('display_errors')) {
                ...
                [$theme_url] = $GLOBALS['xoopsDB']->fetchRow($request);
                $GLOBALS['xoopsDB']->freeRecordSet($request);
            ...
            $context['entire_file'] = htmlspecialchars(stripslashes($_POST['entire_file']), ENT_QUOTES | ENT_HTML5);
    ...
    $context['allow_save_filename'] = strtr($theme_dir . '/' . $_REQUEST['filename'], [$boarddir => '...']);
    $context['edit_filename']       = htmlspecialchars($_REQUEST['filename'], ENT_QUOTES | ENT_HTML5);

    if (substr($_REQUEST['filename'], -4) == '.css') {
        $context['sub_template'] = 'edit_style';

        $context['entire_file'] = htmlspecialchars(strtr(file_get_contents($theme_dir . '/' . $_REQUEST['filename']), ["\t" => '   ']), ENT_QUOTES | ENT_HTML5);
        ...
        $j                     = 0;
        $context['file_parts'] = [['lines' => 0, 'line' => 1, 'data' => '']];
        for ($i = 0, $n = count($file_data); $i < $n; $i++) {
            if (isset($file_data[$i + 1]) && substr($file_data[$i + 1], 0, 9) == 'function ') {
                ...
                $context['file_parts'][++$j] = ['lines' => 0, 'line' => $i + 1, 'data' => ''];
            }

            $context['file_parts'][$j]['lines']++;
            $context['file_parts'][$j]['data'] .= htmlspecialchars(strtr($file_data[$i], ["\t" => '   ']), ENT_QUOTES | ENT_HTML5);
        }

        $context['entire_file'] = htmlspecialchars(strtr(implode('', $file_data), ["\t" => '   ']), ENT_QUOTES | ENT_HTML5);
    } else {
        $context['sub_template'] = 'edit_file';
...
function CopyTemplate()
{
    ...
    [$theme_dir, $context['theme_id'], $base_theme_dir] = $GLOBALS['xoopsDB']->fetchRow($request);
    $GLOBALS['xoopsDB']->freeRecordSet($request);
    ...
    foreach ($templates as $template) {
        $context['available_templates'][$template] = [
    ...
    $context['available_language_files'] = [];
    foreach ($lang_files as $file) {
        $context['available_language_files'][$file] = [
            'filename'       => $file . '.php',
            'value'          => $file,
            'already_exists' => false,
...
function convert_template($output_dir, $old_template = '')
{
    ...
    $old_template = strtr($old_template, ['\'' => '\\\'']);
...
function phpcodefix($string)
{
    ...
    $string = strtr(stripslashes($string), ['\\\'' => '\'']);
