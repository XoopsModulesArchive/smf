...
function Who()
{
    ...
    [$totalMembers] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        $context['members'][$row['session']] = [
            ...
            'time' => strtr(timeformat($row['logTime']), [$txt['smf10'] => '', $txt['smf10b'] => '']),
            'timestamp' => forum_time(true, $row['logTime']),
            'query' => $actions,
            'is_hidden' => 0 == $row['showOnline'],
...
function determineActions($urls)
{
    ...
    foreach ($url_list as $k => $url) {
        ...
        if (!isset($actions['action']) || 'display' == $actions['action']) {
        ...
        elseif ('' == $actions['action']) {
            ...
            if ('profile' == $actions['action'] || 'profile2' == $actions['action']) {
                // Whose?  Their own?
                ...
                $profile_ids[(int)$actions['u']][$k] = 'profile' == $actions['action'] ? $txt['who_viewprofile'] : $txt['who_profile'];
            } elseif (('post' == $actions['action'] || 'post2' == $actions['action']) && empty($actions['topic']) && isset($actions['board'])) {
                $data[$k] = $txt['who_hidden'];
            ...
            elseif (isset($txt['whotopic_' . $actions['action']])) {
                ...
                [$ID_TOPIC, $subject] = $GLOBALS['xoopsDB']->fetchRow($result);
