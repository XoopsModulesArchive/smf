...
function Register()
{
    ...
    if (!empty($modSettings['registration_method']) && 3 == $modSettings['registration_method']) {
    ...
    $context['visual_verification'] = empty($modSettings['disable_visual_verification']) || 1 != $modSettings['disable_visual_verification'];
...
function Register2()
{
    ...
    if (!empty($modSettings['requireAgreement']) && (empty($_POST['regagree']) || 'no' == $_POST['regagree'])) {
        redirectexit();
    }
    ...
    if (!empty($modSettings['coppaAge']) && empty($modSettings['coppaType']) && !isset($_POST['skip_coppa'])) {
        ...
        fatal_lang_error('under_age_registration_prohibited', false, [$modSettings['coppaAge']]);
    }

    // Check whether the visual verification code was entered correctly.

    if ((empty($modSettings['disable_visual_verification']) || 1 != $modSettings['disable_visual_verification']) && (empty($_REQUEST['visual_verification_code']) || mb_strtoupper($_REQUEST['visual_verification_code']) !== $_SESSION['visual_verification_code'])) {
    ...
    if (isset($_POST['MSN']) && 0 != preg_match('~^[0-9A-Za-z=_+\-/][0-9A-Za-z=_\'+\-/\.]*@[\w\-]+(\.[\w\-]+)*(\.[\w]{2,6})$~', $_POST['MSN'])) {
        $profile_strings[] = 'MSN';
    }
    ...
    $regOptions = [
        ...
        'require' => !empty($modSettings['coppaAge']) && !isset($_POST['skip_coppa']) ? 'coppa' : (empty($modSettings['registration_method']) ? 'nothing' : (1 == $modSettings['registration_method'] ? 'activation' : 'approval')),
        'extra_register_vars' => [],
        'theme_vars' => [],
    ];
    ...
    elseif (!empty($modSettings['registration_method'])) {
        ...
        $context += [
            'page_title' => &$txt[97],
            'sub_template' => 'after',
            'description' => 2 == $modSettings['registration_method'] ? $txt['approval_after_registration'] : $txt['activate_after_registration'],
        ];
    } else {
        setLoginCookie(60 * $modSettings['cookieTime'], $memberID, sha1(sha1(mb_strtolower($regOptions['username']) . $regOptions['password']) . mb_substr($regOptions['register_vars']['passwordSalt'], 1, -1)));
...
function Activate()
{
    ...
    if (empty($_REQUEST['u']) && empty($_POST['user'])) {
        ...
        $context['can_activate'] = empty($modSettings['registration_method']) || 1 == $modSettings['registration_method'];

        $context['default_username'] = $_GET['user'] ?? '';
    ...
    if (isset($_POST['new_email'], $_REQUEST['passwd']) && sha1(mb_strtolower($row['memberName']) . $_REQUEST['passwd']) == $row['passwd']) {
        if (empty($modSettings['registration_method']) || 3 == $modSettings['registration_method']) {
            fatal_lang_error(1);
        }
        ...
        if (0 == preg_match('~^[0-9A-Za-z=_+\-/][0-9A-Za-z=_\'+\-/\.]*@[\w\-]+(\.[\w\-]+)*(\.[\w]{2,6})$~', stripslashes($_POST['new_email']))) {
            fatal_error(sprintf($txt[500], htmlspecialchars($_POST['new_email'], ENT_QUOTES | ENT_HTML5)), false);
        }
        ...
        if (0 != $GLOBALS['xoopsDB']->getRowsNum($request)) {
            fatal_error(sprintf($txt[730], htmlspecialchars($_POST['new_email'], ENT_QUOTES | ENT_HTML5)), false);
        }

        $GLOBALS['xoopsDB']->freeRecordSet($request);

        updateMemberData($row['ID_MEMBER'], ['emailAddress' => "'$_POST[new_email]'"]);
    ...
    if (!empty($_REQUEST['sa']) && 'resend' == $_REQUEST['sa'] && (0 == $row['is_activated'] || 2 == $row['is_activated']) && (!isset($_REQUEST['code']) || '' == $_REQUEST['code'])) {
        ...
        sendmail(
            ...
            sprintf(
                $txt[empty($modSettings['registration_method']) || 1 == $modSettings['registration_method'] ? 'resend_activate_message' : 'resend_pending_message'],
                $row['realName'],
                $row['memberName'],
                $row['validation_code'],
...
function CoppaForm()
{
    ...
    [$username] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (isset($_GET['form'])) {
        ...
        if (!isset($_GET['dl'])) {
            ...
            $context['coppa_body'] = str_replace(['{PARENT_NAME}', '{CHILD_NAME}', '{USER_NAME}'], [$context['ul'], $context['ul'], $username], $txt['coppa_form_body']);
        } // Downloading.
            ...
            $data = $context['forum_contacts'] . (string)$crlf . $txt['coppa_form_address'] . ":$crlf" . $txt['coppa_form_date'] . ":$crlf$crlf$crlf" . $txt['coppa_form_body'];
...
function VerificationCode()
{
    ...
    elseif (empty($_REQUEST['format'])) {
        ...
        if (in_array('gd', get_loaded_extensions(), true) && !showCodeImage($_SESSION['visual_verification_code'])) {
        ...
        elseif (isset($_REQUEST['letter'])) {
            ...
            if ($_REQUEST['letter'] > 0 && $_REQUEST['letter'] <= mb_strlen($_SESSION['visual_verification_code']) && !showLetterImage(mb_strtolower($_SESSION['visual_verification_code'][$_REQUEST['letter'] - 1]))) {
                header('HTTP/1.1 400 Bad Request');
            }
