...
function ManageBoards()
{
    ...
    $subActions = [
        ...
        'move' => ['ManageBoardsMain', 'manage_boards'],
    ...
    if (allowedTo('manage_boards')) {
        $context['admin_tabs']['tabs']['modify_boards'] = [
            'title' => $txt['boardsEdit'],
            'description' => $txt[677],
            'href' => $scripturl . '?action=manageboards',
            'is_selected' => 'newcat' != $_REQUEST['sa'] && 'settings' != $_REQUEST['sa'],
        ];
        ...
        $context['admin_tabs']['tabs']['add_cat'] = [
            ...
            'is_selected' => 'newcat' == $_REQUEST['sa'],
...
function ManageBoardsMain()
{
    ...
    if (isset($_REQUEST['sa']) && 'move' == $_REQUEST['sa'] && in_array($_REQUEST['move_to'], ['child', 'before', 'after', 'top'], true)) {
        checkSession('get');

        if ('top' === $_REQUEST['move_to']) {
            $boardOptions = [
    ...
    if (!empty($context['move_board'])) {
        $context['move_title'] = sprintf($txt['mboards_select_destination'], htmlspecialchars($boards[$context['move_board']]['name'], ENT_QUOTES | ENT_HTML5));
        ...
        foreach ($cat_tree as $catid => $tree) {
            ...
            foreach ($boardList[$catid] as $boardid) {
                if (!isset($context['categories'][$catid]['move_link'])) {
                    $context['categories'][$catid]['move_link'] = [
                        'child_level' => 0,
                        'label' => $txt['mboards_order_before'] . ' \'' . htmlspecialchars($boards[$boardid]['name'], ENT_QUOTES | ENT_HTML5) . '\'',
                ...
                if (!$context['categories'][$catid]['boards'][$boardid]['move']) {
                    $context['categories'][$catid]['boards'][$boardid]['move_links'] = [
                        [
                            'child_level' => $boards[$boardid]['level'],
                            'label' => $txt['mboards_order_after'] . '\'' . htmlspecialchars($boards[$boardid]['name'], ENT_QUOTES | ENT_HTML5) . '\'',
                        ...
                        [
                            ...
                            'label' => $txt['mboards_order_child_of'] . ' \'' . htmlspecialchars($boards[$boardid]['name'], ENT_QUOTES | ENT_HTML5) . '\'',
                ...
                if (1 == $difference && !empty($context['categories'][$catid]['boards'][$prev_board]['move_links'])) {
                    $stack[] = array_shift($context['categories'][$catid]['boards'][$prev_board]['move_links']);
                } elseif ($difference < 0) {
            ...
            if (empty($boardList[$catid])) {
                $context['categories'][$catid]['move_link'] = [
                    'child_level' => 0,
                    'label' => $txt['mboards_order_before'] . ' \'' . htmlspecialchars($tree['node']['name'], ENT_QUOTES | ENT_HTML5) . '\'',
...
function EditCategory()
{
    ...
    if ('newcat' == $_REQUEST['sa']) {
        $context['category'] = [
            'id' => 0,
            'name' => $txt['mboards_new_cat_name'],
            'editable_name' => htmlspecialchars($txt['mboards_new_cat_name'], ENT_QUOTES | ENT_HTML5),
            'can_collapse' => true,
            'is_new' => true,
            'is_empty' => true,
        ];
    ...
    elseif (!isset($cat_tree[$_REQUEST['cat']])) {
        ...
        $context['category'] = [
            ...
            'editable_name' => htmlspecialchars($cat_tree[$_REQUEST['cat']]['node']['name'], ENT_QUOTES | ENT_HTML5),
    ...
    if (!isset($_REQUEST['delete'])) {
        ...
        $context['page_title'] = 'newcat' == $_REQUEST['sa'] ? $txt['mboards_new_cat_name'] : $txt['catEdit'];
    } else {
...
function EditCategory2()
{
    ...
    elseif (isset($_POST['delete'])) {
        ...
        if (isset($_POST['delete_action']) && 1 == $_POST['delete_action']) {
...
function EditBoard()
{
    ...
    if ('newboard' == $_REQUEST['sa']) {
        ...
        $context['board']['name'] = htmlspecialchars($context['board']['name'], ENT_QUOTES | ENT_HTML5);

        $context['board']['description'] = htmlspecialchars($context['board']['description'], ENT_QUOTES | ENT_HTML5);
    ...
    $context['groups'] = [
        -1 => [
            ...
            'checked' => in_array('-1', $curBoard['memberGroups'], true),
            'is_post_group' => false,
        ],
        0 => [
            'id' => '0',
            'name' => $txt['parent_members_only'],
            'checked' => in_array('0', $curBoard['memberGroups'], true),
            'is_post_group' => false,
        ],
    ];
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        if ('newboard' == $_REQUEST['sa'] && -1 == $row['minPosts']) {
            $curBoard['memberGroups'][] = $row['ID_GROUP'];
        }

        $context['groups'][(int)$row['ID_GROUP']] = [
            'id' => $row['ID_GROUP'],
            'name' => trim($row['groupName']),
            'checked' => in_array($row['ID_GROUP'], $curBoard['memberGroups'], true),
    ...
    if (!empty($_REQUEST['boardid'])) {
        ...
        foreach ($context['board_order'] as $board) {
            if (false === $board['is_child'] && false === $board['selected']) {
...
function EditBoard2()
{
    ...
    if (isset($_POST['edit']) || isset($_POST['add'])) {
        ...
        elseif (!empty($_POST['placement']) && !empty($_POST['board_order'])) {
            if (!in_array($_POST['placement'], ['before', 'after', 'child'], true)) {
...
function ModifyCat()
{
    ...
    $_POST['id'] = mb_substr($_POST['id'][1], 0, 3);
    ...
    [$cat] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function EditBoardSettings()
{
    ...
    if (!empty($_POST['save_settings'])) {
        ...
        save_inline_permissions(['manage_boards']);
    }
    ...
    init_inline_permissions(['manage_boards'], [-1]);
}
