...
function ReportsMain()
{
    ...
    if (isset($_REQUEST['st']) && isset($reportTemplates[$_REQUEST['st']])) {
        ...
        if (null !== $reportTemplates[$_REQUEST['st']]['layers']) {
...
function BoardReport()
{
    ...
    $groups = [-1 => $txt[28], 0 => $txt['full_member']];
    ...
    $request = db_query(
		...
		FROM {$db_prefix}boards AS b
			...
			LEFT JOIN {$db_prefix}themes AS th ON (th.ID_THEME = b.ID_THEME AND variable = 'name')",
        __FILE__,
        __LINE__
    );

    $boards = [0 => ['name' => $txt['global_boards'], 'local_perms' => 1]];
...
function BoardPermissionsReport()
{
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        $boards[$row['ID_BOARD']] = [
            'name' => $row['name'],
            'local_perms' => !empty($modSettings['permission_enable_by_board']) && 1 == $row['permission_mode'],
    ...
    if (!isset($_REQUEST['groups']) || in_array(-1, $_REQUEST['groups'], true) || in_array(0, $_REQUEST['groups'], true)) {
        $memberGroups = ['col' => '', -1 => $txt['membergroups_guests'], 0 => $txt['membergroups_members']];
    } else {
        $memberGroups = ['col' => ''];
    }
    ...
    foreach ($board_permissions as $board => $groups) {
        ...
        if (0 != $board && !$boards[$board]['local_perms']) {
            continue;
        }
        ...
        foreach ($permissions as $ID_PERM => $perm_info) {
            ...
            $identicalGlobal = 0 == $board ? false : true;
            ...
            foreach ($memberGroups as $ID_GROUP => $name) {
                ...
                if (isset($group_permissions[$ID_PERM])) {
                    ...
                    if (isset($board_permissions[0][$ID_GROUP][$ID_PERM]) && 'x' != $board_permissions[0][$ID_GROUP][$ID_PERM]) {
                        $identicalGlobal = false;
                    }
                }
    ...
    foreach ($boards as $id => $board) {
        if (0 != $id && !empty($board['permission_mode']) && 'normal' != $board['permission_mode']) {
            newTable($board['name'], 'x', 'top');

            // Just add a description of the permission type.

            addData(['<b>' . $txt['board_perms_group_' . $board['permission_mode']] . '</b>']);
        }
    }
}
...
function MemberGroupsReport()
{
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        $boards[$row['ID_BOARD']] = [
            ...
            'name' => $row['name'],
            'local_perms' => !empty($modSettings['permission_enable_by_board']) && 1 == $row['permission_mode'],
            'permission_mode' => empty($modSettings['permission_enable_by_board']) ? (empty($row['permission_mode']) ? $txt['permission_mode_normal'] : (2 == $row['permission_mode'] ? $txt['permission_mode_no_polls'] : (3
                                                                                                                                                                                                                            == $row['permission_mode'] ? $txt['permission_mode_reply_only'] : $txt['permission_mode_read_only']))) : $txt['permission_mode_normal'],
            'groups' => array_merge([1, 3], explode(',', $row['memberGroups'])),
        ];
    }
    ...
    foreach ($rows as $row) {
        ...
        $group = [
            ...
            'minPosts' => -1 == $row['minPosts'] ? 'N/A' : $row['minPosts'],
        ...
        foreach ($boards as $board) {
            $group['board_' . $board['id']] = in_array($row['ID_GROUP'], $board['groups'], true) ? '<span style="color: darkgreen;">'
                                                                                             . (empty($modSettings['permission_enable_by_board']) ? (empty($row['can_moderate']) ? $board['permission_mode'] : $txt['permission_mode_normal']) : $txt['board_perms_allow'])
                                                                                             . '</span>' : 'x';
        }
...
function StaffReport()
{
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        if (in_array($row['ID_MEMBER'], $global_mods, true)) {
...
function newTable($title = '', $defaultValue = '', $shading = 'all', $width_normal = 'auto', $align_normal = 'center', $width_shaded = 'auto', $align_shaded = 'auto')
{
    ...
    $context['tables'][$context['table_count']] = [
        ...
        'shading' => [
            'left' => 'all' == $shading || 'left' == $shading,
            'top' => 'all' == $shading || 'top' == $shading,
        ],
        'width' => [
            'normal' => $width_normal,
            'shaded' => $width_shaded,
...
function addData($inc_data, $custom_table = null)
{
    ...
    if (null !== $custom_table && !isset($context['tables'][$custom_table])) {
        return false;
    } elseif (null !== $custom_table) {
        $table = $custom_table;
    } else {
    ...
    if (!empty($context['keys'])) {
        ...
        foreach ($context['keys'] as $key => $dummy) {
            $data[$key] = [
                ...
                'seperator' => '#sep#' == mb_substr($key, 0, 5) ? true : false,
            ];
        }
    } else {
        $data = $inc_data;
    ...
    if (empty($context['key_method']) || 'rows' == $context['key_method']) {
        // Add the data!
...
function addSeperator($title = '', $custom_table = null)
{
    ...
    if (null !== $custom_table && !isset($context['tables'][$table])) {
        return false;
    } elseif (null !== $custom_table) {
        $table = $custom_table;
    } else {
...
function finishTables()
{
    ...
    foreach ($context['tables'] as $id => $table) {
        ...
        if ($table['shading']['left'] && 'auto' != $table['width']['shaded'] && 'auto' != $table['width']['normal']) {
            $context['tables'][$id]['max_width'] = $table['width']['shaded'] + ($context['tables'][$id]['column_count'] - 1) * $table['width']['normal'];
        } elseif ('auto' != $table['width']['normal']) {
...
function setKeys($method = 'rows', $keys = [], $reverse = false)
{
    ...
    $context['key_method'] = 'rows' == $method ? 'rows' : 'cols';
}
