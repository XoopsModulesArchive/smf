...
function ViewErrorLog()
{
    ...
    if (isset($_GET['value'], $_GET['filter']) && isset($filters[$_GET['filter']])) {
        $filter = [
            ...
            'value' => [
                'sql' => addslashes('message' == $_GET['filter'] || 'url' == $_GET['filter'] ? base64_decode(strtr($_GET['value'], [' ' => '+']), true) : addcslashes($_GET['value'], '\\_%')),
            ],
    ...
    [$num_errors] = $GLOBALS['xoopsDB']->fetchRow($result);

    $GLOBALS['xoopsDB']->freeRecordSet($result);

    // If this filter is empty...

    if (0 == $num_errors && isset($filter)) {
    ...
    $context['page_index'] = constructPageIndex($scripturl . '?action=viewErrorLog' . ('down' == $context['sort_direction'] ? ';desc' : '') . (isset($filter) ? $filter['href'] : ''), $_GET['start'], $num_errors, $modSettings['defaultMaxMessages']);
    ...
    $request = db_query(
		...
		FROM {$db_prefix}log_errors" . (isset($filter) ? "
		WHERE $filter[variable] LIKE '{$filter['value']['sql']}'" : '') . '
		ORDER BY ID_ERROR ' . ('down' == $context['sort_direction'] ? 'DESC' : '') . "
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        $context['errors'][] = [
            ...
            'url' => [
                'html' => htmlspecialchars($scripturl . $row['url'], ENT_QUOTES | ENT_HTML5),
    ...
    if (isset($filter)) {
        ...
        if ('ID_MEMBER' == $filter['variable']) {
            $id = $filter['value']['sql'];

            loadMemberData($id, false, 'minimal');

            $context['filter']['value']['html'] = '<a href="' . $scripturl . '?action=profile;u=' . $id . '">' . $user_profile[$id]['realName'] . '</a>';
        } elseif ('url' == $filter['variable']) {
            $context['filter']['value']['html'] = "'" . htmlspecialchars($scripturl . stripslashes($filter['value']['sql']), ENT_QUOTES | ENT_HTML5) . "'";
        } elseif ('message' == $filter['variable']) {
            $context['filter']['value']['html'] = "'" . strtr(htmlspecialchars(stripslashes($filter['value']['sql']), ENT_QUOTES | ENT_HTML5), ["\n" => '<br>', '&lt;br /&gt;' => '<br>', "\t" => '&nbsp;&nbsp;&nbsp;', '\\_' => '_', '\\%' => '%', '\\\\' => '\\']) . "'";
...
function deleteErrors()
{
    ...
    elseif (!empty($_POST['delete'])) {
        db_query(
            "
			DELETE FROM {$db_prefix}log_errors
			WHERE ID_ERROR IN (" . implode(',', array_unique($_POST['delete'])) . ')',
            __FILE__,
            __LINE__
        );
