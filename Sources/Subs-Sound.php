...
function createWaveFile($word)
{
    ...
    for ($i = 0, $iMax = mb_strlen($word); $i < $iMax; $i++) {
        $sound_letter = implode('', file($settings['default_theme_dir'] . '/fonts/sound/' . $word[$i] . '.' . $sound_language . '.wav'));

        if (false === mb_strpos($sound_letter, 'data')) {
            return false;
        }

        $sound_word .= mb_substr($sound_letter, mb_strpos($sound_letter, 'data') + 8) . str_repeat(chr(0x80), mt_rand(700, 710) * 8);
    }
    ...
    $data_size = mb_strlen($sound_word);

    $file_size = $data_size + 0x24;

    // Add a little randomness.

    for ($i = 0; $i < $data_size; $i += mt_rand(1, 10)) {
        $sound_word[$i] = chr(ord($sound_word[$i]) + mt_rand(-1, 1));
    }
