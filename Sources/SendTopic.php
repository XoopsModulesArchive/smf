...
function SendTopic()
{
    ...
    if ('_' == $_POST['y_name'] || !isset($_POST['y_name']) || '' == $_POST['y_name']) {
    ...
    if (0 == preg_match('~^[0-9A-Za-z=_+\-/][0-9A-Za-z=_\'+\-/\.]*@[\w\-]+(\.[\w\-]+)*(\.[\w]{2,6})$~', stripslashes($_POST['y_email']))) {
        fatal_lang_error(243, false);
    }

    // The receiver should be valid to.

    if ('_' == $_POST['r_name'] || !isset($_POST['r_name']) || '' == $_POST['r_name']) {
        fatal_lang_error(75, false);
    }
    ...
    if (0 == preg_match('~^[0-9A-Za-z=_+\-/][0-9A-Za-z=_\'+\-/\.]*@[\w\-]+(\.[\w\-]+)*(\.[\w]{2,6})$~', stripslashes($_POST['r_email']))) {
        fatal_lang_error(243, false);
    }
    ...
    sendmail(
        $_POST['r_email'],
        $txt[118] . ': ' . $row['subject'] . ' (' . $txt[318] . ' ' . $_POST['y_name'] . ')',
...
function ReportToModerator()
{
    ...
    [$_GET['msg'], $member, $starter] = $GLOBALS['xoopsDB']->fetchRow($result);
    ...
    if ($member == $ID_MEMBER && (allowedTo(['modify_own', 'modify_any']) || ($ID_MEMBER == $starter && allowedTo('modify_replies')))) {
...
function ReportToModerator2()
{
    ...
    [$subject, $member, $posterName, $realName] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        sendmail(
            $row['emailAddress'],
            $txt['rtm3'] . ': ' . $subject . ' ' . $txt['rtm4'] . ' ' . $posterName,
            sprintf($txt['rtm_email1'], $subject)
            . ' '
            . $posterName
            . ' '
            . $txt['rtm_email2']
            . ' '
            . (empty($ID_MEMBER) ? $txt['guest'] . ' (' . $user_info['ip'] . ')' : $reporterName)
            . ' '
            . $txt['rtm_email3']
            . ":\n\n"
            . $scripturl
