...
function Admin()
{
    ...
    if (isset($_GET['area']) && 'copyright' == $_GET['area']) {
...
<div style="margin-top: 1ex;">And for anyone we may have missed, thank you!</div>';
    ...
    if (!empty($modSettings['copy_settings']) || !empty($modSettings['copyright_key'])) {
        ...
        if (empty($_SESSION['copy_expire'])) {
            [$key, $expires] = explode(',', $modSettings['copy_settings']);
            ...
            if ($fp) {
                ...
                $return_data = mb_substr($return_data, mb_strpos($return_data, 'STARTCOPY') + 9);

                $return_data = trim(mb_substr($return_data, 0, mb_strpos($return_data, 'ENDCOPY')));

                if ('void' != $return_data) {
                    [$_SESSION['copy_expire'], $modSettings['copyright_key']] = explode('|', $return_data);

                    $_SESSION['copy_key'] = $key;

                    $modSettings['copy_settings'] = $key . ',' . (int)$return_data;

                    updateSettings(['copy_settings' => $modSettings['copy_settings'], 'copyright_key' => $modSettings['copyright_key']]);
                } else {
    ...
    $context['current_versions'] = [
        'php' => ['title' => $txt['support_versions_php'], 'version' => PHP_VERSION],
        'mysql' => ['title' => $txt['support_versions_mysql'], 'version' => ''],
        'server' => ['title' => $txt['support_versions_server'], 'version' => $_SERVER['SERVER_SOFTWARE']],
    ];
    ...
    if (function_exists('gd_info')) {
        ...
        $context['current_versions']['gd'] = ['title' => $txt['support_versions_gd'], 'version' => $temp['GD Version']];
    }
    ...
    [$context['current_versions']['mysql']['version']] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (defined('MMCACHE_VERSION')) {
        $context['current_versions']['mmcache'] = ['title' => 'Turck MMCache', 'version' => MMCACHE_VERSION];
    }

    if (defined('EACCELERATOR_VERSION')) {
        $context['current_versions']['eaccelerator'] = ['title' => 'eAccelerator', 'version' => EACCELERATOR_VERSION];
    }

    if (isset($_PHPA)) {
        $context['current_versions']['phpa'] = ['title' => 'ionCube PHP-Accelerator', 'version' => $_PHPA['VERSION']];
    }

    if (extension_loaded('apc')) {
        $context['current_versions']['apc'] = ['title' => 'Alternative PHP Cache', 'version' => phpversion('apc')];
    }
    ...
    $quick_admin_tasks = [
        ...
        ['admin_forum', 'theme;sa=admin;sesc=' . $context['session_id'], 'theme_admin', 'theme_admin_info'],
    ...
    if (1 == count($context['quick_admin_tasks']) % 2) {
        $context['quick_admin_tasks'][] = [
            'href' => '',
            'link' => '',
            'title' => '',
    ...
    } elseif (0 != count($context['quick_admin_tasks'])) {
...
function OptimizeTables()
{
    ...
    if (!$result) {
        ...
        while (false !== ($table = $GLOBALS['xoopsDB']->fetchArray($result))) {
            $tables[] = $table + ['table_name' => mysql_tablename($result, $i++)];
        }
    ...
    foreach ($tables as $table) {
        ...
        if (!isset($row['Msg_text']) || false === mb_strpos($row['Msg_text'], 'already') || !isset($table['Data_free']) || 0 != $table['Data_free']) {
            $context['optimized_tables'][] = [
...
function Maintenance()
{
    ...
    } elseif (isset($_GET['sa']) && 'destroy' == $_GET['sa']) {
        // Oh noes!
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
        ...
        $context['categories'][$row['ID_CAT']]['boards'][] = [
            'id' => $row['ID_BOARD'],
            'name' => $row['name'],
    ...
    $context['convert_utf8'] = (!isset($db_character_set) || 'utf8' !== $db_character_set || empty($modSettings['global_character_set']) || 'UTF-8' !== $modSettings['global_character_set']) && version_compare('4.1.2', preg_replace('~\-.+?$~', '', $GLOBALS['xoopsDB']->getServerVersion())) <= 0;

    $context['convert_entities'] = isset($db_character_set, $modSettings['global_character_set']) && 'utf8' === $db_character_set && 'UTF-8' === $modSettings['global_character_set'];
...
function AdminBoardRecount()
{
    ...
    [$max_topics] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if ($_REQUEST['step'] <= 2) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            updateMemberData($row['ID_MEMBER'], ['instantMessages' => $row['realNum']]);
        }
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            updateMemberData($row['ID_MEMBER'], ['unreadMessages' => $row['realNum']]);
        }
...
function VersionDetail()
{
    ...
    $context['file_versions'] = [];

    $context['default_template_versions'] = [];

    $context['template_versions'] = [];

    $context['default_language_versions'] = [];
    ...
    if (1 == preg_match('~\*\s*Software\s+Version:\s+SMF\s+(.+?)[\s]{2}~i', $header, $match)) {
    ...
    while ($entry = $Sources_dir->read()) {
        if ('.php' == mb_substr($entry, -4) && !is_dir($sourcedir . '/' . $entry) && 'index.php' != $entry) {
    ...
    foreach ($directories as $type => $dirname) {
        ...
        while ($entry = $This_dir->read()) {
            if ('template.php' == mb_substr($entry, -12) && !is_dir($dirname . '/' . $entry)) {
                ...
                if (1 == preg_match('~(?://|/\*)\s*Version:\s+(.+?);\s*' . preg_quote(basename($entry, '.template.php'), '~') . '(?:[\s]{2}|\*/)~i', $header, $match)) {
    ...
    while ($entry = $This_dir->read()) {
        if ('.php' == mb_substr($entry, -4) && 'index.php' != $entry && !is_dir($lang_dir . '/' . $entry)) {
            // Read the first 768 bytes from the file.... enough for the header.
            ...
            [$name, $language] = explode('.', $entry);

            // Look for the version comment in the file header.

            if (1 == preg_match('~(?://|/\*)\s*Version:\s+(.+?);\s*' . preg_quote($name, '~') . '(?:[\s]{2}|\*/)~i', $header, $match)) {
...
function CleanupPermissions()
{
    ...
    if (!isset($_REQUEST['perm_type']) || !in_array($_REQUEST['perm_type'], ['free', 'restrictive', 'standard'], true)) {
    ...
    if (isset($package_ftp)) {
        $package_ftp->chmod(strtr($boarddir . '/Settings.php', [$_SESSION['pack_ftp']['root'] => '']), 0755);
    }
    ...
    @chmod($boarddir, 'free' == $_REQUEST['perm_type'] ? $suexec_fix : 0755);

    if (isset($package_ftp)) {
        $package_ftp->chmod(strtr($boarddir . '/.', [$_SESSION['pack_ftp']['root'] => '']), 'free' == $_REQUEST['perm_type'] ? $suexec_fix : 0755);
    }

    $dirs = ['' => $boarddir];

    if (mb_substr($sourcedir, 0, mb_strlen($boarddir)) != $boarddir) {
        $dirs['/Sources'] = $sourcedir;
    }

    if (mb_substr($modSettings['attachmentUploadDir'], 0, mb_strlen($boarddir)) != $boarddir) {
        $dirs['/attachments'] = $modSettings['attachmentUploadDir'];
    }

    if (mb_substr($modSettings['smileys_dir'], 0, mb_strlen($boarddir)) != $boarddir) {
        $dirs['/Smileys'] = $modSettings['smileys_dir'];
    }

    if (mb_substr($modSettings['avatar_directory'], 0, mb_strlen($boarddir)) != $boarddir) {
        $dirs['/avatars'] = $modSettings['avatar_directory'];
    }

    if (isset($modSettings['custom_avatar_dir']) && mb_substr($modSettings['custom_avatar_dir'], 0, mb_strlen($boarddir)) != $boarddir) {
    ...
    while (count($dirs) > 0) {
        ...
        while ($entry = $dir->read()) {
            ...
            if ('free' == $_REQUEST['perm_type'] || in_array($alias . '/' . $entry, $special_files[$_REQUEST['perm_type']], true)) {
            ...
            if (@is_dir($filename) && !in_array($filename, $done_dirs, true)) {
...
function updateSettingsFile($config_vars)
{
    ...
    if (false !== mb_strpos($settingsArray, "\n")) {
        $settingsArray = explode("\n", $settingsArray);
    } elseif (false !== mb_strpos($settingsArray, "\r")) {
    ...
    if (1 == count($config_vars) && isset($config_vars['db_last_error'])) {
        $temp = trim(implode("\n", $settingsArray));

        if ('<?php' != mb_substr($temp, 0, 5) || mb_substr($temp, -2) != '?' . '>') {
            return;
        }

        if (false === mb_strpos($temp, 'sourcedir') || false === mb_strpos($temp, 'boarddir') || false === mb_strpos($temp, 'cookiename')) {
            return;
        }
    }
    ...
    for ($i = 0, $n = count($settingsArray); $i < $n; $i++) {
        ...
        if (mb_substr(trim($settingsArray[$i]), 0, 2) == '?' . '>') {
            $end = $i;
        }
    }
    ...
    $fp = @fopen($boarddir . '/Settings.php', 'wb');
    ...
    if ($fp) {
        ...
        $fp = fopen($boarddir . '/Settings.php', 'r+b');
...
function ConvertUtf8()
{
    ...
    if (!isset($_POST['proceed'])) {
        ...
        if (version_compare('4.1.2', preg_replace('~\-.+?$~', '', $GLOBALS['xoopsDB']->getServerVersion())) > 0) {
        ...
        $column_info = $GLOBALS['xoopsDB']->fetchArray($request);

        $GLOBALS['xoopsDB']->freeRecordSet($request);

        // A collation looks like latin1_swedish. We only need the character set.

        [$context['database_charset']] = explode('_', $column_info['Collation']);

        $context['database_charset'] = in_array($context['database_charset'], $charsets, true) ? array_search($context['database_charset'], $charsets, true) : $context['database_charset'];

        // No need to convert to UTF-8 if it already is.

        if ('utf8' === $db_character_set && !empty($modSettings['global_character_set']) && 'UTF-8' === $modSettings['global_character_set']) {
        ...
        if (!isset($charsets[strtr(mb_strtolower($context['charset_detected']), ['utf' => 'UTF', 'iso' => 'ISO'])])) {
    ...
    if (1 === preg_match('~^`(.+?)`\.(.+?)$~', $db_prefix, $match)) {
    ...
    while (false !== ($table_info = $GLOBALS['xoopsDB']->fetchArray($queryTables))) {
        ...
        while (false !== ($column_info = $GLOBALS['xoopsDB']->fetchArray($queryColumns))) {
            // Only text'ish columns have a character set and need converting.

            if (false !== mb_strpos($column_info['Type'], 'text') || false !== mb_strpos($column_info['Type'], 'char')) {
                $collation = empty($column_info['Collation']) || 'NULL' === $column_info['Collation'] ? $table_info['Collation'] : $column_info['Collation'];

                if (!empty($collation) && 'NULL' !== $collation) {
        ...
        $GLOBALS['xoopsDB']->freeRecordSet($queryColumns);

        // Only change the column if the data doesn't match the current charset.

        if ((1 === count($table_charsets) && key($table_charsets) !== $charsets[$_POST['src_charset']]) || count($table_charsets) > 1) {
            $updates_blob = '';
            ...
            foreach ($table_charsets as $charset => $columns) {
                if ($charset !== $charsets[$_POST['src_charset']]) {
                    foreach ($columns as $column) {
                        $updates_blob .= "
							CHANGE COLUMN $column[Field] $column[Field] " . strtr($column['Type'], ['text' => 'blob', 'char' => 'binary']) . ('YES' === $column['Null'] ? ' NULL' : ' NOT NULL') . (false === mb_strpos($column['Type'], 'char') ? '' : " default '$column[Default]'") . ',';

                        $updates_text .= "
							CHANGE COLUMN $column[Field] $column[Field] $column[Type] CHARACTER SET " . $charsets[$_POST['src_charset']] . ('YES' === $column['Null'] ? '' : ' NOT NULL') . (false === mb_strpos($column['Type'], 'char') ? '' : " default '$column[Default]'") . ',';
                    }
                }
            }
            ...
            db_query(
                "
				ALTER TABLE $table_info[Name]" . mb_substr($updates_blob, 0, -1),
                __FILE__,
                __LINE__
            );
            ...
            db_query(
                "
				ALTER TABLE $table_info[Name]" . mb_substr($updates_text, 0, -1),
                __FILE__,
                __LINE__
            );
        }
        ...
        if ('utf8' !== $charsets[$_POST['src_charset']]) {
            db_query(
                "
    ...
    $GLOBALS['xoopsDB']->freeRecordSet($queryTables);

    // Let the settings know we have a new character set.

    updateSettings(['global_character_set' => 'UTF-8']);
...
function ConvertEntities()
{
    ...
    if ('UTF-8' !== $modSettings['global_character_set'] || !isset($db_character_set) || 'utf8' !== $db_character_set) {
    ...
    $entity_replace = create_function(
        '$string',
        '
    ...
    for (; $context['table'] < $context['num_tables']; $context['table']++) {
        ...
        while (false !== ($column_info = $GLOBALS['xoopsDB']->fetchArray($request))) {
            if (false !== mb_strpos($column_info['Type'], 'text') || false !== mb_strpos($column_info['Type'], 'char')) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            if ('PRIMARY' === $row['Key_name'] && 1 == $row['Seq_in_index']) {
        ...
        [$max_value] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        while ($context['start'] <= $max_value) {
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
                ...
                foreach ($row as $column_name => $column_value) {
                    if ($column_name !== $primary_key && false !== mb_strpos($column_value, '&#')) {
