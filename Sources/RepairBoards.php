...
function RepairBoards()
{
    ...
    if (!isset($_GET['fixErrors'])) {
        ...
        $to_fix = $_SESSION['repairboards_to_fix'] ?? [];

        require_once $sourcedir . '/Subs-Boards.php';

        // Get the MySQL version for future reference.

        $mysql_version = $GLOBALS['xoopsDB']->getServerVersion($db_connection);

        if (empty($to_fix) || in_array('zero_ids', $to_fix, true)) {
        ...
        if (empty($to_fix) || in_array('missing_messages', $to_fix, true)) {
            ...
            if ($GLOBALS['xoopsDB']->getRowsNum($resultTopic) > 0) {
                $stupidTopics = [];

                while (false !== ($topicArray = $GLOBALS['xoopsDB']->fetchArray($resultTopic))) {
            ...
            $GLOBALS['xoopsDB']->freeRecordSet($resultTopic);
        }

        // Fix all messages that have a topic ID that cannot be found in the topics table.

        if (empty($to_fix) || in_array('missing_topics', $to_fix, true)) {
            $result = db_query(
                "
				SELECT
            ...
            while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
                ...
                db_query(
					...
					VALUES ($row[ID_BOARD], $memberStartedID, $memberUpdatedID,
						$row[myID_FIRST_MSG], $row[myID_LAST_MSG], $row[myNumReplies])",
                    __FILE__,
                    __LINE__
                );
        ...
        if (empty($to_fix) || in_array('stats_topics', $to_fix, true)) {
            $resultTopic = db_query(
                "
				SELECT
        ...
        if (empty($to_fix) || in_array('missing_boards', $to_fix, true)) {
            ...
            if ($GLOBALS['xoopsDB']->getRowsNum($resultTopics) > 0) {
                createSalvageArea();
            }

            while (false !== ($topicArray = $GLOBALS['xoopsDB']->fetchArray($resultTopics))) {
                db_query(
                    "
					INSERT INTO {$db_prefix}boards
						(ID_CAT, name, description, numTopics, numPosts, memberGroups)
					VALUES ($salvageCatID, 'Salvaged board', '', $topicArray[myNumTopics], $topicArray[myNumPosts], '1')",
                    __FILE__,
                    __LINE__
                );
                ...
                db_query(
					...
					WHERE ID_BOARD = $topicArray[ID_BOARD]",
                    __FILE__,
                    __LINE__
                );
            ...
            $GLOBALS['xoopsDB']->freeRecordSet($resultTopics);
        }

        // Fix all boards that have a cat ID that cannot be found in the cats table.

        if (empty($to_fix) || in_array('missing_categories', $to_fix, true)) {
            ...
            if ($GLOBALS['xoopsDB']->getRowsNum($resultBoards) > 0) {
                createSalvageArea();
            }

            while (false !== ($boardArray = $GLOBALS['xoopsDB']->fetchArray($resultBoards))) {
                db_query(
                    "
					UPDATE {$db_prefix}boards
					SET ID_CAT = $salvageCatID
					WHERE ID_CAT = $boardArray[ID_CAT]",
                    __FILE__,
                    __LINE__
                );
            }

            $GLOBALS['xoopsDB']->freeRecordSet($resultBoards);
        }

        // Last step-make sure all non-guest posters still exist.

        if (empty($to_fix) || in_array('missing_posters', $to_fix, true)) {
            $result = db_query(
                "
				SELECT m.ID_MSG
				FROM {$db_prefix}messages AS m
        ...
        if (empty($to_fix) || in_array('missing_parents', $to_fix, true)) {
            $resultParents = db_query(
				...
				GROUP BY b.ID_PARENT",
                __FILE__,
                __LINE__
            );

            if ($GLOBALS['xoopsDB']->getRowsNum($resultParents) > 0) {
                createSalvageArea();
            }

            while (false !== ($parentArray = $GLOBALS['xoopsDB']->fetchArray($resultParents))) {
                db_query(
                    "
					UPDATE {$db_prefix}boards
					SET ID_PARENT = $salvageBoardID, ID_CAT = $salvageCatID, childLevel = 1
					WHERE ID_PARENT = $parentArray[ID_PARENT]",
                    __FILE__,
                    __LINE__
                );
            }

            $GLOBALS['xoopsDB']->freeRecordSet($resultParents);
        }

        if (empty($to_fix) || in_array('missing_polls', $to_fix, true)) {
            if (version_compare($mysql_version, '4.0.4') >= 0) {
                ...
                while (false !== ($rowPolls = $GLOBALS['xoopsDB']->fetchArray($resultPolls))) {
                    $polls[] = $rowPolls['ID_POLL'];
                }

                $GLOBALS['xoopsDB']->freeRecordSet($resultPolls);
        ...
        if (empty($to_fix) || in_array('missing_calendar_topics', $to_fix, true)) {
            if (version_compare($mysql_version, '4.0.4') >= 0) {
                db_query(
                    "
                ...
                while (false !== ($rowEvents = $GLOBALS['xoopsDB']->fetchArray($resultEvents))) {
                    $events[] = $rowEvents['ID_TOPIC'];
                }

                $GLOBALS['xoopsDB']->freeRecordSet($resultEvents);
        ...
        if (empty($to_fix) || in_array('missing_log_topics', $to_fix, true)) {
            $result = db_query(
                "
				SELECT lt.ID_TOPIC
        ...
        if (empty($to_fix) || in_array('missing_log_topics_members', $to_fix, true)) {
            $result = db_query(
                "
				SELECT lt.ID_MEMBER
				FROM {$db_prefix}log_topics AS lt
        ...
        if (empty($to_fix) || in_array('missing_log_boards', $to_fix, true)) {
            $result = db_query(
                "
				SELECT lb.ID_BOARD
        ...
        if (empty($to_fix) || in_array('missing_log_boards_members', $to_fix, true)) {
            $result = db_query(
                "
				SELECT lb.ID_MEMBER
				FROM {$db_prefix}log_boards AS lb
        ...
        if (empty($to_fix) || in_array('missing_log_mark_read', $to_fix, true)) {
            $result = db_query(
                "
				SELECT lmr.ID_BOARD
        ...
        if (empty($to_fix) || in_array('missing_log_mark_read_members', $to_fix, true)) {
            $result = db_query(
                "
        ...
        if (empty($to_fix) || in_array('missing_pms', $to_fix, true)) {
            $result = db_query(
                "
				SELECT pmr.ID_PM
        ...
        if (empty($to_fix) || in_array('missing_recipients', $to_fix, true)) {
            $result = db_query(
                "
        ...
        if (empty($to_fix) || in_array('missing_senders', $to_fix, true)) {
            $result = db_query(
                "
				SELECT pm.ID_PM
        ...
        if (empty($to_fix) || in_array('missing_notify_members', $to_fix, true)) {
            $result = db_query(
                "
				SELECT ln.ID_MEMBER
        ...
        if (empty($to_fix) || in_array('missing_cached_subject', $to_fix, true)) {
            $request = db_query(
                "
        ...
        if (empty($to_fix) || in_array('missing_topic_for_cache', $to_fix, true)) {
            $request = db_query(
                "
				SELECT lss.ID_TOPIC
            ...
            if (!empty($deleteTopics)) {
                db_query(
					...
					WHERE ID_TOPIC IN (" . implode(', ', $deleteTopics) . ')',
                    __FILE__,
                    __LINE__
                );
            }
        ...
        if (empty($to_fix) || in_array('missing_member_vote', $to_fix, true)) {
            $result = db_query(
                "
				SELECT lp.ID_MEMBER
        ...
        if (empty($to_fix) || in_array('missing_log_poll_vote', $to_fix, true)) {
            $request = db_query(
                "
				SELECT lp.ID_POLL
				FROM {$db_prefix}log_polls AS lp
...
function findForumErrors()
{
    ...
    if ($_GET['step'] <= 0) {
        ...
        [$zeroTopics] = $GLOBALS['xoopsDB']->fetchRow($result);
        ...
        [$zeroMessages] = $GLOBALS['xoopsDB']->fetchRow($result);
    ...
    if ($_GET['step'] <= 7) {
        ...
        [$messages] = $GLOBALS['xoopsDB']->fetchRow($result);
    ...
    if ($_GET['step'] <= 9) {
        ...
        [$polls] = $GLOBALS['xoopsDB']->fetchRow($result);
    ...
    if ($_GET['step'] <= 11) {
        ...
        [$members] = $GLOBALS['xoopsDB']->fetchRow($result);
        ...
        for (; $_GET['substep'] < $members; $_GET['substep'] += 250) {
            ...
            $result = db_query(
				...
				FROM {$db_prefix}log_topics AS lt
					LEFT JOIN {$db_prefix}topics AS t ON (t.ID_TOPIC = lt.ID_TOPIC)
				WHERE t.ID_TOPIC IS NULL
					AND lt.ID_MEMBER BETWEEN $_GET[substep] AND $_GET[substep] + 249",
                __FILE__,
                __LINE__
            );
            ...
            if (0 != $GLOBALS['xoopsDB']->getRowsNum($result) && !in_array('missing_log_topics', $to_fix, true)) {
    ...
    if ($_GET['step'] <= 12) {
        ...
        for (; $_GET['substep'] < $members; $_GET['substep'] += 150) {
            ...
            if (0 != $GLOBALS['xoopsDB']->getRowsNum($result) && !in_array('missing_log_topics_members', $to_fix, true)) {
    ...
    if ($_GET['step'] <= 17) {
        ...
        [$pms] = $GLOBALS['xoopsDB']->fetchRow($result);
    ...
    if ($_GET['step'] <= 21) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            if (0 != count(text2words($row['subject']))) {
    ...
    if ($_GET['step'] <= 22) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            $context['repair_errors'][] = sprintf($txt['repair_missing_topic_for_cache'], htmlspecialchars($row['word'], ENT_QUOTES | ENT_HTML5));
        }
...
function createSalvageArea()
{
    ...
    if (0 != $GLOBALS['xoopsDB']->getRowsNum($result)) {
        [$salvageCatID] = $GLOBALS['xoopsDB']->fetchRow($result);
    }
    ...
    if (empty($salveageCatID)) {
        db_query(
			...
			INSERT INTO {$db_prefix}categories
				(name, catOrder)
			VALUES (SUBSTRING('" . addslashes($txt['salvaged_category_name']) . "', 1, 255), -1)",
            __FILE__,
            __LINE__
        );
    ...
    if (0 != $GLOBALS['xoopsDB']->getRowsNum($result)) {
        [$salvageBoardID] = $GLOBALS['xoopsDB']->fetchRow($result);
    }
    ...
    if (empty($salvageBoardID)) {
        db_query(
			...
			VALUES (SUBSTRING('" . addslashes($txt['salvaged_board_name']) . "', 1, 255), SUBSTRING('" . addslashes($txt['salvaged_board_description']) . "', 1, 255), $salvageCatID, '1', -1)",
            __FILE__,
            __LINE__
        );
