...
function setLoginCookie($cookie_length, $id, $password = '')
{
    ...
    if (isset($_COOKIE[$cookiename])) {
        ...
        if (isset($array[3]) && $array[3] != $cookie_state) {
            ...
            setcookie($cookiename, serialize([0, '', 0]), time() - 3600, $cookie_url[1], $cookie_url[0], 0);
        }
    }

    // Get the data and path to set it on.

    $data = serialize(empty($id) ? [0, '', 0] : [$id, $password, time() + $cookie_length, $cookie_state]);
...
function url_parts($local, $global)
{
    ...
    if ($global && 0 == preg_match('~^\d{1,3}(\.\d{1,3}){3}$~', $parsed_url['host'])) {
        // If we can't figure it out, just skip it.

        if (1 == preg_match('~(?:[^\.]+\.)?([^\.]{2,}\..+)\z~i', $parsed_url['host'], $parts)) {
...
function adminLogin()
{
    ...
    $q = mb_strpos($scripturl, '?');

    if (false !== $q) {
        parse_str(preg_replace('/&(\w+)(?=&|$)/', '&$1=', strtr(mb_substr($scripturl, $q + 1), ';', '&')), $temp);
    ...
    $context['get_data'] = mb_substr($context['get_data'], 0, -1);
...
function adminLogin_outputPostVars($k, $v)
{
    if (!is_array($v)) {
        return '
<input type="hidden" name="' . $k . '" value="' . strtr(stripslashes($v), ['"' => '&quot;', '<' => '&lt;', '>' => '&gt;']) . '">';
    }  
...
function show_db_error($loadavg = false)
{
    ...
    if (false === $loadavg) {
        ...
        if ($db_last_error < time() - 3600 * 24 * 3 && empty($maintenance) && !empty($db_error_send)) {
            ...
            $mysql_error = @$GLOBALS['xoopsDB']->error($db_connection);

            @mail(
                $webmaster_email,
                $mbname . ': SMF Database Error!',
                'There has been a problem with the database!' . ('' == $mysql_error ? '' : "\nMySQL reported:\n" . $mysql_error) . "\n\nThis is a notice email to let you know that SMF could not connect to the database, contact your host if this continues."
            );
        }
    }
...
function findMembers($names, $use_wildcards = false, $buddies_only = false, $max = null)
{
    ...
    foreach ($names as $i => $name) {
        ...
        $maybe_email |= false !== mb_strpos($name, '@');
    ...
    $request = db_query(
		...
		WHERE (memberName $comparison '" . implode("' OR memberName $comparison '", $names) . "'
			...
			AND is_activated IN (1, 11)' . (null === $max ? '' : '
		LIMIT ' . (int)$max),
        __FILE__,
        __LINE__
    );
...
function JSMembers()
{
    ...
    $context['input_box_name'] = isset($_REQUEST['input']) && 1 === preg_match('~^[\w-]+$~', $_REQUEST['input']) ? $_REQUEST['input'] : 'to';
    ...
    if (isset($_REQUEST['search'])) {
        ...
        $context['results'] = findMembers([$_REQUEST['search']], true, $context['buddy_search']);

        $total_results = count($context['results']);

        $context['page_index'] = constructPageIndex(
            $scripturl . '?action=findmember;search=' . $context['last_search'] . ';sesc=' . $context['session_id'] . ';input=' . $context['input_box_name'] . ($context['quote_results'] ? ';quote=1' : '') . ($context['buddy_search'] ? ';buddies' : ''),
            $_REQUEST['start'],
            $total_results,
            7
        );
...
function RequestMembers()
{
    ...
    $request = db_query(
		...
		LIMIT ' . (mb_strlen($_REQUEST['search']) <= 2 ? '100' : '800'),
        __FILE__,
        __LINE__
    );
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        if (0 != preg_match('~&#\d+;~', $row['realName'])) {
...
function resetPassword($memID, $username = null)
{
    ...
    [$user, $email] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    $newPassword = mb_substr(preg_replace('/\W/', '', md5(mt_rand())), 0, 10);

    $newPassword_sha1 = sha1(mb_strtolower($user) . $newPassword);
    ...
    if (null !== $username) {
        ...
        if (in_array($user, ['_', '|'], true) || 0 != preg_match('~[<>&"\'=\\\]~', $user) || false !== mb_strpos($user, '[code') || false !== mb_strpos($user, '[/code')) {
            fatal_lang_error(240, false);
        }

        if (false !== mb_stristr($user, $txt[28])) {
        ...
        if (isReservedName($user, $memID, false)) {
            fatal_error('(' . htmlspecialchars($user, ENT_QUOTES | ENT_HTML5) . ') ' . $txt[473], false);
        }

        // Update the database...

        updateMemberData($memID, ['memberName' => '\'' . $user . '\'', 'passwd' => '\'' . $newPassword_sha1 . '\'']);
    } else {
...
function validatePassword($password, $username, $restrict_in = [])
{
    ...
    if (mb_strlen($password) < (empty($modSettings['password_strength']) ? 4 : 8)) {
        return 'short';
    }
    ...
    if (0 != preg_match('~\b' . preg_quote($password, '~') . '\b~', implode(' ', $restrict_in))) {
    ...
    if (1 == $modSettings['password_strength']) {
        return null;
    }

    // Otherwise, hard test next, check for numbers and letters, uppercase too.

    $good = 0 != preg_match('~(\D\d|\d\D)~', $password);
