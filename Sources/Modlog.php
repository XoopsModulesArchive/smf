...
function ViewModlog()
{
    ...
    if (isset($_POST['removeall'])) {
        db_query(
			...
			WHERE logtime < " . (time() - $hoursdisable * 3600),
            __FILE__,
            __LINE__
        );
        ...
        db_query(
			...
			WHERE ID_ACTION IN ('" . implode("', '", array_unique($_POST['delete'])) . "')
				AND logTime < " . (time() - $hoursdisable * 3600),
            __FILE__,
            __LINE__
        );
    }
    ...
    $context['columns'] = [
        ...
        'time' => ['sql' => 'lm.logTime', 'label' => $txt['modlog_date']],
        'member' => ['sql' => 'mem.realName', 'label' => $txt['modlog_member']],
        'group' => ['sql' => 'mg.groupName', 'label' => $txt['modlog_position']],
    ...
    if (isset($_REQUEST['params'])) {
        $search_params = base64_decode(strtr($_REQUEST['params'], [' ' => '+']), true);
    ...
    if ('action' == $search_params['type'] && !empty($search_params['string'])) {
        // For the moment they can only search for ONE action!

        foreach ($descriptions as $key => $text) {
            if (false !== mb_strpos($text, $search_params['string'])) {
    ...
    [$context['entry_count']] = $GLOBALS['xoopsDB']->fetchRow($result);
    ...
    $result = db_query(
		...
		LIMIT $context[start], $displaypage",
        __FILE__,
        __LINE__
    );
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($result))) {
        ...
        $context['entries'][$row['ID_ACTION']] = [
            ...
            'extra' => $row['extra'],
            'action' => $descriptions[$row['action']] ?? $row['action'],
        ];
    }
