...
function Display()
{
    ...
    if (isset($_REQUEST['prev_next']) && ('prev' == $_REQUEST['prev_next'] || 'next' == $_REQUEST['prev_next'])) {
        // No use in calculating the next topic if there's only one.

        if ($board_info['num_topics'] > 1) {
            // Just prepare some variables that are used in the query.

            $gt_lt = 'prev' == $_REQUEST['prev_next'] ? '>' : '<';

            $order = 'prev' == $_REQUEST['prev_next'] ? '' : ' DESC';
            ...
            [$topic] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    $topicinfo = $GLOBALS['xoopsDB']->fetchArray($request);
    ...
    if (!is_numeric($_REQUEST['start'])) {
        ...
        if ('new' == $_REQUEST['start']) {
            ...
            if ($user_info['is_guest']) {
                ...
                [$new_from] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        if ('from' == mb_substr($_REQUEST['start'], 0, 4)) {
            $timestamp = (int)mb_substr($_REQUEST['start'], 4);
            ...
            if (0 === $timestamp) {
                ...
                [$context['start_from']] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        elseif ('msg' == mb_substr($_REQUEST['start'], 0, 3)) {
            $virtual_msg = (int)mb_substr($_REQUEST['start'], 3);
    ...
    if (!$user_info['is_guest']) {
        ...
        if (empty($flag)) {
            db_query(
				...
				VALUES ($modSettings[maxMsgID], $ID_MEMBER, $topic)",
                __FILE__,
                __LINE__
            );
        }
        ...
        elseif (isset($_REQUEST['topicseen'])) {
            ...
            [$numNewTopics] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (!empty($settings['display_who_viewing'])) {
        ...
        $request = db_query(
			...
			WHERE INSTR(lo.url, 's:5:\"topic\";i:$topic;') OR lo.session = '" . ($user_info['is_guest'] ? 'ip' . $user_info['ip'] : session_id()) . "'",
            __FILE__,
            __LINE__
        );
    ...
    if (!empty($modSettings['enableAllMessages']) && $modSettings['defaultMaxMessages'] < $topicinfo['numReplies'] + 1 && $modSettings['enableAllMessages'] > $topicinfo['numReplies'] + 1) {
    ...
    if (!empty($board_info['moderators'])) {
        ...
        $context['linktree'][count($context['linktree']) - 2]['extra_after'] = ' (' . (1 == count($context['link_moderators']) ? $txt[298] : $txt[299]) . ': ' . implode(', ', $context['link_moderators']) . ')';
    }
    ...
    $context['is_poll'] = $topicinfo['ID_POLL'] > 0 && '1' == $modSettings['pollMode'] && allowedTo('poll_view');
    ...
    if ($context['is_poll']) {
        ...
        $pollinfo = $GLOBALS['xoopsDB']->fetchArray($request);
        ...
        $request = db_query(
			...
			WHERE pc.ID_POLL = $topicinfo[ID_POLL]",
            __FILE__,
            __LINE__
        );
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            ...
            $pollinfo['has_voted'] |= -1 != $row['votedThis'];
        }
        ...
        $context['poll'] = [
            ...
            'starter' => [
                ...
                'name' => $row['posterName'],
                'href' => 0 == $pollinfo['ID_MEMBER'] ? '' : $scripturl . '?action=profile;u=' . $pollinfo['ID_MEMBER'],
                'link' => 0 == $pollinfo['ID_MEMBER'] ? $row['posterName'] : '<a href="' . $scripturl . '?action=profile;u=' . $pollinfo['ID_MEMBER'] . '">' . $row['posterName'] . '</a>',
            ],
        ];
        ...
        $context['allow_poll_view'] = allowedTo('moderate_board') || 0 == $pollinfo['hideResults'] || (1 == $pollinfo['hideResults'] && $context['poll']['has_voted']) || $context['poll']['is_expired'];
    ...
    if ($start > $topicinfo['numReplies'] / 2 && -1 != $modSettings['defaultMaxMessages']) {
        $ascending = !$ascending;
    ...
    $request = db_query(
		...
		FROM {$db_prefix}messages
		WHERE ID_TOPIC = $topic
		ORDER BY ID_MSG " . ($ascending ? '' : 'DESC') . (-1 == $modSettings['defaultMaxMessages'] ? '' : "
		LIMIT $start, $limit"),
        __FILE__,
        __LINE__
    );
    ...
    if (!empty($messages)) {
        ...
        if (empty($options['view_newest_first'])) {
            ...
            $context['first_new_message'] = isset($context['start_from']) && $topicinfo['numReplies'] - $context['start_from'] == $_REQUEST['start'];
        }
    } else {
    ...
    $context['can_add_poll'] &= '1' == $modSettings['pollMode'] && $topicinfo['ID_POLL'] <= 0;

    $context['can_remove_poll'] &= '1' == $modSettings['pollMode'] && $topicinfo['ID_POLL'] > 0;
...
function prepareDisplayContext($reset = false)
{
    ...
    $context['can_remove_post'] |= allowedTo('delete_own') && (empty($modSettings['edit_disable_time']) || time() <= $message['posterTime'] + $modSettings['edit_disable_time'] * 60) && $message['ID_MEMBER'] == $ID_MEMBER;
    ...
    $output = [
        ...
        'can_modify' => allowedTo('modify_any') || (allowedTo('modify_replies') && $context['user']['started'])
                        || (allowedTo('modify_own') && $message['ID_MEMBER'] == $ID_MEMBER
                            && (empty($modSettings['edit_disable_time'])
                                || time() < $message['posterTime'] + $modSettings['edit_disable_time'] * 60)),
        'can_remove' => allowedTo('delete_any') || (allowedTo('delete_replies') && $context['user']['started'])
                        || (allowedTo('delete_own') && $message['ID_MEMBER'] == $ID_MEMBER
                            && (empty($modSettings['edit_disable_time'])
                                || time() < $message['posterTime'] + $modSettings['edit_disable_time'] * 60)),
...
function Download()
{
    ...
    if (isset($_REQUEST['type']) && 'avatar' == $_REQUEST['type']) {
        $request = db_query(
            "
    ...
    [$real_filename, $ID_ATTACH, $attachmentType] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if (!empty($_SERVER['HTTP_IF_NONE_MATCH']) && false !== mb_strpos($_SERVER['HTTP_IF_NONE_MATCH'], $file_md5)) {
        ob_end_clean();
    ...
    if (!isset($_REQUEST['image']) && in_array(mb_substr($real_filename, -4), ['.gif', '.jpg', '.bmp', '.png', 'jpeg', 'tiff'], true)) {
    ...
    if (!isset($_REQUEST['image']) && in_array(mb_substr($real_filename, -4), ['.txt', '.css', '.htm', '.php', '.xml'], true)) {
        if (false !== mb_strpos($_SERVER['HTTP_USER_AGENT'], 'Windows')) {
    ...
    elseif (isset($callback) || null === @readfile($filename)) {
...
function loadAttachmentContext($ID_MSG)
{
    ...
    if (isset($attachments[$ID_MSG]) && !empty($modSettings['attachmentEnable'])) {
        foreach ($attachments[$ID_MSG] as $i => $attachment) {
            ...
            if (!empty($modSettings['attachmentThumbnails']) && !empty($modSettings['attachmentThumbWidth']) && !empty($modSettings['attachmentThumbHeight']) && ($attachment['width'] > $modSettings['attachmentThumbWidth'] || $attachment['height'] > $modSettings['attachmentThumbHeight'])
                && mb_strlen(
                    $attachment['filename']
                ) < 249) {
                // A proper thumb doesn't exist yet? Create one!

                if (empty($attachment['ID_THUMB']) || $attachment['thumb_width'] > $modSettings['attachmentThumbWidth'] || $attachment['thumb_height'] > $modSettings['attachmentThumbHeight']
                    ...
                    if (createThumbnail($filename, $modSettings['attachmentThumbWidth'], $modSettings['attachmentThumbHeight'])) {
                        ...
                        [$attachment['thumb_width'], $attachment['thumb_height']] = @getimagesize($filename . '_thumb');
                        ...
                        db_query(
							...
							VALUES ($ID_MSG, 3, '$thumb_filename', " . (int)$thumb_size . ', ' . (int)$attachment['thumb_width'] . ', ' . (int)$attachment['thumb_height'] . ')',
                            __FILE__,
                            __LINE__
                        );
