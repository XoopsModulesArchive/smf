...
function getLastPost()
{
    ...
    $row['body'] = strip_tags(strtr(parse_bbc($row['body'], $row['smileysEnabled']), ['<br>' => '&#10;']));
...
function RecentPosts()
{
    ...
    if (!empty($_REQUEST['c']) && empty($board)) {
        ...
        [$total_posts] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        foreach ($list as $permission => $allowed) {
            ...
            foreach ($boards as $board_id) {
                ...
                foreach ($board_ids[$type][$board_id] as $counter) {
                    if ('any' == $type || $context['posts'][$counter]['poster']['id'] == $ID_MEMBER) {
...
function UnreadTopics()
{
    ...
    } elseif ('unread' != $_REQUEST['action'] && !empty($context['load_average']) && !empty($modSettings['loadavg_unreadreplies']) && $context['load_average'] >= $modSettings['loadavg_unreadreplies']) {
    ...
    if (!empty($board)) {
        ...
        $request = db_query(
			...
			WHERE $user_info[query_see_board]
				AND b.ID_BOARD IN (" . implode(', ', $_REQUEST['boards']) . ')',
            __FILE__,
            __LINE__
        );
        ...
        $request = db_query(
			...
			FROM {$db_prefix}boards AS b
			WHERE $user_info[query_see_board]
				AND b.ID_CAT IN (" . implode(', ', $_REQUEST['c']) . ')',
            __FILE__,
            __LINE__
        );
    ...
    if (!empty($_REQUEST['c']) && is_array($_REQUEST['c']) && 1 == count($_REQUEST['c'])) {
        $request = db_query(
            "
			SELECT name
			FROM {$db_prefix}categories
    ...
    $context['linktree'][] = [
        ...
        'name' => 'unread' == $_REQUEST['action'] ? $txt['unread_topics_visit'] : $txt['unread_replies'],
    ];
    ...
    if (WIRELESS) {
        ...
        $context['sub_template'] = 'unread' == $_REQUEST['action'] ? 'unread' : 'replies';
    }
    ...
    $is_topics = 'unread' == $_REQUEST['action'];
    ...
    if ($context['showing_all_topics']) {
        if (!empty($board)) {
            ...
            [$earliest_msg] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        if (empty($earliest_msg)) {
            ...
            if (isset($_SESSION['cached_log_time']) && $_SESSION['cached_log_time'][0] + 45 > time()) {
                ...
                [$earliest_msg2] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if ($modSettings['totalMessages'] > 100000 && $context['showing_all_topics']) {
        ...
        $have_temp_table = false !== db_query(
            "
    ...
    if ($context['showing_all_topics'] && $have_temp_table) {
        ...
        [$num_topics, $min_message] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        $context['links'] = [
            ...
            'prev' => $_REQUEST['start'] >= $modSettings['defaultMaxTopics'] ? $scripturl
                                                                                . '?action='
                                                                                . $_REQUEST['action']
                                                                                . ($context['showing_all_topics'] ? ';all' : '')
                                                                                . sprintf($context['querystring_board_limits'], $_REQUEST['start'] - $modSettings['defaultMaxTopics'])
                                                                                . $context['querystring_sort_limits'] : '',
            'next' => $num_topics > $_REQUEST['start'] + $modSettings['defaultMaxTopics'] ? $scripturl
                                                                                             . '?action='
                                                                                             . $_REQUEST['action']
                                                                                             . ($context['showing_all_topics'] ? ';all' : '')
                                                                                             . sprintf($context['querystring_board_limits'], $_REQUEST['start'] + $modSettings['defaultMaxTopics'])
                                                                                             . $context['querystring_sort_limits'] : '',
            'last' => $num_topics > $_REQUEST['start'] + $modSettings['defaultMaxTopics'] ? $scripturl . '?action=' . $_REQUEST['action'] . ($context['showing_all_topics'] ? ';all' : '') . sprintf(
                $context['querystring_board_limits'],
                floor(($num_topics - 1) / $modSettings['defaultMaxTopics']) * $modSettings['defaultMaxTopics']
            ) . $context['querystring_sort_limits'] : '',
            'up' => $scripturl,
        ];
        ...
        if (0 == $num_topics) {
            ...
            if (';start=%d' == $context['querystring_board_limits']) {
                $context['querystring_board_limits'] = '';
            } else {
        ...
        if ($modSettings['totalMessages'] > 100000) {
            ...
            $have_temp_table = false !== db_query(
				...
				SELECT t.ID_TOPIC, t.ID_BOARD, t.ID_LAST_MSG, IFNULL(lmr.ID_MSG, 0) AS ID_MSG" . (!in_array($_REQUEST['sort'], ['t.ID_LAST_MSG', 't.ID_TOPIC'], true) ? ", $_REQUEST[sort] AS sortKey" : '') . "
				FROM ({$db_prefix}topics AS t, {$db_prefix}messages AS m)
					LEFT JOIN {$db_prefix}log_mark_read AS lmr ON (lmr.ID_BOARD = t.ID_BOARD AND lmr.ID_MEMBER = $ID_MEMBER)" . ($sortKey_joins[$_REQUEST['sort']] ?? '') . "
        ...
        if ($have_temp_table) {
            $request = db_query(
				...
				ORDER BY " . (in_array($_REQUEST['sort'], ['t.ID_LAST_MSG', 't.ID_TOPIC'], true) ? $_REQUEST['sort'] : 't.sortKey') . ($ascending ? '' : ' DESC') . "
				LIMIT $_REQUEST[start], $modSettings[defaultMaxTopics]",
                __FILE__,
                __LINE__
            );
            ...
            $request = db_query(
				...
				FROM ({$db_prefix}topics AS t, {$db_prefix}messages AS m" . (false === mb_strpos($_REQUEST['sort'], 'ms.') ? '' : ", {$db_prefix}messages AS ms") . ')' . (false === mb_strpos($_REQUEST['sort'], 'mems.') ? '' : "
					LEFT JOIN {$db_prefix}members AS mems ON (mems.ID_MEMBER = ms.ID_MEMBER)") . "
					LEFT JOIN {$db_prefix}log_topics AS lt ON (lt.ID_TOPIC = t.ID_TOPIC AND lt.ID_MEMBER = $ID_MEMBER)
					LEFT JOIN {$db_prefix}log_mark_read AS lmr ON (lmr.ID_BOARD = t.ID_BOARD AND lmr.ID_MEMBER = $ID_MEMBER)
				WHERE m.ID_TOPIC = t.ID_TOPIC
					...
					AND IFNULL(lt.ID_MSG, IFNULL(lmr.ID_MSG, 0)) < t.ID_LAST_MSG' . (false !== mb_strpos($_REQUEST['sort'], 'ms.') ? '
					AND ms.ID_MSG = t.ID_FIRST_MSG' : '') . '
				ORDER BY ' . $_REQUEST['sort'] . ($ascending ? '' : ' DESC') . "
				LIMIT $_REQUEST[start], $modSettings[defaultMaxTopics]",
                __FILE__,
    ...
    while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
        ...
        $context['topics'][$row['ID_TOPIC']] = [
            ...
            'last_post' => [
                ...
                'href' => $scripturl . '?topic=' . $row['ID_TOPIC'] . (0 == $row['numReplies'] ? '.0' : '.msg' . $row['ID_LAST_MSG']) . ';topicseen#msg' . $row['ID_LAST_MSG'],
                'link' => '<a href="' . $scripturl . '?topic=' . $row['ID_TOPIC'] . (0 == $row['numReplies'] ? '.0' : '.msg' . $row['ID_LAST_MSG']) . ';topicseen#msg' . $row['ID_LAST_MSG'] . '">' . $row['lastSubject'] . '</a>',
            ],
            'new_from' => $row['new_from'],
            'new_href' => $scripturl . '?topic=' . $row['ID_TOPIC'] . '.msg' . $row['new_from'] . ';topicseen#new',
            'href' => $scripturl . '?topic=' . $row['ID_TOPIC'] . (0 == $row['numReplies'] ? '.0' : '.msg' . $row['new_from']) . ';topicseen' . (0 == $row['numReplies'] ? '' : 'new'),
            'link' => '<a href="' . $scripturl . '?topic=' . $row['ID_TOPIC'] . (0 == $row['numReplies'] ? '.0' : '.msg' . $row['new_from']) . ';topicseen#msg' . $row['new_from'] . '">' . $row['firstSubject'] . '</a>',
