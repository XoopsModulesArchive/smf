...
function deleteMembers($users)
{
    ...
    if (!allowedTo('admin_forum') && (1 != count($users) || $users[0] != $ID_MEMBER)) {
        $request = db_query(
            "
			SELECT ID_MEMBER
    ...
    foreach ($users as $user) {
        ...
        logAction('delete_member', ['member' => $user]);
    }
    ...
    $request = db_query(
		...
		FROM {$db_prefix}members
		WHERE FIND_IN_SET(" . implode(', pm_ignore_list) OR FIND_IN_SET(', $users) . ', pm_ignore_list) OR FIND_IN_SET(' . implode(', buddy_list) OR FIND_IN_SET(', $users) . ', buddy_list)',
        __FILE__,
        __LINE__
    );
...
function deleteMembergroups($groups)
{
    ...
    $groups = array_diff($groups, [-1, 0, 1, 3, 4]);
    ...
    foreach ($updates as $additionalGroups => $memberArray) {
        updateMemberData($memberArray, ['additionalGroups' => '\'' . implode(',', array_diff(explode(',', $additionalGroups), $groups)) . '\'']);
    }
    ...
    $request = db_query(
		...
		WHERE FIND_IN_SET(" . implode(', memberGroups) OR FIND_IN_SET(', $groups) . ', memberGroups)',
        __FILE__,
        __LINE__
    );
...
function addMembersToGroup($members, $group, $type = 'auto')
{
    ...
    if (in_array($group, $implicitGroups, true) || empty($members)) {
        return false;
    }

    // Only admins can add admins.

    if (1 == $group && !allowedTo('admin_forum')) {
        return false;
    }

    // Do the actual updates.

    if ('only_additional' == $type) {
        db_query(
            "
			UPDATE {$db_prefix}members
    ...
    } elseif ('only_primary' == $type || 'force_primary' == $type) {
        db_query(
            "
			UPDATE {$db_prefix}members
			SET ID_GROUP = $group
			WHERE ID_MEMBER IN (" . implode(', ', $members) . ')' . ('force_primary' == $type ? '' : "
				AND ID_GROUP = 0
				AND NOT FIND_IN_SET($group, additionalGroups)") . '
			LIMIT ' . count($members),
            __FILE__,
...
function registerMember(&$regOptions)
{
    ...
    if ('admin' == $regOptions['interface']) {
        is_not_guest();

        isAllowedTo('moderate_forum');
    } // If you're an admin, you're special ;).

    elseif ('guest' == $regOptions['interface']) {
    ...
    if (0 != preg_match('~[<>&"\'=\\\]~', $regOptions['username']) || '_' == $regOptions['username'] || '|' == $regOptions['username'] || false !== mb_strpos($regOptions['username'], '[code') || false !== mb_strpos($regOptions['username'], '[/code')) {
        fatal_lang_error(240, false);
    }

    if (false !== mb_stristr($regOptions['username'], $txt[28])) {
        fatal_lang_error(244, true, [$txt[28]]);
    }

    // !!! Separate the sprintf?

    if (empty($regOptions['email']) || 0 === preg_match('~^[0-9A-Za-z=_+\-/][0-9A-Za-z=_\'+\-/\.]*@[\w\-]+(\.[\w\-]+)*(\.[\w]{2,6})$~', stripslashes($regOptions['email'])) || mb_strlen(stripslashes($regOptions['email'])) > 255) {
    ...
    if (!empty($regOptions['check_reserved_name']) && isReservedName($regOptions['username'], 0, false)) {
        if ('chocolate cake' == $regOptions['password']) {
            fatal_error('Sorry, I don\'t take bribes... you\'ll need to come up with a different name.', false);
        }

        fatal_error('(' . htmlspecialchars($regOptions['username'], ENT_QUOTES | ENT_HTML5) . ') ' . $txt[473], false);
    }
    ...
    if ('activation' == $regOptions['require']) {
        $validation_code = mb_substr(preg_replace('/\W/', '', md5(mt_rand())), 0, 10);
    }

    // If you haven't put in a password generated one.

    if ('admin' == $regOptions['interface'] && '' == $regOptions['password']) {
        mt_srand(time() + 1277);

        $regOptions['password'] = mb_substr(preg_replace('/\W/', '', md5(mt_rand())), 0, 10);
    ...
    if (0 != $GLOBALS['xoopsDB']->getRowsNum($request)) {
        fatal_error(sprintf($txt[730], htmlspecialchars($regOptions['email'], ENT_QUOTES | ENT_HTML5)), false);
    }
    ...
    $regOptions['register_vars'] = [
        ...
        'passwd' => '\'' . sha1(mb_strtolower($regOptions['username']) . $regOptions['password']) . '\'',
        'passwordSalt' => '\'' . mb_substr(md5(mt_rand()), 0, 4) . '\'',
        'posts' => 0,
        'dateRegistered' => time(),
    ...
    elseif ('nothing' == $regOptions['require']) {
        $regOptions['register_vars']['is_activated'] = 1;
    } // Maybe it must be activated by email?

    elseif ('activation' == $regOptions['require']) {
    ...
    if (isset($regOptions['memberGroup'])) {
        ...
        $regOptions['register_vars']['ID_GROUP'] = 1 == $regOptions['memberGroup'] && !allowedTo('admin_forum') ? 0 : $regOptions['memberGroup'];
        ...
        if (in_array($regOptions['register_vars']['ID_GROUP'], $unassignableGroups, true)) {
    ...
    $realName = mb_substr($regOptions['register_vars']['realName'], 1, -1);
    ...
    elseif ('activation' == $regOptions['require'] || 'coppa' == $regOptions['require']) {
    ...
    if (!allowedTo('moderate_forum') && ((!empty($modSettings['reserveName']) && $is_name) || !empty($modSettings['reserveUser']) && !$is_name)) {
        ...
        foreach ($reservedNames as $reserved) {
            ...
            if ($checkMe == $reservedCheck || ($func['strpos']($checkMe, $reservedCheck) !== false && empty($modSettings['reserveWord']))) {
                if ($fatal) {
                    fatal_lang_error(244, true, [$reserved]);
                } else {
                    return true;
                }
            }
        ...
        if (censorText($censor_name) != $name) {
            if ($fatal) {
                fatal_lang_error('name_censored', true, [$name]);
            } else {
                return true;
            }
        }
...
function groupsAllowedTo($permission, $board_id = null)
{
    ...
    if (null === $board_id) {
        ...
        while (false !== ($row = $GLOBALS['xoopsDB']->fetchArray($request))) {
            $membergroups['1' === $row['addDeny'] ? 'allowed' : 'denied'][] = $row['ID_GROUP'];
        }
        ...
        if (isset($board_info['id']) && $board_info['id'] == $board_id) {
            $permission_mode = 'no_polls' == $board_info['permission_mode'] ? 2 : ('reply_only' == $board_info['permission_mode'] ? 3 : ('read_only' == $board_info['permission_mode'] ? 4 : 0));
        } elseif (0 !== $board_id) {
            $request = db_query(
                "
            ...
            [$permission_mode] = $GLOBALS['xoopsDB']->fetchRow($request);
        ...
        if (0 !== $board_id && empty($modSettings['permission_enable_by_board']) && in_array($permission, ['post_reply_own', 'post_reply_any', 'post_new', 'poll_post'], true)) {
            $max_allowable_mode = 3;

            if ('post_new' == $permission) {
                $max_allowable_mode = 2;
            } elseif ('poll_post' == $permission) {
        ...
        $request = db_query(
			...
			WHERE bp.permission = '$permission'" . ($moderator_only ? "
				...
				AND bp.ID_BOARD ' . (empty($modSettings['permission_enable_by_board']) || empty($permission_mode) || 0 === $board_id ? '= 0' : 'IN (0, ' . $board_id . ')'),
            __FILE__,
            __LINE__
        );
...
function membersAllowedTo($permission, $board_id = null)
{
    ...
    $include_moderators = in_array(3, $membergroups['allowed'], true) && null !== $board_id;

    $membergroups['allowed'] = array_diff($membergroups['allowed'], [3]);

    $exclude_moderators = in_array(3, $membergroups['denied'], true) && null !== $board_id;

    $membergroups['denied'] = array_diff($membergroups['denied'], [3]);
    ...
    $request = db_query(
        ...
        . '
		WHERE ('
        . ($include_moderators ? 'mods.ID_MEMBER IS NOT NULL OR ' : '')
        . 'ID_GROUP IN ('
        . implode(', ', $membergroups['allowed'])
        . ') OR FIND_IN_SET('
        . implode(', mem.additionalGroups) OR FIND_IN_SET(', $membergroups['allowed'])
        . ', mem.additionalGroups))'
        . (empty($membergroups['denied']) ? '' : '
			AND NOT (' . ($exclude_moderators ? 'mods.ID_MEMBER IS NOT NULL OR ' : '') . 'ID_GROUP IN (' . implode(', ', $membergroups['denied']) . ') OR FIND_IN_SET(' . implode(', mem.additionalGroups) OR FIND_IN_SET(', $membergroups['denied']) . ', mem.additionalGroups))'),
        __FILE__,
        __LINE__
    );
...
function reattributePosts($memID, $email = false, $post_count = false)
{
    ...
    if (false === $email) {
        ...
        [$email] = $GLOBALS['xoopsDB']->fetchRow($request);
    ...
    if ($post_count) {
        ...
        [$messageCount] = $GLOBALS['xoopsDB']->fetchRow($request);
...
function BuddyListToggle()
{
    ...
    if (in_array($_REQUEST['u'], $user_info['buddies'], true)) {
        $user_info['buddies'] = array_diff($user_info['buddies'], [$_REQUEST['u']]);
    } // ...or add if it's not.
    ...
    updateMemberData($ID_MEMBER, ['buddy_list' => "'" . implode(',', $user_info['buddies']) . "'"]);
